...
class GeometryXTime:
    ...
    def uniform_boundary_points(self, n):
        ...
        if self.geometry.dim == 1:
            ...
            s = 2 * sum(
                map(
                    lambda l: l[0] * l[1],
                    itertools.combinations(self.geometry.bbox[1] -
                                           self.geometry.bbox[0], 2), ))
    ...
    def uniform_initial_points(self, n):
        ...
        if len(x) > n:
            x = x[0: n]
        return np.hstack((x, np.full([len(x), 1], t, dtype=config._dtype)))

    def random_initial_points(self, n, random="pseudo"):
        x = self.geometry.random_points(n, random=random)
        t = self.timedomain.t0
        return np.hstack((x, np.full([n, 1], t, dtype=config._dtype)))
