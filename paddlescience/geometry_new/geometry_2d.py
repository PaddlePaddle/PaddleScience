...
class Rectangle(Hypercube):
    ...
    def uniform_boundary_points(self, n):
        ...
        xbot = np.hstack((
            np.linspace(
                self.xmin[0], self.xmax[0], num=nx, endpoint=False)[:, None],
            np.full([nx, 1], self.xmin[1]), ))
        yrig = np.hstack((
            np.full([ny, 1], self.xmax[0]),
            np.linspace(
                self.xmin[1], self.xmax[1], num=ny,
                endpoint=False)[:, None], ))
        xtop = np.hstack((
            np.linspace(
                self.xmin[0], self.xmax[0], num=nx + 1)[1:, None],
            np.full([nx, 1], self.xmax[1]), ))
        ylef = np.hstack((
            np.full([ny, 1], self.xmin[0]),
            np.linspace(
                self.xmin[1], self.xmax[1], num=ny + 1)[1:, None], ))
    ...
    def is_valid(vertices):
        ...
        return (len(vertices) == 4 and
                np.isclose(np.prod(vertices[1] - vertices[0]), 0) and
                np.isclose(np.prod(vertices[2] - vertices[1]), 0) and
                np.isclose(np.prod(vertices[3] - vertices[2]), 0) and
                np.isclose(np.prod(vertices[0] - vertices[3]), 0))
...
class Triangle(Geometry):
    ...
    def __init__(self, x1, x2, x3):
        ...
        self.x1 = np.array(x1, dtype=config._dtype)
        self.x2 = np.array(x2, dtype=config._dtype)
        self.x3 = np.array(x3, dtype=config._dtype)
    ...
    def inside(self, x):
        ...
        return ~np.logical_and(
            np.any(_sign > 0, axis=-1), np.any(_sign < 0, axis=-1))
    ...
    def boundary_normal(self, x):
        ...
        if np.any(
                np.count_nonzero(
                    np.hstack([on12, on23, on31]), axis=-1) > 1):
            raise ValueError(
                "{}.boundary_normal do not accept points on the vertexes.".
    ...
    def random_points(self, n, random="pseudo"):
        ...
        return ((1 - sqrt_r1) * self.x1 + sqrt_r1 *
                (1 - r2) * self.x2 + r2 * sqrt_r1 * self.x3)

    def uniform_boundary_points(self, n):
        density = n / self.perimeter
        x12 = (np.linspace(
            0, 1, num=int(np.ceil(density * self.l12)),
            endpoint=False)[:, None] * self.v12 + self.x1)
        x23 = (np.linspace(
            0, 1, num=int(np.ceil(density * self.l23)),
            endpoint=False)[:, None] * self.v23 + self.x2)
        x31 = (np.linspace(
            0, 1, num=int(np.ceil(density * self.l31)),
    ...
    def random_boundary_points(self, n, random="pseudo"):
        ...
        u = u[np.logical_not(
            np.isclose(u, (self.l12 + self.l23) / self.perimeter))]
        u = u[:n]
...
class Polygon(Geometry):
    ...
    def __init__(self, vertices):
        self.vertices = np.array(vertices, dtype=config._dtype)
        if len(vertices) == 3:
            raise ValueError(
    ...
    def random_points(self, n, random="pseudo"):
        x = np.empty((0, 2), dtype=config._dtype)
        ...
        for i in range(-1, self.nvertices - 1):
            x.append(
                np.linspace(
                    ...
                    endpoint=False, )[:, None] * (self.vertices[
...
def is_on_line_segment(P0, P1, P2):
    ...
    return (
        ...
        np.isclose(
            np.cross(v01, v02) / np.linalg.norm(v01), 0, atol=1e-6)
        # check that projection of P2 to line is between P0 and P1
        and v01 @v02 >= 0 and v01 @v12 <= 0)
