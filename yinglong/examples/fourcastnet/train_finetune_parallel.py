...
if __name__ == "__main__":
    ...
    TEST_FILE_PATH = "../afs_era5/out_of_sample/2018.h5"
    DATA_MEAN_PATH = "../afs_era5/stat/global_means.npy"
    DATA_STD_PATH = "../afs_era5/stat/global_stds.npy"
    DATA_TIME_MEAN_PATH = "../afs_era5/stat/time_means.npy"
    ...
    if NUM_TIMESTAMPS == 2:
        OUTPUT_DIR = (
            f"../output/era5_parallel_unet_025_05_025_finetune_{NUM_TIMESTAMPS}"
            if not args.output_dir
            else args.output_dir
        )
        PRETRAINED_MODEL_PATH = (
            "../output/era5_parallel_unet_025_05_025/checkpoints/latest"
        )
    else:
        OUTPUT_DIR = (
            f"./output/fourcastnet/finetune_{NUM_TIMESTAMPS}"
            if not args.output_dir
            else args.output_dir
        )
        PRETRAINED_MODEL_PATH = (
            f"./output/fourcastnet/finetune_{NUM_TIMESTAMPS-1}/checkpoints/latest"
        )
    ...
    UPDATE_FREQ = 8
    ITERS_PER_EPOCH = len(sup_constraint.data_loader) // UPDATE_FREQ
