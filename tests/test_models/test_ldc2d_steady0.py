...
def ldc2d_steady(npoints, static=True):
...
standard_value = np.load("standard/ldc2d_steady0.npz", allow_pickle=True)


@pytest.mark.ldc2d_steady
@pytest.mark.skipif(
    paddle.distributed.get_world_size() != 1, reason="skip serial case")
def test_ldc2d_steady_0():
    """
    test ldc2d_steady
    """
...
def test_ldc2d_steady_1():
    """
    test ldc2d_steady
    distributed case: padding
    """
    solution = standard_value['dst_solution'].tolist()
    dst_rslt = ldc2d_steady(10)
    compare(solution, dst_rslt, delta=1e-7)
