...
def GetBcIndex(all_xyzuvwp, train_data_points, train_bc_index, circle_center,
               circle_radius):
    # train_data_xyz = all_xyzuvwp[0:int(train_data_points), 0:3]
    train_data_xyz = all_xyzuvwp
    for i in range(len(train_data_xyz)):
        # if the point is in left or right
        if abs(train_data_xyz[i][0] - (-8)) < 1e-4:
            train_bc_index.append(i)
        elif abs(train_data_xyz[i][0] - (25)) < 1e-4:
            train_bc_index.append(i)
        # if the point is in front or back
        elif abs(train_data_xyz[i][1] - (-8)) < 1e-4:
            train_bc_index.append(i)
        elif abs(train_data_xyz[i][1] - (8)) < 1e-4:
            train_bc_index.append(i)
        # if the point is in top or down
        elif abs(train_data_xyz[i][2] - (-0.5)) < 1e-4:
            train_bc_index.append(i)
        elif abs(train_data_xyz[i][2] - (0.5)) < 1e-4:
            train_bc_index.append(i)
        # if the point is in cylinder
        elif ((train_data_xyz[i][0] - circle_center[0])**2 +
              (train_data_xyz[i][1] - circle_center[1])**2 - circle_radius**2
              ) < 1e-4:
            train_bc_index.append(i)
...
if __name__ == "__main__":
    ...
    # Get real data
    all_xyzuvwp = GetRealPhyInfo(0.25)

    # Discretization
    # This code is useless and will not be executed
    ...
    # hard code for MSE check
    # parse the input data
    all_points = all_xyzuvwp.shape[0]
    real_data_ratio = 0.5
    real_data_points = all_points * real_data_ratio
    train_data_points = all_points - real_data_points
    train_bc_index = []
    # parse the bc_index in train_data
    GetBcIndex(all_xyzuvwp, train_data_points, train_bc_index, circle_center,
               circle_radius)
    train_data = all_xyzuvwp[:, 0:3]
    real_data = all_xyzuvwp[-int(real_data_points):]
    train_bc_index = np.array(train_bc_index)
    geo.space_domain = train_data
    geo.space_domain_size = len(train_data)
    geo.bc_index = train_bc_index
    ...
    if use_saved_model is True:
        # load model
        layer_state_dict = paddle.load("checkpoint/net_params_10000")
        opt_state_dict = paddle.load("checkpoint/opt_params_10000")
    ...
    SaveVtkFile(geo, rslt_t1, "mse_flowRe200/fac3d_train_rslt_t10s_0.25")

    u = rslt_t1[:, 0]
    openfoam_u = all_xyzuvwp[:, 3]
    diff_u = u - openfoam_u
    ...
    for i in range(time_step):
        ...
        algo.loss.physic_info = paddle.to_tensor(current_physic_info, dtype="float32")
        # solve
        ...
        SaveVtkFile(geo, rslt, "train_flowRe200/fac3d_train_rslt_t10s_" + format(current_time, '.2f'))
