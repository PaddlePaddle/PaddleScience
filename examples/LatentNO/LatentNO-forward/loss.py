...
class RelLpLoss(base.Metric):
    def __init__(self, p: int, key: str = "y2", normalizer: Optional[object] = None, eps: float = 1e-12, keep_batch: bool = False):
        """
        p: Lp 范数的 p
    ...
    def forward(self, output_dict: Dict[str, paddle.Tensor], label_dict: Dict[str, paddle.Tensor], weight_dicts: Optional[Dict] = None) -> Dict[str, "paddle.Tensor"]:
        ...
        for label_key in label_dict:
            ...
            error = paddle.sum(paddle.abs(pred - target) ** self.p, axis=tuple(range(1, len(pred.shape)))) ** (1.0 / self.p)
            target_norm = paddle.sum(paddle.abs(target) ** self.p, axis=tuple(range(1, len(target.shape)))) ** (1.0 / self.p)
