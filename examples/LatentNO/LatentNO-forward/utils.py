...
class RelLpLoss_time(base.Metric):
    def __init__(self, p: int, key: str = "y2", normalizer: Optional[object] = None, eps: float = 1e-12, keep_batch: bool = False, use_full_sequence: bool = True):
        if keep_batch:
            raise ValueError(f"keep_batch should be False, but got {keep_batch}.")
    ...
    def forward(self, output_dict: Dict[str, paddle.Tensor], label_dict: Dict[str, paddle.Tensor], weight_dicts: Optional[Dict] = None) -> Dict[str, "paddle.Tensor"]:
        losses: Dict[str, paddle.Tensor] = {}
        ...
        for label_key in label_dict:
            if f"{self.key}_steps" in output_dict and not self.use_full_sequence:
                ...
                for s in range(num_steps):
                    ...
                    # 单个时间步损失
                    error = paddle.sum(paddle.abs(pred_s - tgt_s) ** self.p, 
                                     tuple(range(1, len(pred_s.shape)))) ** (1/self.p)
                    target_norm = paddle.sum(paddle.abs(tgt_s) ** self.p, 
                                           tuple(range(1, len(tgt_s.shape)))) ** (1/self.p)
                ...
                pred_full = output_dict[self.key] if self.key in output_dict else output_dict[label_key]
                target_full = label_dict[label_key]
                
                if self.normalizer is not None:
                    device = paddle.get_device()
                    pred_full = self.normalizer.apply_y2(pred_full, device, inverse=True)
                    target_full = self.normalizer.apply_y2(target_full, device, inverse=True)
                
                error = paddle.sum(paddle.abs(pred_full - target_full) ** self.p, 
                                 tuple(range(1, len(pred_full.shape)))) ** (1/self.p)
                target_norm = paddle.sum(paddle.abs(target_full) ** self.p, 
                                       tuple(range(1, len(target_full.shape)))) ** (1/self.p)
