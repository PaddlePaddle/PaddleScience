...
def wing_loss(output: paddle.Tensor, target: paddle.Tensor, width=5, curvature=0.5, reduction="mean"):
    """
...
def label_smoothed_nll_loss(
    ...
    if ignore_index is not None:
        ...
        smooth_loss = paddle.where(pad_mask, paddle.zeros_like(smooth_loss), smooth_loss)
    else:
        nll_loss = -lprobs.take_along_axis(axis=dim, indices=target)
        smooth_loss = -lprobs.sum(axis=dim, keepdim=True)
