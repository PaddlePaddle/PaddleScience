...
import ppsci
from functions import Cifar10GenFuncs, Cifar10DisFuncs, InceptionScore, show_save_image, load_cifar10
...
def evaluate(cfg: DictConfig):
    ...
    # set Loss
    generator_funcs = Cifar10GenFuncs(**cfg["LOSS"]["gen"], discriminator_model=discriminator_model)
    ...
    validator = ppsci.validate.SupervisedValidator(
        ...
        metric={"IS": ppsci.metric.FunctionalMetric(eval_inception_score.inception_score),},
        name="val",
    )
...
def train(cfg: DictConfig):
    ...
    # set Loss
    generator_funcs = Cifar10GenFuncs(**cfg["LOSS"]["gen"], discriminator_model=discriminator_model)
    discriminator_funcs = Cifar10DisFuncs(**cfg["LOSS"]["dis"], discriminator_model=discriminator_model)
    ...
    optimizer_generator = ppsci.optimizer.Adam(
        learning_rate=lr_scheduler_generator, beta1=cfg["TRAIN"]["optimizer"]["beta1"],
        beta2=cfg["TRAIN"]["optimizer"]["beta2"]
    )
    optimizer_discriminator = ppsci.optimizer.Adam(
        learning_rate=lr_scheduler_discriminator, beta1=cfg["TRAIN"]["optimizer"]["beta1"],
        beta2=cfg["TRAIN"]["optimizer"]["beta2"]
    )
