...
class Cifar10GenFuncs:
    ...
    def __init__(self, discriminator_model, acgan_scale_g=0.1, ):
...
class Cifar10DisFuncs:
    ...
    def loss(self, output_dict: Dict, label_dict: Dict, *args):
        ...
        labels = output_dict["labels"]
        disc_fake = self.discriminator_model({'data': fake_image, 'labels': labels})["disc_fake"]
    ...
    def compute_gradient_penalty(self, real_data, fake_data, labels):
        ...
        gradients = paddle.grad(outputs=self.discriminator_model({'data': interpolates, 'labels': labels})["disc_fake"],
                                inputs=interpolates, create_graph=True, retain_graph=False)[0]
        slopes = paddle.sqrt(paddle.sum(paddle.square(gradients), axis=1))
        gradient_penalty = 10 * paddle.mean((slopes - 1.) ** 2)
...
class InceptionScore:
    ...
    def __init__(self, eps=1e-16, splits=10):
...
class ToyDisFuncs:
    ...
    def compute_gradient_penalty(self, real_data, fake_data):
        ...
        interpolates = real_data + (alpha * differences)
        gradients = \
            paddle.grad(outputs=self.discriminator_model({'data': interpolates})["score"],
                        inputs=interpolates, create_graph=True, retain_graph=False)[0]
        slopes = paddle.sqrt(paddle.sum(paddle.square(gradients), axis=1))
        gradient_penalty = self.lamda * paddle.mean((slopes - 1.) ** 2)
        return gradient_penalty
...
def generate_toy_image(true_dist, discriminator, path):
    ...
    disc_map = discriminator({"data": paddle.to_tensor(points)})["score"].numpy().reshape(-1)
...
class MnistDisFuncs:
    ...
    def compute_gradient_penalty(self, real_data, fake_data):
        ...
        alpha = paddle.rand([fake_data.shape[0], 1])
        interpolates = real_data + (alpha * differences)
        gradients = paddle.grad(outputs=self.discriminator_model({'data': interpolates})["score"],
                                inputs=interpolates, create_graph=True, retain_graph=False)[0]
        slopes = paddle.sqrt(paddle.sum(paddle.square(gradients), axis=1))
        gradient_penalty = self.lamda * paddle.mean((slopes - 1.) ** 2)
        return gradient_penalty
...
def load_mnist(data_path, input_keys, ):
