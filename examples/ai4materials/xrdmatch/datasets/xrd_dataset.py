...
class BasicDatasetPaddle(Dataset):
    def __init__(self, algorithm=None, data_path=None, target_path=None, num_classes=2, transform=None, is_ulb=False, strong_transform=None, **kwargs):
        super().__init__()
        self.algorithm = algorithm
        self.data = data_path if not isinstance(data_path, str) else np.load(data_path)
        self.target = target_path if not isinstance(target_path, str) else np.load(target_path)
