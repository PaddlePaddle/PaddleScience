...
from .config import ProcessGroupConfig
from .config import ProcessGroupNode
...
class DistributedManager(object):
    ...
    def __new__(cls):
        ...
        if not hasattr(obj, "_device"):
            obj._device = "gpu:0" if paddle.device.cuda.device_count() else "cpu"
    ...
    def get_available_backend():
        ...
        if (
            ...
            and paddle.core.is_compiled_with_nccl()
        ):
            return "nccl"
        else:
            return "gloo"
    ...
    def setup(
        ...
        if paddle.device.cuda.device_count() >= 0:
            ...
            paddle.set_device(manager.device)
