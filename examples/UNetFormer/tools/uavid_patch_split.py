...
def rgb2label(label):
    label_seg = np.zeros(tuple(label.shape)[:2], dtype=np.uint8)
...
def image_augment(image, mask, mode="train"):
    image_list = []
    mask_list = []
    image_width, image_height = tuple(image.shape)[1], tuple(image.shape)[0]
    mask_width, mask_height = tuple(mask.shape)[1], tuple(mask.shape)[0]
...
def padifneeded(image, mask):
    ...
    assert tuple(img_pad.shape)[0] == 2048 or tuple(img_pad.shape)[1] == 4096, print(
        tuple(img_pad.shape)
    )
...
def patch_format(inp):
    ...
    for img_path, mask_path in zip(img_paths, mask_paths):
        ...
        assert (
            tuple(img.shape) == tuple(mask.shape) and tuple(img.shape)[0] == 2160
        ), print(tuple(img.shape))
        assert tuple(img.shape)[1] == 3840 or tuple(img.shape)[1] == 4096, print(
            tuple(img.shape)
        )
        ...
        for m in range(len(image_list)):
            ...
            for y in range(0, tuple(img.shape)[0], stride[0]):
                for x in range(0, tuple(img.shape)[1], stride[1]):
                    ...
                    if (
                        tuple(img_tile.shape)[0] == split_size[0]
                        and tuple(img_tile.shape)[1] == split_size[1]
                        and tuple(mask_tile.shape)[0] == split_size[0]
                        and tuple(mask_tile.shape)[1] == split_size[1]
                    ):
                        if mode == "train":
...
if __name__ == "__main__":
    ...
    inp = [
        (input_dir, seq, imgs_output_dir, masks_output_dir, mode, split_size, stride)
        for seq in seqs
    ]
    t0 = time.time()
