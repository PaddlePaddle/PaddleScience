...
def car_aug(image, mask):
    ...
    resize_crop_1 = albu.Compose(
        [
            albu.Resize(
                width=int(tuple(image.shape)[0] * 1.25),
                height=int(tuple(image.shape)[1] * 1.25),
            ),
            albu.RandomCrop(width=tuple(image.shape)[0], height=tuple(image.shape)[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
    resize_crop_2 = albu.Compose(
        [
            albu.Resize(
                width=int(tuple(image.shape)[0] * 1.5),
                height=int(tuple(image.shape)[1] * 1.5),
            ),
            albu.RandomCrop(width=tuple(image.shape)[0], height=tuple(image.shape)[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
    resize_crop_3 = albu.Compose(
        [
            albu.Resize(
                width=int(tuple(image.shape)[0] * 1.75),
                height=int(tuple(image.shape)[1] * 1.75),
            ),
            albu.RandomCrop(width=tuple(image.shape)[0], height=tuple(image.shape)[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
    resize_crop_4 = albu.Compose(
        [
            albu.Resize(
                width=int(tuple(image.shape)[0] * 2.0),
                height=int(tuple(image.shape)[1] * 2.0),
            ),
            albu.RandomCrop(width=tuple(image.shape)[0], height=tuple(image.shape)[1]),
        ]
    )(image=image.copy(), mask=mask.copy())
    ...
    image_resize_crop_1, mask_resize_crop_1 = (
        resize_crop_1["image"],
        resize_crop_1["mask"],
    )
    image_resize_crop_2, mask_resize_crop_2 = (
        resize_crop_2["image"],
        resize_crop_2["mask"],
    )
    image_resize_crop_3, mask_resize_crop_3 = (
        resize_crop_3["image"],
        resize_crop_3["mask"],
    )
    image_resize_crop_4, mask_resize_crop_4 = (
        resize_crop_4["image"],
        resize_crop_4["mask"],
    )
            ...
            for x in range(0, tuple(img.shape)[1], stride):
                ...
                if (
                    ...
                    cf = class_pixel_counts / (
                        tuple(mask_tile.shape)[0] * tuple(mask_tile.shape)[1]
                    )
