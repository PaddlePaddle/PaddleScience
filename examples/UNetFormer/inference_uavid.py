...
def get_args():
    ...
    arg(
        ...
        "--image_path",
        type=str,
        default="data/uavid/uavid_test",
        help="Path to  huge image",
    )
...
def load_checkpoint(checkpoint_path, model):
    pretrained_dict = paddle.load(path=str(checkpoint_path))["model_state_dict"]
...
def main():
    ...
    model.cuda(device_id=device2int(config.gpus[0]))
    model.eval()
    if args.tta == "lr":
    ...
    for seq in seqs:
        ...
        for ext in ("*.tif", "*.png", "*.jpg"):
            img_paths.extend(
                glob.glob(os.path.join(args.image_path, str(seq), "Images", ext))
            )
        ...
        for img_path in img_paths:
            ...
            with paddle.no_grad():
                ...
                for input in tqdm(dataloader):
                    raw_predictions = model(
                        input["img"].cuda(device_id=device2int(config.gpus[0]))
                    )
            ...
            assert img_shape == tuple(output_mask.shape)
