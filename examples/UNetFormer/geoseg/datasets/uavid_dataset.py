...
class UAVIDDataset(paddle.io.Dataset):
    def __init__(
        self,
...
class UAVIDDatasetV2(paddle.io.Dataset):
    def __init__(
        self,
        data_root="data/uavid/val",
        mode="val",
        ...
        class_interest=[4, 5, 6],
        class_ratio=[0.1, 0.1, 0.01],
    ):
        self.data_root = data_root
        self.img_dir = img_dir
        self.mask_dir = mask_dir
    ...
    def tiny_object_crop(
        self, img, mask, num_classes, class_interest, class_ratio, crop_size
    ):
        count = 0
        while True:
