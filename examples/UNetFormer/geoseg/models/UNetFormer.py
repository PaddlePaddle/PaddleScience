    ...
    def pad(self, x, ps):
        ...
        if W % ps != 0:
            x = paddle.nn.functional.pad(
                x=x, pad=(0, ps - W % ps), mode="reflect"
            )
        if H % ps != 0:
            x = paddle.nn.functional.pad(
                x=x,
    ...
    def forward(self, x):
        ...
        out = self.attn_x(
            paddle.nn.functional.pad(
                x=attn, pad=(0, 0, 0, 1), mode="reflect"
            )
        ) + self.attn_y(
            paddle.nn.functional.pad(
                x=attn, pad=(0, 1, 0, 0), mode="reflect"
            )
        )
        out = out + local
...
class create_model(paddle.nn.Layer):
    def __init__(self, pretrained=True):
        ...
        self.outputs = []
        self.feature_info = type('FeatureInfo', (object,), {
