...
class MLP(paddle.nn.Layer):
    ...
    def __init__(self, channel_list, dropout=0.0, batch_norm=True,
        relu_first=False):
        super().__init__()
        ...
        for dim in zip(self.channel_list[1:-1]):
            self.norms.append(paddle.nn.BatchNorm1D(num_features=dim) if
                batch_norm else paddle.nn.Identity())
