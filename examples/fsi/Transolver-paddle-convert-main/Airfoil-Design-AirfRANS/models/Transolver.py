...
class Transolver_block(paddle.nn.Layer):
    ...
    def __init__(self, num_heads: int, hidden_dim: int, dropout: float, act
        ...
        self.mlp_new = MLP(hidden_dim, hidden_dim * mlp_ratio, hidden_dim,
            n_layers=0, res=False, act=act)
        if self.last_layer:
...
class Transolver(paddle.nn.Layer):
    ...
    def __init__(self, space_dim=1, n_layers=5, n_hidden=256, dropout=0,
        ...
        32, ref=8, unified_pos=False, device='cuda:1'):
        ...
        if self.unified_pos:
            self.preprocess = MLP(fun_dim + space_dim + self.ref * self.ref,
    ...
    def get_grid(self, my_pos):
        ...
        grid_ref = paddle.concat(x=(gridx, gridy), axis=-1).to(self.device
            ).reshape(batchsize, self.ref ** 2, 2)
        pos = paddle.sqrt(x=paddle.sum(x=(my_pos[:, :, None, :] - grid_ref[
            :, None, :, :]) ** 2, axis=-1)).reshape(batchsize, tuple(my_pos
            .shape)[1], self.ref * self.ref).contiguous()
        return pos
