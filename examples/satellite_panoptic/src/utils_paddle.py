...
def pad_tensor(x, target_shape, pad_value=0):
    ...
    for i in range(len(x.shape) - 1, -1, -1):
        ...
        if pad_size < 0:
            raise ValueError(f"Target size {target_shape[i]} smaller than current size {x.shape[i]} in dim {i}")
    ...
    if tuple(result.shape) != tuple(target_shape):
        ...
        print(f"Debug: input_shape={x.shape}, target_shape={target_shape}, pad={pad}, result_shape={result.shape}")
...
def pad_collate(batch, pad_value=0):
    ...
    if isinstance(elem, paddle.Tensor):
        ...
        return elem_type(*(pad_collate(list(samples), pad_value) for samples in zip(*batch)))
