...
from src.dataset_paddle import PASTIS_Dataset
from src.learning.weight_init_paddle import weight_init
from src.panoptic.metrics_paddle import PanopticMeter
from src.panoptic.paps_loss_paddle import PaPsLoss
from src.utils_paddle import pad_collate
from src.model_utils_paddle import get_ntrainparams
...
def get_model(config):
    ...
    if config.backbone == "utae":
        from src.backbones.utae_paddle import UTAE
...
def iterate(
    model, data_loader, criterion, panoptic_meter, config, optimizer=None, mode="train", device="gpu"
):
    loss_meter = 0
    batch_count = 0
    t_start = time.time()
    ...
    for i, batch in enumerate(data_loader):
        ...
        if mode != "train":
            with paddle.no_grad():
                ...
                predictions = model(x, batch_positions=dates, zones=targets[:, :, :, 2:3] if config.supmax else None, 
                                  heatmap_only=heatmap_only, pseudo_nms=True)  # Enable pseudo_nms for testing
        else:
        ...
        if (i + 1) % config.display_step == 0:
            ...
            print(f"{mode} - Step [{i+1}/{len(data_loader)}] Loss: {loss_meter/batch_count:.4f} "
...
def main(config):
    ...
    for fold in folds:
        ...
        val_fold = [fold]
        test_fold = [fold]  # Same as validation for now
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            train_loss, train_SQ, train_RQ, train_PQ, train_time = iterate(
                model, train_loader, criterion, train_panoptic_meter, config, optimizer, "train", config.device
            )
            ...
            if epoch % config.val_every == 0 and epoch > config.val_after:
                ...
                val_loss, val_SQ, val_RQ, val_PQ, val_time = iterate(
                    model, val_loader, criterion, val_panoptic_meter, config, mode="val", device=config.device
                )
                
                print(f"Train - Loss: {train_loss:.4f}, SQ: {train_SQ*100:.1f}, RQ: {train_RQ*100:.1f}, PQ: {train_PQ*100:.1f}")
                print(f"Val - Loss: {val_loss:.4f}, SQ: {val_SQ*100:.1f}, RQ: {val_RQ*100:.1f}, PQ: {val_PQ*100:.1f}")
