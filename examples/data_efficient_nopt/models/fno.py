...
def window_partition(x, window_size):
    ...
    if len(x.shape) == 4:
        ...
        windows = (
            ...
            .contiguous()
            .view(B, -1, C, window_size, window_size)
        ...
        windows = (
            ...
            .contiguous()
            .view(B, J, -1, C, window_size, window_size)
...
class FNN2d(nn.Layer):
    ...
    def forward_icl(self, x, demo_xs, demo_ys, use_tqdm=False):
        raise NotImplementedError("forward_icl is not implemented.")
        # """
        # x: B, C, H, W
        # demo_xs: J, C, H, W
        # demo_ys: J, H, W
...
class FNN2d_FewShot_Baseline(nn.Layer):
    ...
    def forward(self, demo_XY_query_x):
        ...
        demo_features = (
            self.backbone(demo_X.view(B, J, C, H, W).contiguous().view(B * J, C, H, W))
        ...
        x = paddle.stack(
            [
                paddle.concat(
                    [
                        x[_b],
                        demo_Y[_b].repeat([self.num_heads, 1, 1]).permute([1, 2, 0]),
                    ],
                    axis=-1,
                )
                for _b in range(B)
    ...
    def forward(self, demo_XY_query_x):
        ...
        if not self.skip_backbone:
            ...
            demo_features = self.backbone(
                demo_X.view(B, J, C, H, W).contiguous().view(B * J, C, H, W)
            ).view(B, J, self.C_fno, H, W)
        else:
        ...
        query_windows = query_windows.permute(0, 1, 3, 4, 2).view(
            B * n_win**2, self.win_s**2, self.C_fno
        )  # B*n_win*n_win, win_s*win_s, C
        demo_windows = (
            demo_windows.permute(0, 2, 1, 4, 5, 3)
            .contiguous()
            .view(B * n_win**2, J * self.win_s**2, self.C_fno)
        ...
        y = self.fc1(
            query_features.permute(0, 2, 3, 1)
...
class FNN2d_FewShot_Spatial_v2(nn.Layer):
    ...
    def forward(self, demo_XY_query_x):
        ...
        y_demo = self.fc1(
            demo_features.permute(0, 1, 3, 4, 2)
