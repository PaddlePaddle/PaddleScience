...
def draw_heatmap(
    pred, gt, save_name, hours, min=0, max=25, linear_scale=True, colorbar_label=None
):
    if not linear_scale:
    ...
    ax.set_yticks([0, 120, 240, 360, 480, 600, 719], [90, 60, 30, 0, -30, -60, -90])
    ax.set_xticks(
        [i for i in range(0, 1440, 120)] + [1439], [i for i in range(360, -1, -30)]
    )
    if linear_scale:
        map = ax.imshow(
            pred, interpolation="nearest", cmap=cmap, aspect="auto", vmin=min, vmax=max
        )
    else:
        map = ax.imshow(
            pred, interpolation="nearest", cmap=cmap, aspect="auto", norm=norm
        )
    cb = plt.colorbar(mappable=map, cax=None, ax=None, shrink=0.5, label=colorbar_label)

    bx = figure.add_subplot(2, 1, 2)
    bx.title.set_text("ERA5: lead time = {} hours".format(hours))
    bx.set_yticks([0, 120, 240, 360, 480, 600, 719], [90, 60, 30, 0, -30, -60, -90])
    bx.set_xticks(
        [i for i in range(0, 1440, 120)] + [1439], [i for i in range(360, -1, -30)]
    )
    if linear_scale:
        map = bx.imshow(
            gt, interpolation="nearest", cmap=cmap, aspect="auto", vmin=min, vmax=max
        )
    else:
        map = bx.imshow(
            gt, interpolation="nearest", cmap=cmap, aspect="auto", norm=norm
        )
    cb = plt.colorbar(mappable=map, cax=None, ax=bx, shrink=0.5, label=colorbar_label)
...
def visu_wind(file_path, save_path, means, stds):
    ...
    for iter in range(prediction_length):
        ...
        frames.append(imageio.imread(os.path.join(save_path, "{}.png".format(iter))))
    imageio.mimsave(
