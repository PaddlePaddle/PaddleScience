...
if __name__=='__main__':
    # Geometry
    # just define the geo but do not have tensor
    geo = psci.geometry.Rectangular(
        space_origin=(-0.05, -0.05, -0.05), space_extent=(0.05, 0.05, 0.05))
    print("lxd_debug: define the geo")

    
    # PDE 3D N-S equation
    pdes = psci.pde.NavierStokes(nu=0.01, rho=1.0)
    print("lxd_debug: load the pde")
    
    
    # Discretization
    pdes, geo = psci.discretize(pdes, geo, space_nsteps=(21, 21, 21))
    print("lxd_debug: discretize the geo")
    print(geo.get_space_domain())
    print("lxd_debug: the boundry index is:")
    print(geo.get_bc_index().size)

    
    # bc value
    bc_value = GenBC(geo.get_space_domain(), geo.get_bc_index())
    pdes.set_bc_value(bc_value=bc_value, bc_check_dim=[0, 1, 2, 3])
    print("lxd_debug: the bc_value is set")
    
    # Network
    # for stable flow, it have 3 input: x y z
    # 4 output: u v w p
    net = psci.network.FCNet(
        ...
        dtype="float32",
        activation='tanh')
    print("lxd_debug: the net is set")
