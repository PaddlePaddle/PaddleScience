...
class KANLinear(paddle.nn.Layer):
    ...
    def update_grid(self, x: paddle.Tensor, margin=0.01):
        ...
        grid = paddle.concat(x=[
            grid[:1] - uniform_step * paddle.arange(start=self.spline_order, end=0, step=-1, dtype='float32').unsqueeze(axis=1), 
            grid, 
            grid[-1:] + uniform_step * paddle.arange(start=1, end=self.spline_order + 1, dtype='float32').unsqueeze(axis=1)], axis=0)
...
class KAN(paddle.nn.Layer):
    ...
    def forward(self, x_dict, update_grid=False):
        assert all(key in x_dict for key in self.input_keys)
        x = paddle.concat(x=[x_dict[key] for key in self.input_keys], axis=-1)
        ...
        out_dic = {key: x[:, i:i+1] for i, key in enumerate(self.output_keys)}
        return out_dic
