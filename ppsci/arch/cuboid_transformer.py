...
class RMSNorm(paddle.nn.Layer):
    ...
    def forward(self, x):
        if self.p < 0.0 or self.p > 1.0:
            ...
            partial_x, _ = paddle.split(x=x, num_or_sections=[
                partial_size, self.d - partial_size], axis=-1)
...
def cuboid_reorder(data, cuboid_size, strategy):
    ...
    num_cuboids = T // cuboid_size[0] * H // cuboid_size[1] * W // cuboid_size[
        2]
    ...
    reordered_data = data.transpose(perm=(0,) + tuple(nblock_axis) + tuple(
        block_axis) + (7,))
...
class CuboidSelfAttentionLayer(paddle.nn.Layer):
    ...
    def __init__(self, dim, num_heads, cuboid_size=(2, 7, 7), shift_size=(0,
        ...
        if global_dim_ratio != 1:
            assert separate_global_qkv == True, f'Setting global_dim_ratio != 1 requires separate_global_qkv == True.'
    ...
    def __init__(self, dim, num_heads, block_cuboid_size=[(4, 4, 4), (4, 4,
        ...
        if self.use_inter_ffn:
            self.ffn_l = paddle.nn.LayerList(sublayers=[PositionwiseFFN(
                ...
                dropout=ffn_drop, gated_proj=gated_ffn, activation=
                activation, normalization=norm_layer, pre_norm=True,
            ...
            if self.use_global_vector:
                for idx, (attn, ffn) in enumerate(zip(self.attn_l, self.ffn_l)
                    ...
                    if self.checkpoint_level >= 1 and self.training:
                        ...
                        x = ffn(x)
                        if self.use_global_vector_ffn:
                            global_vectors = self.global_ffn_l[idx](
                                global_vectors)
...
class CuboidCrossAttentionLayer(paddle.nn.Layer):
    ...
    def forward(self, x, mem, mem_global_vectors=None):
        ...
        assert B_mem == B and H == H_mem and W == W_mem and C_in == C_mem, f'Shape of memory and the input tensor does not match. x.shape={x.shape}, mem.shape={mem.shape}'
        pad_t_mem = (n_temporal - T_mem % n_temporal) % n_temporal
        pad_t_x = (n_temporal - T_x % n_temporal) % n_temporal
        pad_h = (cuboid_hw[0] - H % cuboid_hw[0]) % cuboid_hw[0]
        pad_w = (cuboid_hw[1] - W % cuboid_hw[1]) % cuboid_hw[1]
    ...
    def __init__(self, input_shape, base_units=128, block_units=None,
        ...
        if block_attn_patterns is not None:
            ...
            if isinstance(block_attn_patterns, (tuple, list)):
                ...
                block_attn_patterns = [block_attn_patterns for _ in range(
                    self.num_blocks)]
            block_cuboid_size = []
            block_strategy = []
            block_shift_size = []
        ...
        if self.use_global_vector:
            ...
            for i in range(self.num_blocks):
                ...
                if self.num_blocks > 1 and i < self.num_blocks - 1:
                    ...
                    global_vectors = self.down_layer_global_proj[i](
                        global_vectors)
    ...
    def get_init_params(enc_input_shape, enc_out_shape_list, large_channel=
        ...
        if large_channel:
            ...
            for i, enc_out_shape in enumerate(enc_out_shape_list[::-1]):
                ...
                dec_target_shape_list_large_channel.append(tuple(
                    dec_target_shape_large_channel))
...
class CuboidTransformerModel(base.Arch):
    ...
    def __init__(self, 
        ...
        new_input_shape = self.get_initial_encoder_final_decoder(
            ...
            initial_downsample_activation, initial_downsample_conv_layers=
            initial_downsample_conv_layers, final_upsample_conv_layers=
            final_upsample_conv_layers, padding_type=padding_type,
    ...
    def reset_parameters(self):
        if self.num_global_vectors > 0:
            self.init_global_vectors = initializer.trunc_normal_(self.init_global_vectors, std=0.02)
