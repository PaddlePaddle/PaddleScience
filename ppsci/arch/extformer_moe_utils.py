...
class GatingNet(paddle.nn.Layer):
    ...
    def __init__(self, moe_config, input_shape, in_channels):
        ...
        self.noise_lin = paddle.nn.Linear(in_features=in_channels, out_features=self.num_experts, bias_attr=False)
        self.noise_eps = 1e-2
    ...
    def load_loss_cell(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        top_values_flat = noisy_top_values.transpose([1, 2, 3, 0, 4]).reshape([T, H, W, B * M])

        threshold_positions_if_in = paddle.arange(B) * M + self.out_planes
        threshold_if_in = paddle.take_along_axis(top_values_flat, axis=-1, indices=threshold_positions_if_in.unsqueeze(axis=[0, 1, 2])).unsqueeze(-1) # T, H, W, B, 1
        is_in = noisy_values > threshold_if_in # T, H, W, B, E
        threshold_positions_if_out = threshold_positions_if_in - 1
        threshold_if_out = paddle.take_along_axis(top_values_flat, axis=-1, indices=threshold_positions_if_out.unsqueeze(axis=[0, 1, 2])).unsqueeze(-1)
        
        prob_if_in = self.intra_cdf((clean_values - threshold_if_in) / noise_stddev) # T, H, W, B, E
        prob_if_out = self.intra_cdf((clean_values - threshold_if_out) / noise_stddev) # T, H, W, B, E
    ...
    def load_loss_all(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        threshold_if_in = paddle.take_along_axis(top_values_flat, axis=-1, indices=threshold_positions_if_in).unsqueeze(-1) # B, 1
        is_in = noisy_values > threshold_if_in # B, E
        threshold_positions_if_out = threshold_positions_if_in - 1 # B
        threshold_if_out = paddle.take_along_axis(top_values_flat, axis=-1, indices=threshold_positions_if_out).unsqueeze(-1) # B, 1
        
        prob_if_in = self.intra_cdf((clean_values - threshold_if_in) / noise_stddev) # B, E
        prob_if_out = self.intra_cdf((clean_values - threshold_if_out) / noise_stddev) # B, E
    ...
    def forward(self, x, t_map=None, eps=1e-25, dense_routing=False):
        ...
        top_logits, top_indices = logits.topk(min(self.out_planes + 1, self.num_experts), axis=-1)
        top_k_logits = top_logits[:, :, :, :, :self.out_planes]
        top_k_indices = top_indices[:, :, :, :, :self.out_planes]
        top_k_gates = top_k_logits / (top_k_logits.sum(axis=-1, keepdim=True) + eps) # normalization 
        ...
        if self.training: 
            if self.aux_loss_style == 'cell':
                ...
                load_loss = self.load_loss_cell(raw_logits, noisy_logits, noise, top_logits)
            elif self.aux_loss_style == 'all':
                importance_loss = self.importance_loss_all(logits.reshape([B * T * H * W, E]))
                load_loss = self.load_loss_all(raw_logits.reshape([B * T * H * W, E]), 
                                               noisy_logits.reshape([B * T * H * W, E]), 
                                               noise.reshape([B * T * H * W, E]), 
                                               top_logits.reshape([B * T * H * W, -1]))
            else:
                raise NotImplementedError
            loss = self.importance_weight * importance_loss + self.load_weight * load_loss
        else: 
            loss = None
...
class LinearGatingNet(GatingNet):
    ...
    def __init__(self, moe_config, input_shape, in_channels):
        ...
        self.lin = paddle.nn.Linear(in_features=in_channels, out_features=self.num_experts, bias_attr=False)
...
class SpatialLatentGatingNet(GatingNet):
    ...
    def __init__(self, moe_config, input_shape, in_channels):
        ...
        self.routing_weights = paddle.create_parameter(shape=[H, W, self.num_experts], dtype='float32', 
            default_initializer=paddle.nn.initializer.Uniform(-bound, bound))

    def gating(self, x, t_map=None):
        # assert t_map is not None
        routing_weights = self.routing_weights.unsqueeze(0).tile([x.shape[0], x.shape[1], 1, 1, 1]) # [B, T, H, W, E]
        return routing_weights  
...
class SpatialLatentLinearGatingNet(GatingNet):
    ...
    def __init__(self, moe_config, input_shape, in_channels):
        ...
        fan = self.out_planes / self.num_experts
        bound = gain * math.sqrt(3.0 / fan) 
        self.spatial_routing_weights = paddle.create_parameter(shape=[H, W, self.num_experts], dtype='float32', 
            default_initializer=paddle.nn.initializer.Uniform(-bound, bound))
        self.lin = paddle.nn.Linear(in_features=in_channels, out_features=self.num_experts, bias_attr=False)
        
        self.combine_weight = paddle.create_parameter(shape=[H, W, self.num_experts, 2], dtype='float32', 
            default_initializer=paddle.nn.initializer.Uniform(-bound, bound))

    def gating(self, x, t_map=None):
        # assert t_map is not None
        spatial_routing_weights = self.spatial_routing_weights.tile([x.shape[0], x.shape[1], 1, 1, 1]) # [B, T, H, W, E]
        linear_routing_weights = self.lin(x) # [B, T, H, W, E]
        routing_weights = paddle.stack([spatial_routing_weights, linear_routing_weights], axis=-1) # [B, T, H, W, E, 2]
        combine_weight = self.combine_weight.tile([x.shape[0], x.shape[1], 1, 1, 1, 1]) # [B, T, H, W, E, 2]
...
class CuboidLatentGatingNet(GatingNet):
    ...
    def __init__(self, moe_config, input_shape, in_channels):
        ...
        fan = self.out_planes / self.num_experts
        bound = gain * math.sqrt(3.0 / fan) 
        self.routing_weights = paddle.create_parameter(shape=[T, H, W, self.num_experts], dtype='float32', 
            default_initializer=paddle.nn.initializer.Uniform(-bound, bound))

    def gating(self, x, t_map=None):
        # assert t_map is not None
        routing_weights = self.routing_weights.unsqueeze(0).tile([x.shape[0], 1, 1, 1, 1]) # [B, T, H, W, E]
        return routing_weights
...
class CuboidLatentLinearGatingNet(GatingNet):
    ...
    def __init__(self, moe_config, input_shape, in_channels):
        ...
        fan = self.out_planes / self.num_experts
        bound = gain * math.sqrt(3.0 / fan) 
        self.cuboid_routing_weights = paddle.create_parameter(shape=[T, H, W, self.num_experts], dtype='float32', 
            default_initializer=paddle.nn.initializer.Uniform(-bound, bound))
        
        self.lin = paddle.nn.Linear(in_features=in_channels, out_features=self.num_experts, bias_attr=False)
        
        self.combine_weight = paddle.create_parameter(shape=[T, H, W, self.num_experts, 2], dtype='float32', 
            default_initializer=paddle.nn.initializer.Uniform(-bound, bound))

    def gating(self, x, t_map=None):
        # assert t_map is not None
        cuboid_routing_weights = self.cuboid_routing_weights.unsqueeze(0).tile([x.shape[0], 1, 1, 1, 1]) # [B, T, H, W, E]
        linear_routing_weights = self.lin(x) # [B, T, H, W, E]
        routing_weights = paddle.stack([cuboid_routing_weights, linear_routing_weights], axis=-1) # [B, T, H, W, E, 2]
        combine_weight = self.combine_weight.tile([x.shape[0], 1, 1, 1, 1, 1]) # [B, T, H, W, E, 2]
        routing_weights = (routing_weights * combine_weight).sum(-1) # [B, T, H, W, E]
        return routing_weights  
...
class SparseDispatcherScatter(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        sorted_experts, index_sorted_experts = paddle.nonzero(gates).sort(0), paddle.nonzero(gates).argsort(0)
        ...
        self._nonzero_gates = paddle.take_along_axis(gates_exp, axis=1, indices=self._expert_index)
...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, top_k_gates, top_k_indices):
        ...
        self.batch_index_per_expert = paddle.stack([(top_k_indices == expert_id).sum(-1).astype('bool') for expert_id in range(E)], axis=0) # [E, B]
        self.gates_per_expert = paddle.concat([top_k_gates[top_k_indices == expert_id] for expert_id in range(E)]) # B * K
        self.batch_index_all = paddle.nonzero(self.batch_index_per_expert)[:, 1] # B * K
        self.expert_size = self.batch_index_per_expert.sum(-1) # [E]

    def dispatch(self, x):
        B, C = x.shape
        dispatched_x = [x[batch_index] for batch_index in self.batch_index_per_expert] # E * [B_e, C]
        return dispatched_x
    ...
    def combine(self, expert_out):
        ...
        zeros.stop_gradient = False
        combined_res = zeros.index_add(axis=0, index=self.batch_index_all, value=com_res * self.gates_per_expert.unsqueeze(-1))
        return combined_res
...
class DenseDispatcher(object):
    ...
    def combine(self, expert_out):
        ...
        selected_out = paddle.take_along_axis(expert_out, axis=1, indices=self.gate_inds.unsqueeze(-1)) # [B, K, C]
...
class LabelDifference(paddle.nn.Layer):
    ...
    def forward(self, labels):
        ...
        if self.distance_type == 'l1':
            return paddle.abs(labels[:, :, None, :] - labels[:, None, :, :]).sum(axis=-1)
        else:
...
class FeatureSimilarity(paddle.nn.Layer):
    ...
    def forward(self, features):
        ...
        if self.similarity_type == 'l2':
            logits = - (features[:, :, None, :] - features[:, None, :, :]).norm(2, axis=-1)
            logits /= self.t
...
class RnCLoss(paddle.nn.Layer):
    ...
    def __init__(self, rnc_config):
        ...
        self.feature_sim_fn = FeatureSimilarity(rnc_config["feature_similarity_style"], self.t)
    ...
    def cal_loss(self, features, labels):
        ...
        # remove diagonal
        logits = logits.masked_select((1 - paddle.eye(n)).astype('bool').unsqueeze(0).tile([B, 1, 1])).reshape([B, n, n - 1])
        exp_logits = exp_logits.masked_select((1 - paddle.eye(n)).astype('bool').unsqueeze(0).tile([B, 1, 1])).reshape([B, n, n - 1])
        label_diffs = label_diffs.masked_select((1 - paddle.eye(n)).astype('bool').unsqueeze(0).tile([B, 1, 1])).reshape([B, n, n - 1])
        ...
        if self.loss_cal_mode == "memory-efficient":
            ...
            for k in range(n - 1):
                ...
                neg_mask = (label_diffs >= pos_label_diffs.unsqueeze(-1)).astype('float32') # [B, n, n - 1]
                pos_log_probs = pos_logits - paddle.log((neg_mask * exp_logits).sum(axis=-1))  # [B, n]
                loss += - pos_log_probs.sum()
            loss /= B * n * (n - 1)
        elif self.loss_cal_mode == "computation-efficient":
            neg_mask = (label_diffs.unsqueeze(-2) >= label_diffs.unsqueeze(-1)).astype('float32')  # [B, n, n - 1, n - 1]
            pos_log_probs = logits - paddle.log((neg_mask * exp_logits.unsqueeze(-2).tile([1, 1, n - 1, 1])).sum(axis=-1)) # [B, n, n - 1]
