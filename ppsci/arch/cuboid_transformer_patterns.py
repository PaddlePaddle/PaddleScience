...
for p in [1, 2, 4, 8, 10]:
    for m in [1, 2, 4, 8, 16, 32]:
        CuboidSelfAttentionPatterns.register(f'video_swin_{p}x{m}',
            functools.partial(self_video_swin, P=p, M=m))
...
for m in [1, 2, 4, 8, 16, 32]:
    CuboidSelfAttentionPatterns.register(f'spatial_lg_{m}', functools.
        partial(self_spatial_lg_v1, M=m))


def self_axial_space_dilate_K(input_shape, K=2):
    T, H, W, _ = input_shape
    K = min(K, H, W)
    cuboid_size = [(T, 1, 1), (1, H // K, 1), (1, H // K, 1), (1, 1, W // K
        ), (1, 1, W // K)]
    strategy = [('l', 'l', 'l'), ('d', 'd', 'd'), ('l', 'l', 'l'), ('d',
        'd', 'd'), ('l', 'l', 'l')]
...
for k in [2, 4, 8]:
    CuboidSelfAttentionPatterns.register(f'axial_space_dilate_{k}',
        functools.partial(self_axial_space_dilate_K, K=k))
...
for k in [1, 2, 4, 8]:
    CuboidCrossAttentionPatterns.register(f'cross_{k}x{k}', functools.
        partial(cross_KxK, K=k))
    CuboidCrossAttentionPatterns.register(f'cross_{k}x{k}_lg', functools.
        partial(cross_KxK_lg, K=k))
    CuboidCrossAttentionPatterns.register(f'cross_{k}x{k}_heter', functools
        .partial(cross_KxK_heter, K=k))
