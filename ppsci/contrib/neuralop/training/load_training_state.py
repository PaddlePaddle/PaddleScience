...
def load_training_state(save_dir: Union[str, Path], save_name: str,
                        ...
                        regularizer: nn.Layer = None) -> dict:
    ...
    if optimizer is not None:
        ...
        if optimizer_pth.exists():
            training_state['optimizer'] = optimizer.load_state_dict(paddle.load(optimizer_pth))
        else:
    ...
    if scheduler is not None:
        ...
        if scheduler_pth.exists():
            training_state['scheduler'] = scheduler.load_state_dict(paddle.load(scheduler_pth))
        else:
    ...
    if regularizer is not None:
        ...
        if regularizer_pth.exists():
            training_state['regularizer'] = scheduler.load_state_dict(paddle.load(regularizer_pth))
        else:
