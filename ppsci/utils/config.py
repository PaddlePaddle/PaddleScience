...
__all__ = ["get_config", "replace_shape_with_inputspec_", "AttrDict", "SolverConfig"]
...
class TrainConfig(BaseModel):
    ...
    def eval_during_train_check(cls, v):
        if not isinstance(v, bool):
            raise ValueError(
                "'eval_during_train' should be a bool" f", but got {misc.typename(v)}"
            )
        return v
...
class SolverConfig(BaseModel):
    ...
    def mode_check(cls, v):
        if v not in ["train", "eval"]:
            raise ValueError(f"'mode' should be one of ['train', 'eval'], but got {v}")
        return v
    ...
    def output_dir_check(cls, v):
        if v is not None and not isinstance(v, str):
            raise ValueError(
                "'output_dir' should be a string or None" f"but got {misc.typename(v)}"
            )
        return v
    ...
    def use_vdl_check(cls, v):
        if not isinstance(v, bool):
            raise ValueError(f"'use_vdl' should be a bool, but got {misc.typename(v)}")
        return v
    ...
    def device_check(cls, v):
        if not isinstance(v, str):
            raise ValueError(f"'device' should be a str, but got {misc.typename(v)}")
        if v not in ["cpu", "gpu"]:
            raise ValueError(f"'device' should be one of ['cpu', 'gpu'], but got {v}")
        return v

    @field_validator("use_amp")
    def use_amp_check(cls, v):
        if not isinstance(v, bool):
            raise ValueError(f"'use_amp' should be a bool, but got {misc.typename(v)}")
        return v
