...
def stress_disp_xx_expected_result(u,v,w,x,y,z,lambda_,mu,dim,sigma_xx):
    stress_disp_xx = (
        lambda_ * (jacobian(u, x) + jacobian(v, y))
        + 2 * mu * jacobian(u, x)
        - sigma_xx
    )
    if dim == 3:
...
def equilibrium_x_expected_result(u,x,y,z,t,rho,dim,time,sigma_xx,sigma_xy,sigma_xz=None):
...
def equilibrium_y_expected_result(v,x,y,z,t,rho,dim,time,sigma_yy,sigma_xy,sigma_yz=None):
...
def traction_x_expected_result(normal_x, normal_y, sigma_xx, sigma_xy, normal_z=None, sigma_xz=None):
...
def traction_y_expected_result(normal_x, normal_y, sigma_xy, sigma_yy, normal_z=None, sigma_yz=None):
...
def navier_x_expected_result(u, v, w, x, y, z, t, lambda_, mu, rho, dim, time):
        ...
        navier_x = (
            -(lambda_ + mu) * jacobian(duxvywz, x) - mu * duxxuyyuzz
        )
        if time:
...
def navier_y_expected_result(u, v, w, x, y, z, t, lambda_, mu, rho, dim, time):
    ...
    navier_y = (
        -(lambda_ + mu) * jacobian(duxvywz, y) - mu * dvxxvyyvzz
    )
    if time:
...
def test_linear_elasticity(E, nu, lambda_, mu, rho, dim, time):
    ...
    if dim == 3:
        ...
        sigma_xx, sigma_xy, sigma_xz, sigma_yy, sigma_yz, sigma_zz = (
            output[:, 3:4], output[:, 4:5], output[:, 5:6], output[:, 6:7], output[:, 7:8], output[:, 8:9]
        )
    else:
        w = None
    ...
    expected_stress_disp_xx = stress_disp_xx_expected_result(u,v,w,x,y,z,lambda_,mu,dim,sigma_xx)
    expected_stress_disp_yy = stress_disp_yy_expected_result(u,v,w,x,y,z,lambda_,mu,dim,sigma_yy)
    expected_stress_disp_xy = stress_disp_xy_expected_result(u,v,x,y,mu,sigma_xy)
    expected_equilibrium_x=equilibrium_x_expected_result(u, x, y, z, t,  rho, dim, time,sigma_xx,sigma_xy,sigma_xz)
    expected_equilibrium_y=equilibrium_y_expected_result(v, x, y, z, t,  rho, dim, time,sigma_yy,sigma_xy,sigma_yz)
    expected_traction_x = traction_x_expected_result(normal_x, normal_y, sigma_xx, sigma_xy, normal_z, sigma_xz)
    expected_traction_y = traction_y_expected_result(normal_x, normal_y, sigma_xy, sigma_yy, normal_z, sigma_yz)
    expected_navier_x = navier_x_expected_result(u, v, w, x, y, z, t, lambda_, mu, rho, dim, time)
    expected_navier_y=navier_y_expected_result(u, v, w, x, y, z, t, lambda_, mu, rho, dim, time)
    if dim == 3:
        expected_stress_disp_zz = stress_disp_zz_expected_result(u,v,w,x,y,z,lambda_,mu,sigma_zz)
        expected_stress_disp_xz = stress_disp_xz_expected_result(u,w,x,z,mu,sigma_xz)
        expected_stress_disp_yz = stress_disp_yz_expected_result(v,w,y,z,mu,sigma_yz)
        expected_equilibrium_z=equilibrium_z_expected_result(w, x, y, z, t,  rho,  time,sigma_xz,sigma_yz,sigma_zz)
        expected_navier_z = navier_z_expected_result(u, v, w, x, y, z, t, lambda_, mu, rho,time)    
        expected_traction_z = traction_z_expected_result(normal_x, normal_y, normal_z, sigma_xz, sigma_yz, sigma_zz)


    linear_elasticity = LinearElasticity(E=E, nu=nu, lambda_=lambda_, mu=mu, rho=rho, dim=dim, time=time)
    data_dict = {
    "x": x,
    "y": y,
    "u": u,
