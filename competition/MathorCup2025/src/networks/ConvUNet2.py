        ...
        if apply_pooling:
            if pool_type == "max":
                if is3d:
                    ...
                    self.pooling = paddle.nn.MaxPool2D(kernel_size=pool_kernel_size)
            elif is3d:
                self.pooling = paddle.nn.AvgPool3D(
                    kernel_size=pool_kernel_size, exclusive=False
                )
            else:
                self.pooling = paddle.nn.AvgPool2D(
                    kernel_size=pool_kernel_size, exclusive=False
                )
        else:
            self.pooling = None
...
class InterpolateUpsampling(AbstractUpsampling):
    ...
    def _interpolate(x, size, mode):
        return paddle.nn.functional.interpolate(
            x=x, size=size, mode=mode, data_format="NCDHW"
        )
...
class TransposeConvUpsampling(AbstractUpsampling):
    ...
    def __init__(
        ...
        upsample = paddle.nn.Conv3DTranspose(
            in_channels=in_channels,
            out_channels=out_channels,
            kernel_size=kernel_size,
            stride=scale_factor,
...
class AbstractUNet(BaseModel):
    ...
    def __init__(
        ...
        if is3d:
            self.final_conv = paddle.nn.Conv3D(
                in_channels=f_maps[0], out_channels=out_channels, kernel_size=1
            )
        else:
            self.final_conv = paddle.nn.Conv2D(
                in_channels=f_maps[0], out_channels=out_channels, kernel_size=1
            )
        if is_segmentation:
            if final_sigmoid:
...
class UNet3DWithSamplePoints(UNet3D):
    def __init__(
        ...
        super(UNet3DWithSamplePoints, self).__init__(
            in_channels, hidden_channels, f_maps=hidden_channels, num_levels=num_levels
        )
    ...
    def data_dict_to_input(self, data_dict):
        ...
        if self.use_position_input:
            ...
            input_grid_features = paddle.concat(
                x=(input_grid_features, grid_points), axis=1
            )
