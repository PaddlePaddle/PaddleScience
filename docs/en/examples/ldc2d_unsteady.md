...
According to the dimensionless formulas and boundary conditions obtained in [2. Problem Definition](#2), they correspond to two constraints guiding model training in the computational domain, namely:
...
The first parameter of `InteriorConstraint` is the equation expression, used to describe how to calculate the constraint target. Here fill in `equation["NavierStokes"].equations` instantiated in section [3.2 Equation Construction](#32);
...
The third parameter is the computational domain where the constraint equation acts. Here fill in `geom["time_rect"]` instantiated in section [3.3 Computational Domain Construction](#33);
...
The equation setting is the same as the setting in [Constraint Construction](#34), indicating how to calculate the target variables to be evaluated;

Here we set label values of 0 for the three target variables `momentum_x`, `continuity`, `momentum_y`;

The computational domain is the same as the setting in [Constraint Construction](#34), indicating evaluation on the specified computational domain;
...
Other configurations are similar to the settings in [Constraint Construction](#34).
