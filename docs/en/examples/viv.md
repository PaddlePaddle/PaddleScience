...
The first parameter of `SupervisedConstraint` is the reading configuration of supervised constraint, here fill in `train_dataloader_cfg` instantiated in [3.2 Equation Construction](#32-equation-construction) section;
...
The third parameter is the equation expression, used to describe how to calculate the constraint target. Here fill in the calculation function of `eta` and `equation["VIV"].equations` instantiated in [3.2 Equation Construction](#32-equation-construction) section;
...
Other configurations are similar to the settings in [3.4.1 Supervised Constraint](#341-supervised-constraint).
