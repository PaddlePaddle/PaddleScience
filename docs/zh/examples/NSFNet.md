...
## 6. 结果说明
我们使用PINN对不可压纳韦斯托克方程进行数值求解，时间和空间的坐标被当作输入值，速度场以及压强场被当作输出值，使用初值、边界条件当作监督约束以及纳韦斯托克方程本身的当作无监督约束条件放入损失函数进行训练。我们训练了三个不同类型的纳韦斯托克方程，设计了三个不用的神经网络，即NSFNet1、NSFNet2、NSFNet3。通过损失函数的下降可以证明神经网络在求解纳韦斯托克方程中的收敛性，表明PINN拥有对不可压纳韦斯托克方程的求解能力。而通过实验结果表明，三个NSFNet方程都可以很好的逼近对应的纳韦斯托克方程，并且，我们发现增加边界约束以及处置约束的权重可以使得神经网络拥有更好的逼近效果。
## 7. 参考资料
