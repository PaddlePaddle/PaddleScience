...
我们使用ABP和Levy RTP模型进行了三种最大流速（5、10和15微米/秒）下的流体和粒子动力学模拟，以生成Geo-FNO的训练和测试数据。对于训练数据，我们在50个GPU上并行生成了1000个模拟，耗时10小时，每个模拟中的设计随机选自以下参数空间：

- 高度为$20微米 < h < 30微米$的障碍物周期性地放置在通道壁上
- 障碍物间距离为$60微米 < d < 250微米$
- 基底长度满足$15微米 < L < \frac{d}{4}$，并且尖端位置满足$-\frac{d}{4} < s < \frac{d}{4}$。
