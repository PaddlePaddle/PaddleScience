...
class LogMemoryMixin:
    r"""A context manager to enable logging of memory consumption in DataLoader workers."""
...
class MultithreadingMixin:
    ...
    def _mt_init_fn(self, worker_id: int) -> None:
        try:
            paddle.set_num_threads(int(self._worker_threads))
        except IndexError:
    ...
    def enable_multithreading(
        ...
        if worker_threads is None:
            worker_threads = paddle.get_num_threads() // self.num_workers
...
class AffinityMixin:
    ...
    def _aff_init_fn(self, worker_id: int) -> None:
        try:
            ...
            if paddle.multiprocessing.get_start_method() == 'spawn':
                paddle.set_num_threads(len(worker_cores))
