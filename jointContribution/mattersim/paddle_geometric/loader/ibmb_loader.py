...
from paddle_geometric.utils import get_ppr, is_undirected, subgraph
...
class _IBMBBaseLoader(paddle.io.DataLoader):
...
def define_sampler(
    ...
    if batch_order == 'rand':
        ...
        sampler = paddle.io.RandomSampler(ys)
...
def create_batchwise_out_aux_pairs(
    adj: SparseTensor,
    partitions: List[Union[paddle.int64, np.ndarray]],
    prime_indices: Union[paddle.int64, np.ndarray],
    topk: int,
    ...
    def ppr_power_method(
        adj: SparseTensor,
        batch: List[Union[np.ndarray, paddle.int64]],
        topk: int,
        num_iter: int,
        alpha: float,
    ) -> List[np.ndarray]:
        ...
        nonzeros = paddle.minimum(
            nonzeros,
            paddle.to_tensor([topk], dtype=paddle.int64, device=adj.device()))
...
def topk_ppr_matrix(
    ...
    alpha: float,
    eps: float,
    output_node_indices: Union[np.ndarray, paddle.int64],
    topk: int,
    normalization='row',
    ...
    neighbors = [
        n.cpu().numpy()
        for n in paddle.split(neighbors[1],
                             neighbor_counts.cpu().tolist(), dim=0)
    ]
    weights = [
        n.cpu().numpy()
        for n in paddle.split(weights,
                             neighbor_counts.cpu().tolist(), dim=0)
    ]
    ...
    _, out_degree = paddle.unique(edge_index[0], sorted=True,
                                 return_counts=True)
...
class IBMBBaseLoader(paddle.io.DataLoader):
    def __init__(
        self,
    ...
    def prepare_cache(
        ...
        for out, aux in pbar:
            mask = paddle.to_tensor(np.in1d(aux, out))
            if isinstance(aux, np.ndarray):
                aux = paddle.to_tensor(aux)
...
class IBMBBatchLoader(IBMBBaseLoader):
    ...
    Args:
        ...
        **kwargs (optional): Additional arguments of
            :class:`paddle.io.DataLoader`, such as :obj:`batch_size`,
...
class IBMBNodeLoader(IBMBBaseLoader):
    ...
    def create_loader(self, graph: Data, adj: SparseTensor):
        ...
        ppr_matrix, neighbors = topk_ppr_matrix(
            ...
            paddle.to_tensor(self.output_indices),
...
class IBMBOrderedSampler(paddle.io.Sampler[int]):
    ...
    """
    def __init__(self, data_source: Union[np.ndarray, paddle.Tensor,
                                          List]) -> None:
...
class IBMBWeightedSampler(paddle.io.Sampler[int]):
    ...
    """
    def __init__(self, batch_kl_div: Union[np.ndarray, paddle.Tensor]) -> None:
