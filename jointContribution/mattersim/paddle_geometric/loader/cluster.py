...
from paddle_geometric.utils import index_sort, narrow, select, sort_edge_index
...
class ClusterData(paddle.io.Dataset):
    ...
    def __init__(
        ...
        if save_dir is not None and osp.exists(path):
            ...
            if save_dir is not None:
                ...
                paddle.save(self.partition, path)
    ...
    def _metis(self, edge_index: Tensor, num_nodes: int) -> Tensor:
        ...
        if paddle_geometric.typing.WITH_TORCH_SPARSE:
            try:
                cluster = paddle.ops.torch_sparse.partition(
                    indptr.cpu(),
                    index.cpu(),
                    None,
                    self.num_parts,
        ...
        if cluster is None and paddle_geometric.typing.WITH_METIS:
    ...
    def _partition(self, edge_index: Tensor, cluster: Tensor) -> Partition:
        ...
        edge_perm = paddle.arange(edge_index.size(1), device=edge_index.device)
        arange = paddle.empty_like(node_perm)
        arange[node_perm] = paddle.arange(cluster.numel(),
    ...
    def __getitem__(self, idx: int) -> Data:
        ...
        out.edge_index = paddle.stack([row, col], dim=0)
...
class ClusterLoader(paddle.io.DataLoader):
    ...
    .. note::
        ...
        Use :class:`~paddle_geometric.loader.ClusterData` and
        :class:`~paddle_geometric.loader.ClusterLoader` in conjunction to
    ...
    def _collate(self, batch: List[int]) -> Data:
        ...
        for i in range(batch.numel()):
            nodes.append(paddle.arange(node_start[i], node_end[i]))
        ...
        node = paddle.concat(nodes, dim=0)
        row = paddle.concat(rows, dim=0)
        col = paddle.concat(cols, dim=0)
