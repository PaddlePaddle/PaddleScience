...
from paddle_geometric.graphgym.loss import compute_loss
from paddle_geometric.graphgym.models.gnn import GNN
from paddle_geometric.graphgym.optim import create_optimizer, create_scheduler
from paddle_geometric.graphgym.register import network_dict, register_network

register_network('gnn', GNN)


class GraphGymModule(paddle.nn.Layer):
    ...
    @property
    def encoder(self) -> paddle.nn.Layer:
        return self.model.encoder

    @property
    def mp(self) -> paddle.nn.Layer:
        return self.model.mp

    @property
    def post_mp(self) -> paddle.nn.Layer:
        return self.model.post_mp

    @property
    def pre_mp(self) -> paddle.nn.Layer:
        return self.model.pre_mp

    def lr_scheduler_step(self, *args, **kwargs):
        # Adjust the learning rate scheduler step method for Paddle
...
def create_model(to_device=True, dim_in=None, dim_out=None) -> GraphGymModule:
    ...
    if to_device:
        model.to(paddle.device(cfg.accelerator))
    return model
