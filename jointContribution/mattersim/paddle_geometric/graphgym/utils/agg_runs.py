...
from paddle_geometric.graphgym.utils.io import (
    dict_list_to_json,
    dict_list_to_tb,
    dict_to_json,
    json_to_dict_list,
...
try:
    from visualdl import LogWriter  # PaddlePaddle's equivalent to TensorboardX
except ImportError:
    LogWriter = None
...
def join_list(l1, l2):
    assert len(l1) == len(l2), \
        'Results with different seeds must have the same format'
...
def agg_runs(dir, metric_best='auto'):
    ...
    for key, value in results.items():
        ...
        if cfg.tensorboard_agg:
            if LogWriter is None:
                raise ImportError('Tensorboard support requires `visualdl` package.')
            writer = LogWriter(dir_out)
    ...
    logging.info('Results aggregated across runs saved in {}'.format(osp.join(dir, 'agg')))
...
def agg_batch(dir, metric_best='auto'):
    ...
    for key in results:
        if len(results[key]) > 0:
            ...
            results[key] = results[key].sort_values(list(dict_name.keys()), ascending=[True] * len(dict_name))
            fname = osp.join(dir_out, f'{key}_best.csv')
            results[key].to_csv(fname, index=False)

    # Repeat for final epoch results and best epoch results
