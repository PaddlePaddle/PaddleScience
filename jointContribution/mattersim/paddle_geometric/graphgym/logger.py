...
from paddle_geometric.graphgym.utils.device import get_current_gpu_usage
from paddle_geometric.graphgym.utils.io import dict_to_json, dict_to_tb
...
class Logger:
    def __init__(self, name='train', task_type=None):
        ...
        if cfg.tensorboard_each_run:
            ...
            self.tb_writer = LogWriter(logdir=self.out_dir)
    ...
    def _get_pred_int(self, pred_score):
        if len(pred_score.shape) == 1 or pred_score.shape[1] == 1:
            return (pred_score > cfg.model.thresh).astype('int64')
        else:
    ...
    def classification_binary(self):
        ...
        true, pred_score = paddle.concat(self._true), paddle.concat(self._pred)
        pred_int = self._get_pred_int(pred_score)
        try:
            r_a_score = roc_auc_score(true.numpy(), pred_score.numpy())
        except ValueError:
            r_a_score = 0.0
        return {
            'accuracy': round(accuracy_score(true.numpy(), pred_int.numpy()), cfg.round),
            'precision': round(precision_score(true.numpy(), pred_int.numpy()), cfg.round),
            'recall': round(recall_score(true.numpy(), pred_int.numpy()), cfg.round),
            'f1': round(f1_score(true.numpy(), pred_int.numpy()), cfg.round),
    ...
    def classification_multi(self):
        ...
        true, pred_score = paddle.concat(self._true), paddle.concat(self._pred)
        pred_int = self._get_pred_int(pred_score)
        return {'accuracy': round(accuracy_score(true.numpy(), pred_int.numpy()), cfg.round)}

    def regression(self):
        from sklearn.metrics import mean_absolute_error, mean_squared_error

        true, pred = paddle.concat(self._true), paddle.concat(self._pred)
        return {
            'mae': float(round(mean_absolute_error(true.numpy(), pred.numpy()), cfg.round)),
            'mse': float(round(mean_squared_error(true.numpy(), pred.numpy()), cfg.round)),
            'rmse': float(round(math.sqrt(mean_squared_error(true.numpy(), pred.numpy())), cfg.round))
        }
    ...
    def write_epoch(self, cur_epoch):
        ...
        for custom_metric in cfg.custom_metrics:
            ...
            if not func:
                raise ValueError(f'Unknown custom metric function name: {custom_metric}')
...
class LoggerCallback:
    ...
    def _get_stats(
        ...
        return dict(
            true=outputs['true'].cpu(),
            pred=outputs['pred_score'].cpu(),
            loss=float(outputs['loss']),
            lr=trainer.lr_scheduler_configs[0].scheduler.get_lr()[0],
