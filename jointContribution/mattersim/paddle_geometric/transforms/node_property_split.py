...
from paddle_geometric.utils import to_networkx
...
class NodePropertySplit(BaseTransform):
    ...
    def _compute_popularity_property(G: Any, ascending: bool = True) -> Tensor:
        ...
        property_values = paddle.to_tensor(list(A.pagerank(G).values()))
    ...
    def _compute_density_property(G: Any, ascending: bool = True) -> Tensor:
        ...
        property_values = paddle.to_tensor(list(A.clustering(G).values()))
        property_values *= -1 if ascending else 1
        return property_values
    ...
    def _mask_nodes_by_property(
        ...
        num_nodes = property_values.shape[0]
        sizes = (num_nodes * paddle.to_tensor(ratios)).round().astype('int64')
        sizes[-1] -= sizes.sum() - num_nodes

        perm = paddle.randperm(num_nodes)
        id_size = int(sizes[:3].sum())
        perm = perm[property_values[perm].argsort()]
        perm[:id_size] = perm[:id_size][paddle.randperm(id_size)]
        ...
        for name, node_split in zip(names, node_splits):
            split_mask = paddle.zeros(num_nodes, dtype=paddle.bool)
