...
class EdgeTypePadding(MappingPadding):
    r"""Padding dependent on edge types.
...
class Pad(BaseTransform):
    ...
    In case of applying :class:`Pad` to a :class:`~paddle_geometric.data.Data`
    ...
    * an object of :class:`AttrNamePadding` class for cases when padding is
    ...
    :class:`~paddle_geometric.data.HeteroData` object, the :obj:`node_pad_value`
    ...
    * if :obj:`max_num_nodes` is a dictionary, value for every node type must
    ...
    :class:`~paddle_geometric.data.HeteroData` object. The object is padded to
    ...
    def __pad_node_store(
        ...
        if self.add_pad_mask:
            pad_node_mask = paddle.ones([num_target_nodes], dtype=paddle.bool)
    ...
    def __pad_edge_store(
        ...
        if self.add_pad_mask:
            pad_edge_mask = paddle.ones([num_target_edges], dtype=paddle.bool)
    ...
    @staticmethod
    def _pad_tensor_dim(input: paddle.Tensor, dim: int, length: int,
                        pad_value: float) -> paddle.Tensor:
        r"""Pads the input tensor in the specified dim with a constant value of
        the given length.
        """
        pads = [0] * (2 * len(input.shape))
        pads[-2 * dim - 1] = length
        return paddle.nn.functional.pad(input, pads, value=pad_value, mode='constant')

    @staticmethod
    def _pad_edge_index(input: paddle.Tensor, length: int, src_pad_value: float,
                        dst_pad_value: float) -> paddle.Tensor:
        r"""Pads the edges :obj:`edge_index` feature with values specified
        separately for src and dst nodes.
        """
        pads = [0, length, 0, 0]
        padded = paddle.nn.functional.pad(input, pads, mode='constant', value=src_pad_value)
