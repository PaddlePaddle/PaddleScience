...
from paddle_geometric.utils import to_scipy_sparse_matrix
...
class LargestConnectedComponents(BaseTransform):
    ...
    def forward(self, data: Data) -> Data:
        ...
        subset = paddle.to_tensor(subset_np, dtype=paddle.bool)

        # Ensure that the tensor is on the same device as `edge_index`
        # subset = subset.cast(data.edge_index.dtype)
