...
from paddle_geometric.explain.config import ExplanationType, ModelMode
...
def auc(x: paddle.Tensor, y: paddle.Tensor) -> paddle.Tensor:
    if paddle.any(paddle.diff(x) < 0):
        raise ValueError("'x' must be given in ascending order")
    return paddle.trapezoid(y, x)