...
from paddle_geometric.data.data import Data, warn_or_raise
from paddle_geometric.data.hetero_data import HeteroData
from paddle_geometric.explain.config import ThresholdConfig, ThresholdType
...
class ExplanationMixin:
    ...
    def validate_masks(self, raise_on_error: bool = True) -> bool:
        ...
        for store in self.node_stores:
            ...
            if store.node_mask.shape[0] not in {1, store.num_nodes}:
                status = False
                warn_or_raise(
                    f"Expected a 'node_mask' with {store.num_nodes} nodes "
                    f"(got {store.node_mask.shape[0]} nodes)", raise_on_error)

            if 'x' in store:
                num_features = store.x.shape[-1]
            else:
                num_features = store.node_mask.shape[-1]

            if store.node_mask.shape[1] not in {1, num_features}:
                status = False
                warn_or_raise(
                    f"Expected a 'node_mask' with {num_features} features ("
                    f"got {store.node_mask.shape[1]} features)", raise_on_error)
        ...
        for store in self.edge_stores:
            ...
            if store.edge_mask.shape[0] != store.num_edges:
                status = False
                warn_or_raise(
                    f"Expected an 'edge_mask' with {store.num_edges} edges "
                    f"(got {store.edge_mask.shape[0]} edges)", raise_on_error)
        ...
        if threshold_config.type in [
            ...
            if threshold_config.value >= mask.numel():
                if threshold_config.type == ThresholdType.topk:
                    ...
                    return paddle.ones_like(mask)

            value, index = paddle.topk(
                mask.flatten(),
                k=threshold_config.value,
            )

            out = paddle.zeros_like(mask.flatten())
            if threshold_config.type == ThresholdType.topk:
                out.scatter_(index, value)
            else:
                out.scatter_(index, 1.0)
    ...
    def visualize_feature_importance(
        ...
        if feat_labels is None:
            ...
            for node_type, node_mask in node_mask_dict.items():
                feat_labels[node_type] = range(node_mask.shape[1])

        score = paddle.concat(
            [paddle.sum(node_mask, axis=0) for node_mask in node_mask_dict.values()],
            axis=0)
