...
from paddle_geometric.sampler.base import DataType, NumNeighbors, SubgraphType
from paddle_geometric.sampler.utils import remap_keys, to_csc, to_hetero_csc
...
class NeighborSampler(BaseSampler):
    ...
    by :class:`~paddle_geometric.loader.NeighborLoader`.
    """
    def __init__(
        self,
        ...
        if self.data_type == DataType.homogeneous:
            ...
            # Conversion to/from Python string type:
        ...
        if (self.edge_time is not None
                and not paddle_geometric.typing.WITH_EDGE_TIME_NEIGHBOR_SAMPLE):
            raise ImportError("Edge-level temporal sampling requires a "
                              "more recent 'pgl-lib' installation")

        if (self.edge_weight is not None
                and not paddle_geometric.typing.WITH_WEIGHTED_NEIGHBOR_SAMPLE):
    ...
    def _sample(
        self,
        seed: Union[paddle.Tensor, Dict[NodeType, paddle.Tensor]],
        seed_time: Optional[Union[paddle.Tensor, Dict[NodeType, paddle.Tensor]]] = None,
        **kwargs,
    ) -> Union[SamplerOutput, HeteroSamplerOutput]:
        r"""Implements neighbor sampling by calling either :obj:`pgl-lib` (if
        installed) or Paddle's built-in sampling routines.
        """
        if isinstance(seed, dict):  # Heterogeneous sampling:
            ...
            if (paddle_geometric.typing.WITH_PGL_LIB
                ...
                dtype = colptrs[0].dtype if len(colptrs) > 0 else paddle.int64
                seed = {k: v.astype(dtype) for k, v in seed.items()}
                ...
                if paddle_geometric.typing.WITH_EDGE_TIME_NEIGHBOR_SAMPLE:
                    args += (self.edge_time, )
                args += (seed_time, )
                if paddle_geometric.typing.WITH_WEIGHTED_NEIGHBOR_SAMPLE:
                ...
                args += (
                    ...
                    # TODO Ensure `return_edge_id` if edge features present
                    True,  # return_edge_id
                )

                out = paddle.ops.pgl.hetero_neighbor_sample(*args)
                row, col, node, edge, batch = out[:4] + (None, )

                # `pgl-lib>0.1.0` returns sampled number of nodes/edges:
                num_sampled_nodes = num_sampled_edges = None
                if len(out) >= 6:
                    num_sampled_nodes, num_sampled_edges = out[4:6]

                if self.disjoint:
                    node = {k: v.transpose([1, 0]).contiguous() for k, v in node.items()}
                    batch = {k: v[0] for k, v in node.items()}
                    node = {k: v[1] for k, v in node.items()}

            elif paddle_geometric.typing.WITH_PADDLE_SPARSE:
                if self.disjoint:
                    if self.subgraph_type == SubgraphType.induced:
                        ...
                        raise ValueError("'disjoint' sampling not supported "
                                         ...
                                         "'paddle-sparse'. Please install "
                                         "'pgl-lib' for improved and "
                                         "optimized sampling routines.")

                out = paddle.ops.paddle_sparse.hetero_neighbor_sample(
                    self.node_types,
                    self.edge_types,
                    self.colptr_dict,
                    self.row_dict,
                ...
                raise ImportError(f"'{self.__class__.__name__}' requires "
                                  f"either 'pgl-lib' or 'paddle-sparse'")
            ...
            if (paddle_geometric.typing.WITH_PGL_LIB
                ...
                out = paddle.ops.pgl.neighbor_sample(*args)
                row, col, node, edge, batch = out[:4] + (None, )
                ...
                if self.disjoint:
                    batch, node = node.transpose([1, 0]).contiguous()

            elif paddle_geometric.typing.WITH_PADDLE_SPARSE:
                if self.disjoint:
                    raise ValueError("'disjoint' sampling not supported for "
                                     "neighbor sampling via 'paddle-sparse'. "
                                     "Please install 'pgl-lib' for improved "
                                     "and optimized sampling routines.")

                out = paddle.ops.paddle_sparse.neighbor_sample(
                    self.colptr,
                    self.row,
                    seed,  # seed
                    self.num_neighbors.get_mapped_values(),
...
def edge_sample(
    ...
    disjoint: bool,
    node_time: Optional[Union[paddle.Tensor, Dict[str, paddle.Tensor]]] = None,
    neg_sampling: Optional[NegativeSampling] = None,
) -> Union[SamplerOutput, HeteroSamplerOutput]:
    ...
    if neg_sampling is not None:
        ...
        if neg_sampling.is_binary():
            ...
            if edge_label is None:
                edge_label = paddle.ones([num_pos], dtype=paddle.int64)
            size = [num_neg] + list(edge_label.shape[1:])
            edge_neg_label = paddle.zeros(size, dtype=edge_label.dtype)
            edge_label = paddle.concat([edge_label, edge_neg_label])

            if edge_label_time is not None:
                src_time = dst_time = edge_label_time.tile(
            ...
            if edge_label_time is not None:
                dst_time = edge_label_time.tile([1 + neg_sampling.amount])
    ...
    if input_type is not None:
        ...
        if input_type[0] != input_type[-1]:  # Two distinct node types:
            ...
            if not disjoint:
                src, inverse_src = paddle.unique(src, return_inverse=True)
                dst, inverse_dst = paddle.unique(dst, return_inverse=True)
            ...
            if not disjoint:
                seed, inverse_seed = paddle.unique(seed, return_inverse=True)

            seed_dict = {input_type[0]: seed}

            if edge_label_time is not None:  # Always disjoint.
                seed_time_dict = {
                    input_type[0]: paddle.concat([src_time, dst_time], axis=0),
                }
        ...
        if neg_sampling is None or neg_sampling.is_binary():
            if disjoint:
                if input_type[0] != input_type[-1]:
                    edge_label_index = paddle.arange(num_pos + num_neg)
                    edge_label_index = edge_label_index.tile([2]).reshape([2, -1])
                else:
                    edge_label_index = paddle.arange(2 * (num_pos + num_neg))
                    edge_label_index = edge_label_index.reshape([2, -1])
            else:
                if input_type[0] != input_type[-1]:
                    edge_label_index = paddle.stack([
                        inverse_src,
                        inverse_dst,
                    ], axis=0)
                else:
            ...
            if disjoint:
                ...
                if input_type[0] != input_type[-1]:
                    dst_pos_index = paddle.arange(num_pos)
                    dst_neg_index = paddle.arange(
                        num_pos, seed_dict[input_type[-1]].shape[0])
                    dst_neg_index = dst_neg_index.reshape([-1, num_pos]).t()
                else:
                    dst_pos_index = paddle.arange(num_pos, 2 * num_pos)
                    dst_neg_index = paddle.arange(
                        2 * num_pos, seed_dict[input_type[-1]].shape[0])
                    dst_neg_index = dst_neg_index.reshape([-1, num_pos]).t()
            else:
                if input_type[0] != input_type[-1]:
            ...
            dst_neg_index = dst_neg_index.reshape([num_pos, -1]).squeeze(-1)
...
def neg_sample(
        ...
        num_nodes: int,
        seed_time: Optional[paddle.Tensor],
        node_time: Optional[paddle.Tensor],
        endpoint: Literal['src', 'dst'],
) -> paddle.Tensor:
    num_neg = math.ceil(seed.shape[0] * neg_sampling.amount)
    ...
    seed_time = paddle.expand(seed_time.reshape([1, -1]), [num_samples, -1])

    out = neg_sampling.sample(num_samples * seed.shape[0], endpoint, num_nodes)
    out = out.reshape([num_samples, seed.shape[0]])
    mask = paddle.greater(node_time[out], seed_time)  # holds all invalid samples.
    neg_sampling_complete = False

    for _ in range(5):  # Retry sampling to resolve invalid samples
        ...
        tmp = neg_sampling.sample(num_invalid, endpoint, num_nodes)
        out = paddle.where(mask, tmp, out)
        mask = paddle.greater(node_time[tmp], seed_time[mask])

    if not neg_sampling_complete:  # Fallback: Use node with minimum timestamp
        out = paddle.where(mask, paddle.argmin(node_time), out)

    return out.flatten()[:num_neg]
