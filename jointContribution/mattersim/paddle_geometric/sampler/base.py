...
from paddle_geometric.sampler.utils import to_bidirectional
from paddle_geometric.typing import EdgeType, EdgeTypeStr, NodeType, OptTensor
...
class DataType(Enum):
    ...
    def from_data(cls, data: Any):
        ...
        elif isinstance(data, (list, tuple)) and len(data) == 2 and isinstance(data[0], FeatureStore) and isinstance(data[1], GraphStore):
            return cls.remote
...
class NodeSamplerInput:
    ...
    Args:
        ...
        node (paddle.Tensor): The indices of seed nodes to start sampling from.
        time (paddle.Tensor, optional): The timestamp for the seed nodes.
            (default: :obj:`None`)
...
class EdgeSamplerInput:
    r"""The sampling input of :meth:`~paddle_geometric.sampler.BaseSampler.sample_from_edges`.

    Args:
        input_id (paddle.Tensor, optional): The indices of the data loader input
            of the current mini-batch.
        row (paddle.Tensor): The source node indices of seed links to start
            sampling from.
        col (paddle.Tensor): The destination node indices of seed links to start
            sampling from.
        label (paddle.Tensor, optional): The label for the seed links.
            (default: :obj:`None`)
        time (paddle.Tensor, optional): The timestamp for the seed links.
            (default: :obj:`None`)
...
class SamplerOutput:
    ...
    Args:
        node (paddle.Tensor): The sampled nodes in the original graph.
        row (paddle.Tensor): The source node indices of the sampled subgraph.
            Indices must be re-indexed to :obj:`{ 0, ..., num_nodes - 1 }`
            corresponding to the nodes in the :obj:`node` tensor.
        col (paddle.Tensor): The destination node indices of the sampled
            subgraph.
            Indices must be re-indexed to :obj:`{ 0, ..., num_nodes - 1 }`
            corresponding to the nodes in the :obj:`node` tensor.
        edge (paddle.Tensor, optional): The sampled edges in the original graph.
            This tensor is used to obtain edge features from the original
            graph. If no edge attributes are present, it may be omitted.
        batch (paddle.Tensor, optional): The vector to identify the seed node
        ...
        num_sampled_edges (List[int], optional): The number of sampled edges
            per hop. (default: :obj:`None`)
        orig_row (paddle.Tensor, optional): The original source node indices
        ...
        orig_col (paddle.Tensor, optional): The original destination node
    ...
    orig_row: Optional[Tensor] = None
    orig_col: Optional[Tensor] = None
    ...
    def to_bidirectional(
        ...
        # Assuming `to_bidirectional` is a function in the previous codebase
...
class HeteroSamplerOutput:
    ...
    Args:
        node (Dict[str, paddle.Tensor]): The sampled nodes in the original graph
            for each node type.
        row (Dict[Tuple[str, str, str], paddle.Tensor]): The source node indices
            of the sampled subgraph for each edge type.
            Indices must be re-indexed to :obj:`{ 0, ..., num_nodes - 1 }`
            corresponding to the nodes in the :obj:`node` tensor of the source
            node type.
        col (Dict[Tuple[str, str, str], paddle.Tensor]): The destination node
            indices of the sampled subgraph for each edge type.
            Indices must be re-indexed to :obj:`{ 0, ..., num_nodes - 1 }`
            corresponding to the nodes in the :obj:`node` tensor of the
            destination node type.
        edge (Dict[Tuple[str, str, str], paddle.Tensor], optional): The sampled
            edges in the original graph for each edge type.
            This tensor is used to obtain edge features from the original
            graph. If no edge attributes are present, it may be omitted.
        batch (Dict[str, paddle.Tensor], optional): The vector to identify the
        ...
        num_sampled_edges (Dict[EdgeType, List[int]], optional): The number of
            ...
            (default: :obj:`None`)
        orig_row (Dict[EdgeType, paddle.Tensor], optional): The original source
            node indices returned by the sampler.
            Filled in case :meth:`to_bidirectional` is called with the
            :obj:`keep_orig_edges` option. (default: :obj:`None`)
        orig_col (Dict[EdgeType, paddle.Tensor], optional): The original
