...
from paddle_geometric.typing import FeatureTensorType
...
class MyFeatureStore(FeatureStore):
    ...
    def _put_tensor(self, tensor: FeatureTensorType, attr: TensorAttr) -> bool:
        ...
        if index is None:
            index = paddle.arange(0, tensor.shape[0])
    ...
    def _get_tensor(self, attr: TensorAttr) -> Optional[Tensor]:
        ...
        idx = paddle.concat([(index == v).nonzero() for v in attr.index]).reshape([-1])
        return tensor[idx]
