...
def count_parameters(model: paddle.nn.Layer) -> int:
    r"""Given a :class:`paddle.nn.Layer`, count its trainable parameters.

    Args:
        model (paddle.nn.Layer): The model.
    """
    return sum([p.numel() for p in model.parameters() if p.trainable])


def get_model_size(model: paddle.nn.Layer) -> int:
    r"""Given a :class:`paddle.nn.Layer`, get its actual disk size in bytes.

    Args:
        model (paddle model): The model.
    """
    path = f'{random.randrange(sys.maxsize)}.pdmodel'
...
def get_data_size(data: BaseData) -> int:
    r"""Given a :class:`paddle_geometric.data.Data` object, get its theoretical
...
def get_cpu_memory_from_gc() -> int:
    ...
    """
    warnings.filterwarnings('ignore', '.*paddle.distributed.reduce_op.*')
