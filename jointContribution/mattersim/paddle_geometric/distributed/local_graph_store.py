...
from paddle_geometric.data import EdgeAttr, GraphStore
from paddle_geometric.distributed.partition import load_partition_info
from paddle_geometric.io import fs
from paddle_geometric.typing import EdgeTensorType, EdgeType, NodeType
from paddle_geometric.utils import sort_edge_index


class LocalGraphStore(GraphStore):
    r"""Implements the :class:`~paddle_geometric.data.GraphStore` interface to
    ...
    def get_partition_ids_from_eids(self, eids: paddle.Tensor,
    ...
    def from_data(
        ...
        Args:
            edge_id (paddle.Tensor): The global identifier for every local edge.
            edge_index (paddle.Tensor): The local edge indices.
    ...
    def from_hetero_data(
        ...
        Args:
            edge_id_dict (Dict[EdgeType, paddle.Tensor]): The global identifier
                for every local edge of every edge type.
            edge_index_dict (Dict[EdgeType, paddle.Tensor]): The local edge
    ...
    def from_partition(cls, root: str, pid: int) -> 'LocalGraphStore':
        ...
        graph_data = fs.paddle_load(osp.join(part_dir, 'graph.pdparams'))
        graph_store.is_sorted = meta['is_sorted']

        if not meta['is_hetero']:
            edge_index = paddle.stack([graph_data['row'], graph_data['col']],
                                      axis=0)
        ...
        if meta['is_hetero']:
            for edge_type, data in graph_data.items():
                ...
                edge_index = paddle.stack([data['row'], data['col']], axis=0)
