...
import paddle_geometric.distributed as pyg_dist
from paddle_geometric.data import Data, HeteroData
from paddle_geometric.io import fs
from paddle_geometric.loader.cluster import ClusterData
from paddle_geometric.sampler.utils import sort_csc
from paddle_geometric.typing import EdgeType, EdgeTypeStr, NodeType
...
class Partitioner:
    ...
    **Heterogeneous graphs:**
    ...
    def generate_partition(self):
        ...
        node_map = paddle.zeros([data.num_nodes], dtype=paddle.int64)
        edge_map = paddle.zeros([data.num_edges], dtype=paddle.int64)
        ...
        if self.is_hetero:
            ...
            for pid in range(self.num_parts):
                ...
                paddle.save(efeat, osp.join(path, 'edge_feats.pdparams'))
                paddle.save(graph, osp.join(path, 'graph.pdparams'))
                ...
                paddle.save(nfeat, osp.join(path, 'node_feats.pdparams'))
            ...
            for i, node_type in enumerate(self.node_types):
                ...
                paddle.save(node_map[mask], osp.join(path, f'{node_type}.pdparams'))
            ...
            for i, edge_type in enumerate(self.edge_types):
                ...
                paddle.save(
                    edge_map[mask],
                    osp.join(path, f'{EdgeTypeStr(edge_type)}.pdparams'),
                )
            ...
            logging.info('Saving partition mapping info')
            paddle.save(node_map, osp.join(self.root, 'node_map.pdparams'))
            paddle.save(edge_map, osp.join(self.root, 'edge_map.pdparams'))
...
def load_partition_info(
    root_dir: str,
    partition_idx: int,
) -> Tuple[Dict, int, int, paddle.Tensor, paddle.Tensor]:
    ...
    if meta['is_hetero'] is False:
        node_pb = fs.paddle_load(osp.join(root_dir, 'node_map.pdparams'))
        edge_pb = fs.paddle_load(osp.join(root_dir, 'edge_map.pdparams'))
        ...
        for ntype in meta['node_types']:
            node_pb_dict[ntype] = fs.paddle_load(
                osp.join(node_pb_dir, f'{pyg_dist.utils.as_str(ntype)}.pdparams'))

        edge_pb_dict = {}
        edge_pb_dir = osp.join(root_dir, 'edge_map')
        for etype in meta['edge_types']:
            edge_pb_dict[tuple(etype)] = fs.paddle_load(
                osp.join(edge_pb_dir, f'{pyg_dist.utils.as_str(etype)}.pdparams'))
