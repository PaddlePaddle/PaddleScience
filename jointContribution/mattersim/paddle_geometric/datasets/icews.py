...
class EventDataset(InMemoryDataset):
    ...
    def _process_data_list(self) -> List[Data]:
        ...
        events = events - paddle.min(events, axis=0, keepdim=True)

        data_list = []
        for (sub, rel, obj, t) in events.numpy().tolist():
...
class ICEWS18(EventDataset):
    ...
    def __init__(
        ...
        super().__init__(root, transform, pre_transform, pre_filter,
                         force_reload=force_reload)
        idx = self.processed_file_names.index(f'{split}.pdparams')
    ...
    def process_events(self) -> Tensor:
        ...
        for path in self.raw_paths:
            data = read_txt_array(path, sep='\t', end=4, dtype='int64')
            data[:, 3] = data[:, 3] // 24
            events.append(data)
        return paddle.concat(events, axis=0)
