...
from paddle_geometric.data import HeteroData, InMemoryDataset, download_url
...
class IGMCDataset(InMemoryDataset):
    ...
    def load_matlab_file(path_file: str, name: str) -> Tensor:
        ...
        out = paddle.to_tensor(np.asarray(db[name]), dtype='float32').transpose()
        db.close()
    ...
    def process(self) -> None:
        ...
        if self.name == 'flixster':
            ...
            user_x = self.load_matlab_file(self.raw_paths[0], 'W_users')
            item_x = paddle.eye(M.shape[1])
        elif self.name == 'yahoo_music':
            user_x = paddle.eye(M.shape[0])
        ...
        edge_index = paddle.nonzero(train_mask).transpose([1, 0])
        ...
        edge_label_index = paddle.nonzero(test_mask).transpose([1, 0])
