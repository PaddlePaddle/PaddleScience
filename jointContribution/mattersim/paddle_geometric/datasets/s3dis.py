...
class S3DIS(InMemoryDataset):
    ...
    def process(self) -> None:
        ...
        for filename in filenames:
            ...
            xs += [paddle.to_tensor(h5['data'][:][i]) for i in range(len(h5['data']))]
            ys += [paddle.to_tensor(h5['label'][:][i], dtype=paddle.int64) for i in range(len(h5['label']))]
