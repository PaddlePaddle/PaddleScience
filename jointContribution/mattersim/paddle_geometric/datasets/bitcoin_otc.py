...
class BitcoinOTC(InMemoryDataset):
    ...
    **STATS:**
    ...
    def num_nodes(self) -> int:
        ...
        return int(paddle.max(self._data.edge_index)) + 1
    ...
    def process(self) -> None:
        with open(self.raw_paths[0]) as f:
            lines = [[x for x in line.split(',')] for line in f.read().split('\n')[:-1]]

            edge_indices = [[int(line[0]), int(line[1])] for line in lines]
            edge_index = paddle.to_tensor(edge_indices, dtype='int64')
            edge_index = edge_index - paddle.min(edge_index)
            edge_index = edge_index.t()
            num_nodes = int(paddle.max(edge_index)) + 1

            edge_attrs = [int(line[2]) for line in lines]
            edge_attr = paddle.to_tensor(edge_attrs, dtype='int64')
        ...
        graph_idx = paddle.to_tensor(graph_indices, dtype='int64')

        data_list = []
        for i in range(int(paddle.max(graph_idx)) + 1):
            mask = (graph_idx > (i - self.edge_window_size)) & (graph_idx <= i)
            data = Data()
            data.edge_index = edge_index[:, mask.nonzero().flatten()]
            data.edge_attr = edge_attr[mask.nonzero().flatten()]
