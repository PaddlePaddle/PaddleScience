...
class JODIEDataset(InMemoryDataset):
    ...
    Args:
        ...
        transform (callable, optional): A function/transform that takes in an
            :obj:`paddle_geometric.data.TemporalData` object and returns a
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        src = paddle.to_tensor(df.iloc[:, 0].values, dtype='int64')
        dst = paddle.to_tensor(df.iloc[:, 1].values, dtype='int64')
        dst += int(src.max().item()) + 1
        t = paddle.to_tensor(df.iloc[:, 2].values, dtype='int64')
        y = paddle.to_tensor(df.iloc[:, 3].values, dtype='int64')
        msg = paddle.to_tensor(df.iloc[:, 4:].values, dtype='float32')
