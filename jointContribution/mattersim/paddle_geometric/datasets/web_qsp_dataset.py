...
def retrieval_via_pcst(
    ...
    verbosity_level = 0
    if topk > 0:
        n_prizes = paddle.nn.functional.cosine_similarity(q_emb, data.x)
        topk = min(topk, data.num_nodes)
        _, topk_n_indices = paddle.topk(n_prizes, topk, largest=True)

        n_prizes = paddle.zeros_like(n_prizes)
        n_prizes[topk_n_indices] = paddle.arange(topk, 0, -1, dtype='float32')
    else:
        n_prizes = paddle.zeros([data.num_nodes])

    if topk_e > 0:
        e_prizes = paddle.nn.functional.cosine_similarity(q_emb, data.edge_attr)
        unique_e_prizes, _ = paddle.unique(e_prizes)
        topk_e = min(topk_e, unique_e_prizes.size)

        topk_e_values, _ = paddle.topk(unique_e_prizes, topk_e, largest=True)
        e_prizes = paddle.where(e_prizes < topk_e_values[-1], paddle.zeros_like(e_prizes), e_prizes)
        last_topk_e_value = topk_e
        for k in range(topk_e):
            indices = (e_prizes == topk_e_values[k])
            value = min((topk_e - k) / paddle.sum(indices), last_topk_e_value - c)
            e_prizes = paddle.where(indices, paddle.full_like(e_prizes, value), e_prizes)
            last_topk_e_value = value * (1 - c)
        cost_e = min(cost_e, e_prizes.max().item() * (1 - c / 2))
    else:
        e_prizes = paddle.zeros([data.num_edges])
    ...
    if len(virtual_vertices) > 0:
        ...
        selected_edges = np.concatenate([selected_edges, virtual_edges])
