...
from paddle_geometric.data import Data, InMemoryDataset, extract_tar
...
class OMDB(InMemoryDataset):
    ...
    def processed_file_names(self) -> List[str]:
        return ['train_data.pdparams', 'test_data.pdparams']
    ...
    def process(self) -> None:
        ...
        for material, bandgap in zip(materials, bandgaps):
            pos = paddle.to_tensor(material.get_positions(), dtype='float32')
            z = paddle.to_tensor(material.get_atomic_numbers(), dtype='int64')
            y = paddle.to_tensor([float(bandgap)], dtype='float32')
