...
class DynamicFAUST(InMemoryDataset):
    ...
    .. note::
        ...
        :obj:`paddle_geometric.transforms.SamplePoints` as :obj:`transform` to
    ...
    def process(self) -> None:
        ...
        face = paddle.to_tensor(fm['faces'][()], dtype='int64')
        ...
        for (sid, cat) in product(self.subjects, self.categories):
            idx = f'{sid}_{cat}'
            if idx in fm:
                pos = paddle.to_tensor(fm[idx][()])
            elif idx in ff:
                pos = paddle.to_tensor(ff[idx][()])
            else:
                continue
            pos = pos.transpose([2, 0, 1])
            data_list.append(Data(pos=pos, face=face, num_nodes=pos.shape[1]))
