...
class MD17(InMemoryDataset):
    ...
    **STATS:**
    ...
    def processed_file_names(self) -> List[str]:
        if self.ccsd:
            return ['train.pdparams', 'test.pdparams']
        else:
    ...
    def process(self) -> None:
        ...
        for raw_path, processed_path in it:
            ...
            if self.revised:
                z = paddle.to_tensor(raw_data['nuclear_charges'], dtype='int64')
                pos = paddle.to_tensor(raw_data['coords'], dtype='float32')
                energy = paddle.to_tensor(raw_data['energies'], dtype='float32')
                force = paddle.to_tensor(raw_data['forces'], dtype='float32')
            else:
                z = paddle.to_tensor(raw_data['z'], dtype='int64')
                pos = paddle.to_tensor(raw_data['R'], dtype='float32')
                energy = paddle.to_tensor(raw_data['E'], dtype='float32')
                force = paddle.to_tensor(raw_data['F'], dtype='float32')
