...
class HeterophilousGraphDataset(InMemoryDataset):
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        x = paddle.to_tensor(raw['node_features'], dtype='float32')
        y = paddle.to_tensor(raw['node_labels'], dtype='int64')
        edge_index = paddle.to_tensor(raw['edges'].T, dtype='int64')
        edge_index = to_undirected(edge_index, num_nodes=x.shape[0])
        train_mask = paddle.to_tensor(raw['train_masks'].T, dtype='bool')
        val_mask = paddle.to_tensor(raw['val_masks'].T, dtype='bool')
        test_mask = paddle.to_tensor(raw['test_masks'].T, dtype='bool')
