...
class EllipticBitcoinDataset(InMemoryDataset):
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        x = paddle.to_tensor(feat_df.loc[:, 2:].values, dtype='float32')
        ...
        y = paddle.to_tensor(class_df['class'].values, dtype='int64')
        ...
        edge_index = paddle.to_tensor(edge_df.values.T, dtype='int64')

        # Timestamp based split:
        # train_mask: 1 - 34 time_step, test_mask: 35-49 time_step
        time_step = paddle.to_tensor(feat_df['time_step'].values, dtype='int64')
        train_mask = paddle.logical_and(time_step < 35, y != 2)
        test_mask = paddle.logical_and(time_step >= 35, y != 2)
