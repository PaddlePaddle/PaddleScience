...
from paddle_geometric.data import Data, OnDiskDataset, download_url, extract_zip
...
class PCQM4Mv2(OnDiskDataset):
    ...
    Args:
        ...
        from_smiles (callable, optional): A custom function that takes a SMILES
            string and outputs a :obj:`~paddle_geometric.data.Data` object.
            If not set, defaults to :meth:`~paddle_geometric.utils.from_smiles`.
            (default: :obj:`None`)
    """
    ...
    def __init__(
        ...
        schema = {
            'x': dict(dtype=paddle.int64, size=(-1, 9)),
            'edge_index': dict(dtype=paddle.int64, size=(2, -1)),
            'edge_attr': dict(dtype=paddle.int64, size=(-1, 3)),
            'smiles': str,
            'y': float,
        }
        ...
        split_idx = fs.paddle_load(self.raw_paths[1])
