...
class MixHopSyntheticDataset(InMemoryDataset):
    ...
    def process(self) -> None:
        x = paddle.to_tensor(np.load(self.raw_paths[0]), dtype='float32')
        y = paddle.to_tensor(np.load(self.raw_paths[1]), dtype='int64').argmax(axis=-1)

        with open(self.raw_paths[2], 'rb') as f:
        ...
        edge_index = paddle.to_tensor([row, col], dtype='int64')
