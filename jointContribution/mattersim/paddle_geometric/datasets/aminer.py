...
class AMiner(InMemoryDataset):
    ...
    def process(self) -> None:
        ...
        data['author'].y = paddle.to_tensor(df['y'].values, dtype='int64') - 1
        data['author'].y_index = paddle.to_tensor(df['idx'].values, dtype='int64')
        ...
        data['venue'].y = paddle.to_tensor(df['y'].values, dtype='int64') - 1
        data['venue'].y_index = paddle.to_tensor(df['idx'].values, dtype='int64')

        # Get paper<->author connectivity.
        path = osp.join(self.raw_dir, 'paper_author.txt')
        paper_author = pd.read_csv(path, sep='\t', header=None)
        paper_author = paddle.to_tensor(paper_author.values, dtype='int64').t()
        ...
        paper_venue = paddle.to_tensor(paper_venue.values, dtype='int64').t()
