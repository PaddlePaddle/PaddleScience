...
class BrcaTcga(InMemoryDataset):
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        graph_feat = paddle.to_tensor(graph_feat, dtype='float32')
        graph_labels = np.loadtxt(self.raw_paths[1], delimiter=',')
        graph_label = paddle.to_tensor(graph_labels, dtype='float32')
        edge_index = fs.paddle_load(self.raw_paths[2])

        data_list = []
        for x, y in zip(graph_feat, graph_label):
            data = Data(x=x.reshape([-1, 1]), edge_index=edge_index, y=y)
