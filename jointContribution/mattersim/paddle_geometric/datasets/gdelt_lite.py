...
class GDELTLite(InMemoryDataset):
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        x = fs.paddle_load(self.raw_paths[0])
        df = pd.read_csv(self.raw_paths[1])
        edge_attr = fs.paddle_load(self.raw_paths[2])

        row = paddle.to_tensor(df['src'].values, dtype='int64')
        col = paddle.to_tensor(df['dst'].values, dtype='int64')
        edge_index = paddle.stack([row, col], axis=0)
        time = paddle.to_tensor(df['time'].values, dtype='int64')
