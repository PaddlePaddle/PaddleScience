...
try:  # pragma: no cover
    LEGACY_MODE = False
    from pylibcugraphops.paddle import CSC, HeteroCSC
...
class CuGraphModule(paddle.nn.Layer):  # pragma: no cover
    ...
    def get_cugraph(
        ...
        if num_src_nodes != colptr.shape[0] - 1:  # Bipartite graph:
            if max_num_neighbors is None:
                max_num_neighbors = int((colptr[1:] - colptr[:-1]).max())

            if LEGACY_MODE:
                dst_nodes = paddle.arange(colptr.shape[0] - 1, device=row.device)
    ...
    def get_typed_cugraph(
        ...
        Args:
            edge_index (EdgeIndex): The edge indices.
            edge_type (paddle.Tensor): The edge type.
        ...
        edge_type = edge_type.astype('int32')
