...
from paddle_geometric.nn.fx import Transformer, get_submodule
from paddle_geometric.typing import EdgeType, Metadata, NodeType
from paddle_geometric.utils.hetero import (
...
try:
    ...
    GraphLayer, Graph, Node = 'GraphLayer', 'Graph', 'Node'  # 后期需要替换
...
def to_hetero(module: Layer, metadata: Metadata, aggr: str = "sum",
              input_map: Optional[Dict[str, str]] = None,
              debug: bool = False) -> GraphLayer:
    """
...
class ToHeteroTransformer(Transformer):
    ...
    def call_message_passing_module(self, node: Node, target: Any, name: str):
        ...
        for key in self.metadata[1]:
            ...
            out = self.graph.create_node('call_layer',
        ...
        for dst, keys in keys_per_dst.items():
            ...
            if self.aggr == 'mean':
                ...
                out = self.graph.create_node(
                    'call_function', target=paddle.divide,
    ...
    def init_submodule(self, module: Layer, target: str) -> Layer:
        ...
        module_dict = paddle.nn.LayerDict()
        for key in self.metadata[int(has_edge_level_target)]:
            ...
            elif sum([is_uninitialized_parameter(p) or p.numel()
                      for p in module.parameters()]) > 0:
                warnings.warn(
