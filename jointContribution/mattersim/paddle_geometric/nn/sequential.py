...
from paddle_geometric.inspector import Parameter, Signature, eval_type, split
...
class Sequential(paddle.nn.Layer):
    r"""An extension of the :class:`paddle.nn.Layer` container in order to
    define a sequential GNN model.

    Since GNN operators take in multiple input arguments,
    :class:`paddle_geometric.nn.Sequential` additionally expects both global
    ...
    .. code-block:: python
        ...
        from paddle_geometric.nn import Sequential, GCNConv
    ...
    def __getitem__(self, idx: int) -> paddle.nn.Layer:
    ...
    def _set_jittable_template(self, raise_on_error: bool = False) -> None:
        try:  # Optimize `forward()` via `*.jinja` templates:
            ...
            # support `paddle.jit.trace` - this is generally dangerous to do,
            # and limits `paddle.jit.trace` to a single `Sequential` module:
    ...
    def __prepare_scriptable__(self) -> 'Sequential':
        ...
        type_store = paddle.jit._recursive.concrete_type_store.type_store
