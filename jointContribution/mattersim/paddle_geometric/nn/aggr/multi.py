...
from paddle.nn import Linear, MultiHeadAttention
...
class MultiAggregation(Aggregation):
    ...
    def __init__(
        ...
        self.aggrs = paddle.nn.LayerList([
        ...
        if mode in dense_combine_modes:
            self.dense_combine = getattr(paddle, mode)
    ...
    def combine(self, inputs: List[Tensor]) -> Tensor:
        ...
        if hasattr(self, 'lin'):
            return self.lin(paddle.concat(inputs, axis=-1))
        ...
        if hasattr(self, 'dense_combine'):
            out = self.dense_combine(paddle.stack(inputs, axis=0), axis=0)
