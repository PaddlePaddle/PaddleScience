...
class AttentionalAggregation(Aggregation):
    ...
    Args:
        gate_nn (paddle.nn.Layer): A neural network :math:`h_{\mathrm{gate}}`
            that computes attention scores by mapping node features :obj:`x` of
            shape :obj:`[-1, in_channels]` to shape :obj:`[-1, 1]` (for
            node-level gating) or :obj:`[1, out_channels]` (for feature-level
            gating), *e.g.*, defined by :class:`paddle.nn.Sequential`.
        nn (paddle.nn.Layer, optional): A neural network
    ...
    def __init__(
        self,
        gate_nn: paddle.nn.Layer,
        nn: Optional[paddle.nn.Layer] = None,
    ):
        super().__init__()
