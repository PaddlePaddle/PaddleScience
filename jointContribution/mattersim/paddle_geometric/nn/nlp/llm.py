...
def get_llm_kwargs(required_memory: int, dtype=paddle.float32) -> Dict[str, Any]:
    paddle.cuda.empty_cache()

    gpu_memory: List[int] = []
    for i in range(paddle.device.cuda.device_count()):
        gpu_memory.append(paddle.device.cuda.memory_allocated(i) // 1024**3)
...
class LLM(paddle.nn.Layer):
    ...
    num_params (int): An integer representing how many parameters the
        ...
        memory of your GPUs.
    dtype (paddle.dtype, optional): The data type to use for the LLM.
        (default :obj: `paddle.float16`)
    """
    def __init__(
        self,
        model_name: str,
        ...
        if 'max_memory' not in kwargs:  # Pure CPU:
            ...
            self.device = paddle.device('cpu')
            self.autocast_context = nullcontext()
        else:
            self.device = self.llm.device
            self.autocast_context = paddle.amp.autocast('gpu', dtype=dtype)
    ...
    def _encode_inputs(
        ...
        pad_token = paddle.to_tensor(self.tokenizer.pad_token_id,
                                     device=self.device)
    ...
    def _inputs_embeds(
        ...
        inputs_embeds = self.word_embedding(
            paddle.to_tensor(input_ids, device=self.device))

        to_cat = [bos_embeds]
        if embedding is not None and embedding[i] is not None:
            to_cat.append(embedding[i])
        to_cat.append(inputs_embeds)
        return paddle.concat(to_cat, axis=0).to(self.device)
    ...
    def _append_embeds(
        ...
        if label_input_ids is not None:
            pad = inputs_embeds.shape[0] - len(label_input_ids)
    ...
    def _pad_embeds(
        ...
        for i in range(batch_size):
            pad = max_length - batch_inputs_embeds[i].shape[0]
            batch_inputs_embeds[i] = paddle.concat([pad_embeds.repeat(pad, 1),
        ...
        inputs_embeds = paddle.stack(batch_inputs_embeds, axis=0)
        attention_mask = paddle.to_tensor(batch_attention_mask, device=self.device)
        label_input_ids = None
        if batch_label_input_ids is not None:
            label_input_ids = paddle.to_tensor(batch_label_input_ids,
                                               device=self.device)
