...
class PANPooling(paddle.nn.Layer):
    ...
    def __init__(
        ...
        # Initialize parameters with paddle.create_parameter
        self.p = paddle.create_parameter(shape=[in_channels], dtype='float32')
        self.beta = paddle.create_parameter(shape=[2], dtype='float32')
    ...
    def forward(
        ...
        """
        if batch is None:
            batch = x.new_zeros(x.shape[0], dtype=paddle.long)

        row, col, edge_weight = M.coo()
        assert edge_weight is not None

        score1 = (x * self.p).sum(axis=-1)
        score2 = scatter(edge_weight, col, 0, dim_size=x.shape[0], reduce='sum')
