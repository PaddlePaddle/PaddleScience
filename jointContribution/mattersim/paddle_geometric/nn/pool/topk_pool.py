...
class TopKPooling(nn.Layer):
    ...
    If :obj:`min_score` :math:`\tilde{\alpha}` is :obj:`None`, computes:
        ...
        .. math::
            \mathbf{y} &= \sigma \left( \frac{\mathbf{X}\mathbf{p}}{\| \mathbf{p} \|} \right)

            \mathbf{i} &= \mathrm{top}_k(\mathbf{y})

            \mathbf{X}^{\prime} &= (\mathbf{X} \odot \mathrm{tanh}(\mathbf{y}))_{\mathbf{i}}
    ...
    where nodes are dropped based on a learnable projection score :math:`\mathbf{p}`.
    ...
    def __init__(
        ...
        # SelectTopK and FilterEdges are assumed to be paddle_geometric equivalents
