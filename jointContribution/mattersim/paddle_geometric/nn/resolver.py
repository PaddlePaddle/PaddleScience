...
from paddle_geometric.nn.lr_scheduler import (
...
from paddle_geometric.resolver import normalize_string, resolver
...
def activation_resolver(query: Union[Any, str] = 'relu', *args, **kwargs):
    ...
    acts = [
        ...
        if isinstance(getattr(paddle.nn, act), type) and issubclass(getattr(paddle.nn, act), base_cls)
    ]
    acts += [
        swish,
    ]
