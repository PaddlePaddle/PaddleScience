...
from paddle_geometric.nn.models.dimenet import triplets
from paddle_geometric.nn.models.schnet import ShiftedSoftplus
from paddle_geometric.typing import OptTensor
from paddle_geometric.utils import scatter


class GaussianFilter(nn.Layer):
    ...
    def forward(self, dist: Tensor) -> Tensor:
        ...
        return paddle.exp(self.coeff * dist.pow(2))


class NodeBlock(nn.Layer):
    def __init__(self, hidden_node_channels: int, hidden_edge_channels: int):
        super().__init__()
        self.lin_c1 = nn.Linear(hidden_node_channels + hidden_edge_channels,
        ...
        self.bn_c1 = nn.BatchNorm1D(2 * hidden_node_channels)
        self.bn = nn.BatchNorm1D(hidden_node_channels)
    ...
    def forward(self, node_emb: Tensor, edge_emb: Tensor, i: Tensor) -> Tensor:
        c1 = paddle.concat([node_emb[i], edge_emb], axis=1)
        c1 = self.bn_c1(self.lin_c1(c1))
        c1_filter, c1_core = paddle.chunk(c1, 2, axis=1)
        c1_filter = c1_filter.sigmoid()
        c1_core = c1_core.tanh()
        c1_emb = scatter(c1_filter * c1_core, i, dim=0,
                         dim_size=node_emb.shape[0], reduce='sum')
...
class EdgeBlock(nn.Layer):
    def __init__(self, hidden_node_channels: int, hidden_edge_channels: int):
        super().__init__()
        self.lin_c2 = nn.Linear(hidden_node_channels, 2 * hidden_edge_channels)
        self.lin_c3 = nn.Linear(
        ...
        # BN was added based on previous studies.
        # ref: https://github.com/txie-93/cgcnn/blob/master/cgcnn/model.py
        self.bn_c2 = nn.BatchNorm1D(2 * hidden_edge_channels)
        self.bn_c3 = nn.BatchNorm1D(2 * hidden_edge_channels)
        self.bn_c2_2 = nn.BatchNorm1D(hidden_edge_channels)
        self.bn_c3_2 = nn.BatchNorm1D(hidden_edge_channels)
    ...
    def forward(
        ...
        c2_filter, c2_core = paddle.chunk(c2, 2, axis=1)
...
class GNNFF(nn.Layer):
    ...
    def __init__(
        ...
        self.node_blocks = nn.LayerList([
            NodeBlock(hidden_node_channels, hidden_edge_channels)
            for _ in range(num_layers)
        ])
        self.edge_blocks = nn.LayerList([
    ...
    def forward(self, z: Tensor, pos: Tensor,
        ...
        dist = (pos[i] - pos[j]).pow(2).sum(axis=-1).sqrt()
