...
class CorrectAndSmooth(paddle.nn.Layer):
    ...
    def forward(self, y_soft: Tensor, *args) -> Tensor:
    ...
    def correct(self, y_soft: Tensor, y_true: Tensor, mask: Tensor,
        ...
        numel = int(mask.sum()) if mask.dtype == paddle.bool else mask.size(0)
        assert y_true.size(0) == numel

        if y_true.dtype == paddle.int64 and y_true.size(0) == y_true.numel():
            y_true = one_hot(y_true.view(-1), num_classes=y_soft.size(-1),
                             dtype=y_soft.dtype)

        error = paddle.zeros_like(y_soft)
        error[mask] = y_true - y_soft[mask]

        if self.autoscale:
            smoothed_error = self.prop1(error, edge_index,
                                        edge_weight=edge_weight,
                                        post_step=lambda x: paddle.clip(x, min=-1., max=1.))

            sigma = error[mask].abs().sum() / numel
            scale = sigma / smoothed_error.abs().sum(axis=1, keepdim=True)
