...
from paddle_geometric.nn import TemporalEncoding
from paddle_geometric.utils import scatter, to_dense_batch


class NodeEncoder(nn.Layer):
    ...
    def forward(
        ...
        Args:
            x (paddle.Tensor): The input node features.
            edge_index (paddle.Tensor): The edge indices.
            edge_time (paddle.Tensor): The timestamp attached to every edge.
            seed_time (paddle.Tensor): The seed time :math:`t_0` for every
                destination node.
        """
        ...
        mean = scatter(x[src], dst, dim=0, dim_size=x.shape[0], reduce='mean')
        return x + mean
...
class _MLPMixer(nn.Layer):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Args:
            x (paddle.Tensor): Tensor of size
...
def get_latest_k_edge_attr(
    ...
    if not is_sorted:
        perm = np.lexsort([edge_time.detach().cpu().numpy(), col.detach().cpu().numpy()])
        perm = paddle.to_tensor(perm).to(edge_index.device)
        col = col[perm]
...
class LinkEncoder(nn.Layer):
    ...
    def forward(
        ...
        Args:
            edge_index (paddle.Tensor): The edge indices.
            edge_attr (paddle.Tensor): The edge features of shape
                :obj:`[num_edges, in_channels]`.
            edge_time (paddle.Tensor): The time tensor of shape
        ...
        edge_attr = paddle.concat([time_enc, edge_attr], axis=-1)
