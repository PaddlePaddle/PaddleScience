...
class JumpingKnowledge(nn.Layer):
    ...
    **max pooling** (:obj:`"max"`)
    ...
    def forward(self, xs: List[Tensor]) -> Tensor:
        ...
        Args:
            xs (List[paddle.Tensor]): List containing the layer-wise
                representations.
        """
        if self.mode == 'cat':
            return paddle.concat(xs, axis=-1)
        elif self.mode == 'max':
            return paddle.stack(xs, axis=-1).max(axis=-1)[0]
        else:  # self.mode == 'lstm'
            assert self.lstm is not None and self.att is not None
            x = paddle.stack(xs, axis=1)  # [num_nodes, num_layers, num_channels]
            alpha, _ = self.lstm(x)
            alpha = self.att(alpha).squeeze(-1)  # [num_nodes, num_layers]
            alpha = paddle.nn.functional.softmax(alpha, axis=-1)
...
class HeteroJumpingKnowledge(nn.Layer):
    ...
    def forward(self, xs_dict: Dict[str, List[Tensor]]) -> Dict[str, Tensor]:
        ...
        Args:
            xs_dict (Dict[str, List[paddle.Tensor]]): A dictionary holding a
