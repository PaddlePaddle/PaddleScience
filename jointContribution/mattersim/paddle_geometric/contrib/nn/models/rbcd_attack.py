...
class PRBCDAttack(paddle.nn.Layer):
    ...
    *e.g.*, :class:`~paddle_geometric.nn.conv.GCNConv` or
    :class:`~paddle_geometric.nn.conv.GraphConv`. For non-differentiable models
    ...
    Args:
        model (paddle.nn.Layer): The GNN module to assess.
    ...
    def attack(
        ...
        Args:
            ...
            labels (paddle.Tensor): The labels.
            budget (int): The number of allowed perturbations (i.e.
                number of edges that are flipped at most).
            idx_attack (paddle.Tensor, optional): Filter for predictions/labels.
    ...
    def _update(self, epoch: int, gradient: Tensor, x: Tensor, labels: Tensor,
        ...
        # For monitoring
        pmass_update = paddle.clamp(self.block_edge_weight, 0, 1)
        ...
        topk_block_edge_weight = paddle.zeros_like(self.block_edge_weight)
        topk_block_edge_weight[paddle.topk(self.block_edge_weight,
                                           budget).indices] = 1
    ...
    def _forward_and_gradient(self, x: Tensor, labels: Tensor,
        ...
        self.block_edge_weight.stop_gradient = False
        ...
        # (Algorithm 1, line 7 / Algorithm 2, line 8)
        gradient = paddle.autograd.grad(loss, self.block_edge_weight)[0]
    ...
    def _get_modified_adj(self, edge_index: Tensor, edge_weight: Tensor,
        ...
        modified_edge_index = paddle.concat(
            (edge_index.to(paddle.CUDAPlace(0)), block_edge_index), axis=-1)
        modified_edge_weight = paddle.concat(
            (edge_weight.to(paddle.CUDAPlace(0)), block_edge_weight))
    ...
    def _sample_random_block(self, budget: int = 0):
        for _ in range(self.coeffs['max_trials_sampling']):
            ...
            self.current_block = paddle.randint(high=num_possible_edges,
                                                shape=[self.block_size, ])
            self.current_block = paddle.unique(self.current_block)
            self.current_block = paddle.sort(self.current_block)
            if self.is_undirected:
            ...
            self.block_edge_weight = paddle.full(self.current_block.shape,
                                                 self.coeffs['eps'])
            if self.current_block.shape[0] >= budget:
                return
    ...
    def _resample_random_block(self, budget: int):
        ...
        sorted_idx = paddle.argsort(self.block_edge_weight)
        keep_above = (self.block_edge_weight
                      <= self.coeffs['eps']).sum().long()
        if keep_above < sorted_idx.shape[0] // 2:
            keep_above = sorted_idx.shape[0] // 2
        ...
        for _ in range(self.coeffs['max_trials_sampling']):
            n_edges_resample = self.block_size - self.current_block.shape[0]
            num_possible_edges = self._num_possible_edges(
                self.num_nodes, self.is_undirected)
            lin_index = paddle.randint(num_possible_edges, (n_edges_resample,))

            current_block = paddle.concat((self.current_block, lin_index))
            self.current_block, unique_idx = paddle.unique(
            ...
            self.block_edge_weight = paddle.full(self.current_block.shape,
                                                 self.coeffs['eps'])
            self.block_edge_weight[
                unique_idx[:sorted_idx.shape[0]]] = block_edge_weight_prev
    ...
    def _sample_final_edges(self, x: Tensor, labels: Tensor, budget: int,
        ...
        for i in range(self.coeffs['max_final_samples']):
            if i == 0:
                ...
                sampled_edges = paddle.zeros_like(block_edge_weight)
                sampled_edges[paddle.topk(block_edge_weight,
                                          budget).indices] = 1
            else:
                sampled_edges = paddle.bernoulli(block_edge_weight).float()
    ...
    def _project(budget: int, values: Tensor, eps: float = 1e-7) -> Tensor:
        ...
        """
        if paddle.clamp(values, 0, 1).sum() > budget:
            left = (values - 1).min()
            right = values.max()
            miu = PRBCDAttack._bisection(values, left, right, budget)
            values = values - miu
        return paddle.clamp(values, min=eps, max=1 - eps)
    ...
    def _bisection(edge_weights: Tensor, a: float, b: float, n_pert: int,
        ...
        def shift(offset: float):
            return (paddle.clamp(edge_weights - offset, 0, 1).sum() - n_pert)
    ...
    def _linear_to_triu_idx(n: int, lin_idx: Tensor) -> Tensor:
        ...
        """
        nn = n * (n - 1)
        row_idx = n - 2 - paddle.floor(
            paddle.sqrt(-8 * lin_idx.double() + 4 * nn - 7) / 2.0 - 0.5).long()
        col_idx = 1 + lin_idx + row_idx - nn // 2 + paddle.div(
            (n - row_idx) * (n - row_idx - 1), 2, rounding_mode='floor')
        return paddle.stack((row_idx, col_idx))

    @staticmethod
    def _linear_to_full_idx(n: int, lin_idx: Tensor) -> Tensor:
        """Linear index to dense matrix including diagonal."""
        n = paddle.full_like(lin_idx, n, dtype=lin_idx.dtype)
        n = paddle.cast(n, 'float32')
        lin_idx = paddle.cast(lin_idx, 'float32')
        row_idx = paddle.divide(lin_idx, n)
        row_idx = paddle.floor(row_idx)
        col_idx = lin_idx % n
        return paddle.stack((row_idx, col_idx))
    ...
    def _margin_loss(score: Tensor, labels: Tensor,
        ...
        linear_idx = paddle.arange(score.shape[0])
...
class GRBCDAttack(PRBCDAttack):
    ...
    def _prepare(self, budget: int) -> List[int]:
        """Prepare attack."""
        self.flipped_edges = paddle.empty([2, 0], dtype=self.edge_index.dtype)
    ...
    def _update(self, step_size: int, gradient: Tensor, *args,
                **kwargs) -> Dict[str, Any]:
        """Update edge weights given gradient."""
        _, topk_edge_index = paddle.topk(gradient, step_size)

        flip_edge_index = self.block_edge_index[:, topk_edge_index]
        flip_edge_weight = paddle.ones_like(flip_edge_index[0],
                                            dtype=paddle.float32)

        self.flipped_edges = paddle.concat((self.flipped_edges, flip_edge_index),
                                        axis=-1)
        ...
        edge_index = paddle.concat(
            ...
            dim=-1)
        edge_weight = paddle.concat((self.edge_weight.to(self.device),
                                  flip_edge_weight.to(self.device)))
        edge_index, edge_weight = coalesce(edge_index, edge_weight,
                                           num_nodes=self.num_nodes,
                                           reduce='sum')

        is_one_mask = paddle.isclose(edge_weight, paddle.to_tensor(1.))
        self.edge_index = edge_index[:, is_one_mask]
        self.edge_weight = edge_weight[is_one_mask]
        # self.edge_weight = paddle.ones_like(self.edge_weight)
        assert self.edge_index.shape[1] == self.edge_weight.shape[0]
