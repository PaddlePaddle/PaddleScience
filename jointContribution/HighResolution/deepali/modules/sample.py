...
from ..core.grid import grid_points_transform
from ..core.linalg import homogeneous_matmul
from ..core.linalg import homogeneous_transform
from ..core.types import Scalar


class SampleImage(paddle.nn.Layer):
    """Sample images at grid points."""
    ...
    def axes(self) -> Axes:
        """Axes with respect to which target grid points and transformations thereof are defined."""
        return self._axes

    def target_grid(self) -> Grid:
        """Target sampling grid."""
        return self._target
    ...
    def padding(self) -> Union[PaddingMode, Scalar]:
        """Image padding mode or value, respectively."""
    ...
    def padding_mode(self) -> PaddingMode:
        ...
        return (
            self._padding
            if isinstance(self._padding, PaddingMode)
            else PaddingMode.CONSTANT
        )

    def padding_value(self) -> float:
        """Image padding value if mode is "constant"."""
    ...
    def align_corners(self) -> bool:
        """Whether to sample images using ``align_corners=False`` or ``align_corners=True``."""
        return self._target.align_corners()

    def _matrix(self) -> paddle.Tensor:
        """Homogeneous coordinate transformation from target grid points to source grid cube."""
        ...
        if self._align_centers:
            offset = self._target.world_to_cube(
                self._source.center(), align_corners=align_corners
            )
    ...
    def _transform_target_to_source(self, grid: paddle.Tensor) -> paddle.Tensor:
        ...
        matrix = cast(paddle.Tensor, self.matrix)
    ...
    def _sample_source_image(
        ...
        data: Optional[paddle.paddle.Tensor] = None,
        mask: Optional[paddle.Tensor] = None,
    ) -> Union[
        paddle.Tensor, Tuple[paddle.Tensor, paddle.Tensor], Dict[str, paddle.Tensor]
    ]:
        """Sample images at specified source grid points."""
        source = {}
        output = {}
        shape = None
        align_corners = self.align_corners()
        ...
        if isinstance(input, dict):
            ...
            for data in input.values():
                if not isinstance(data, paddle.Tensor):
                    raise TypeError(
                        f"{type(self).__name__}() 'input' dict values must be paddle.Tensor"
                    )
            ...
            raise TypeError(
                f"{type(self).__name__}() 'input' must be paddle.Tensor or Mapping[str, paddle.Tensor]"
            )
    ...
    def forward(
        ...
        """Sample images at target grid points after mapping these to the source grid cube."""
...
class TransformImage(SampleImage):
    """Sample images at transformed target grid points.
    ...
    def _grid(self) -> paddle.Tensor:
        """Target grid points before spatial transformation."""
    ...
    def forward(
        ...
        """Sample images at transformed target grid points after mapping these to the source grid cube."""
        grid = cast(paddle.Tensor, self.grid)
        if isinstance(transform, paddle.Tensor):
            ...
            raise TypeError("TransformImage() 'transform' must be paddle.Tensor")
        grid = self._transform_target_to_source(grid)
        return self._sample_source_image(grid, input=input, data=data, mask=mask)


class AlignImage(SampleImage):
    """Sample images at linearly transformed target grid points.
    ...
    def forward(
        ...
        """Sample batch of optionally masked images at linearly transformed target grid points."""
        composite_transform = cast(paddle.Tensor, self.matrix)
        if transform is not None:
            if not isinstance(transform, paddle.Tensor):
                raise TypeError("AlignImage() 'transform' must be paddle.Tensor")
            if transform.ndim != 3:
                raise ValueError(
                    "AlignImage() 'transform' must be 3-dimensional tensor"
                )
