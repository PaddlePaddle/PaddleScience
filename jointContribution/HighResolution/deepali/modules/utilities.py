...
def has_children(module: paddle.nn.Layer) -> bool:
    """Check if module has other modules as children."""
    try:
...
def module_ids_by_class_name(
    ...
    """Obtain ids of module objects in model, indexed by module type name.
    ...
    for module in modules.values():
        ...
        module_ids_by_class_name(
            module, duplicates=duplicates, containers=containers, memo=memo
        )
    if not duplicates:
...
def module_counts_by_class_name(
    ...
    """Count how many module objects are in a model of each module type.
...
def remove_layers_in_state_dict(
    state: Mapping[str, Any], prefix: str
) -> Dict[str, Any]:
    """Remove layers in loaded state dict."""
    metadata = getattr(state, "_metadata", None)
    result = OrderedDict(state)
...
def rename_layers_in_state_dict(
    ...
    """Rename layers in loaded state dict."""
    metadata = getattr(state, "_metadata", None)
