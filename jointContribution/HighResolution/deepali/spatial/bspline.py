...
class BSplineTransform(ParametricTransform, NonRigidTransform):
    """Non-rigid transformation parameterized by cubic B-spline function."""
    ...
    def __init__(
        ...
        if not grid.align_corners():
            raise ValueError(
                "BSplineTransform() requires 'grid.align_corners() == True'"
            )
        if stride is None:
            stride = 5
        ...
        if len(stride) != grid.ndim:
            raise ValueError(
                f"BSplineTransform() 'stride' must be single int or {grid.ndim} ints"
            )
    ...
    def data_shape(self) -> list:
        ...
        grid = self.grid()
        shape = U.cubic_bspline_control_point_grid_size(
            tuple(grid.shape), self.data_stride
        )
    ...
    def grid_(self: TBSplineTransform, grid: Grid) -> TBSplineTransform:
        ...
        if subdivide_dims:
            ...
            for dim in subdivide_dims:
                ...
                start = 1
                length = new_shape[dim]
                new_params = paddle.slice(
                    new_params, axes=[dim], starts=[start], ends=[start + length]
                )
            self.data_(new_params)
        return self

    @staticmethod
    def kernel_name(stride: int) -> str:
        """Get name of buffer for 1-dimensional kernel for given control point spacing."""
    ...
    def kernel(
        ...
        """Get 1-dimensional kernels for given control point spacing."""
        if stride is None:
            stride = self.stride
        if isinstance(stride, int):
    ...
    def register_kernels(self, stride: Union[int, Sequence[int]]) -> None:
        """Precompute cubic B-spline kernels."""
        if isinstance(stride, int):
            stride = [stride]
        for s in stride:
    ...
    def deregister_kernels(self, stride: Union[int, Sequence[int]]) -> None:
        """Remove precomputed cubic B-spline kernels."""
        if isinstance(stride, int):
            stride = [stride]
        for s in stride:
            name = self.kernel_name(s)
    ...
    def evaluate_spline(self) -> paddle.Tensor:
        """Evaluate cubic B-spline at sampling grid points."""
        data = self.data()
        grid = self.grid()
        if not grid.align_corners():
...
class FreeFormDeformation(BSplineTransform):
    """Cubic B-spline free-form deformation model."""
...
class StationaryVelocityFreeFormDeformation(BSplineTransform):
    """Stationary velocity field based transformation model using cubic B-spline parameterization."""
    ...
    def __init__(
        ...
        super().__init__(
            grid, groups=groups, params=params, stride=stride, transpose=transpose
        )
