...
class ParametricTransform:
    """Mix-in for spatial transformations that have (optimizable) parameters."""
    ...
    def __init__(
        ...
        Args:
            ...
            params: Initial parameters. If a tensor is given, it is only registered as optimizable module
                parameters when of type ``paddle.base.framework.EagerParamBase``. When a callable is given instead, it will be
        ...
        if params is None:
            ...
            if isinstance(params, paddle.base.framework.EagerParamBase):
                self.params = params
            else:
            ...
            self.register_buffer(
                name="p", tensor=paddle.empty(shape=shape), persistable=False
            )
            self.reset_parameters()
        else:
            raise TypeError(
                f"{type(self).__name__}() 'params' must be bool, Callable, paddle.Tensor, or None"
            )

    def has_parameters(self) -> bool:
        """Whether this transformation has optimizable parameters."""
    ...
    def data_shape(self) -> list:
        """Get required shape of transformation parameters tensor, excluding batch dimension."""
        raise NotImplementedError(f"{type(self).__name__}.data_shape")

    @overload
    def data(self) -> paddle.Tensor:
        """Get (buffered) transformation parameters."""
        ...

    @overload
    def data(self: TSpatialTransform, arg: paddle.Tensor) -> TSpatialTransform:
        """Get shallow copy with specified parameters."""
        ...

    def data(
        self: Union[TSpatialTransform, ParametricTransform],
        arg: Optional[paddle.Tensor] = None,
    ) -> Union[TSpatialTransform, paddle.Tensor]:
        """Get transformation parameters or shallow copy with specified parameters, respectively."""
        params = self.params
        if arg is None:
            if params is None:
        ...
        if arg.shape != shape:
            raise ValueError(
                f"{type(self).__name__}.data() 'arg' must have shape {shape!r}"
            )
        ...
        if isinstance(params, paddle.base.framework.EagerParamBase) and not isinstance(
            ...
            out_1 = paddle.create_parameter(
                shape=arg.shape,
                dtype=arg.numpy().dtype,
                default_initializer=paddle.nn.initializer.Assign(arg),
            )
            out_1.stop_gradient = not not params.stop_gradient
            copy.params = out_1
        else:
            copy.params = arg
        copy.clear_buffers()
    ...
    def data_(
        ...
        """Replace transformation parameters.

        Args:
            arg: paddle.Tensor of transformation parameters with shape matching ``self.data_shape``,
        ...
        if not isinstance(arg, paddle.Tensor):
            raise TypeError(
                f"{type(self).__name__}.data_() 'arg' must be tensor, not {type(arg)}"
            )
        shape = self.data_shape
        if arg.ndim != len(shape) + 1:
            raise ValueError(
        ...
        if isinstance(params, paddle.base.framework.EagerParamBase) and not isinstance(
            ...
            out_2.stop_gradient = not not params.stop_gradient
            self.params = out_2
        else:
            self.params = arg
        self.clear_buffers()
    ...
    def _data(self: Union[TSpatialTransform, ParametricTransform]) -> paddle.Tensor:
        ...
        if params is None:
            raise AssertionError(
                f"{type(self).__name__}._data() 'params' must be set first"
            )
        ...
        if callable(params):
            ...
            if tuple(pred.shape) != shape:
                raise ValueError(
                    f"{type(self).__name__}.params() tensor must have shape {shape!r}"
                )
            return pred
    ...
    def link(
        ...
        """Make shallow copy of this transformation which is linked to another instance."""
    ...
    def link_(
        ...
        """Link this transformation to another of the same type.
        ...
        if other is self:
            raise ValueError(
                f"{type(self).__name__}.link() cannot link tranform to itself"
            )
    ...
    def unlink(
        ...
        """Make a shallow copy of this transformation with parameters set to ``None``."""
        return shallow_copy(self).unlink_()

    def unlink_(
        self: Union[TSpatialTransform, ParametricTransform]
    ) -> TSpatialTransform:
        """Resets transformation parameters to ``None``."""
        self.params = None
        if hasattr(self, "p"):
            delattr(self, "p")
        return self
...
class InvertibleParametricTransform(ParametricTransform):
    """Mix-in for spatial transformations that support on-demand inversion."""
    ...
    def __init__(
        ...
        Args:
            ...
            groups: Number of transformations. A given image batch can either be deformed by a
                ...
                in the batch, or the batch length of the ``params`` tensor if provided.
            params: Initial parameters. If a tensor is given, it is only registered as optimizable module
                parameters when of type ``paddle.base.framework.EagerParamBaser``. When a callable is given instead, it will be
