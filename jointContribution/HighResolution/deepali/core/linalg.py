...
class HomogeneousTensorType(Enum):
    """Type of homogeneous transformation tensor."""
...
def as_homogeneous_matrix(
    ...
    """Convert tensor to homogeneous coordinate transformation matrix.

    Args:
        tensor: paddle.Tensor of translations of shape ``(D,)`` or ``(..., D, 1)``, tensor of square affine
...
def homogeneous_transform(
    ...
    """Transform points or vectors by given homogeneous transformations.
    ...
    Args:
        transform: paddle.Tensor of translations of shape ``(D,)``, ``(D, 1)`` or ``(N, D, 1)``, tensor of
    ...
    Returns:
        paddle.Tensor of transformed points/vectors with the same shape as the input ``points``, except
    ...
    if tuple(transform.shape)[2] == 1:
        ...
        perm_7[1] = 2
        perm_7[2] = 1
        points = paddle.bmm(x=points, y=x.transpose(perm=perm_7))
...
def hmm(a: paddle.Tensor, b: paddle.Tensor) -> paddle.Tensor:
    """Compose two homogeneous coordinate transformations.

    Args:
        a: paddle.Tensor of second homogeneous transformation.
        b: paddle.Tensor of first homogeneous transformation.
...
def homogeneous_matmul(*args: paddle.Tensor) -> paddle.Tensor:
    """Compose homogeneous coordinate transformations.
    ...
    dtype = a.dtype
    device = a.place
    if dtype not in (paddle.float16, paddle.float32, paddle.float64):
    ...
    for b in args[1:]:
        ...
        if str(b.place) != str(device):
            raise RuntimeError(
                "homogeneous_matmul() tensors must be on the same 'device'"
            )
        ...
        if a_numel > 1:
            ...
            if b.ndim > a.ndim:
                raise ValueError(
                    "Homogeneous tensors have different number of leading dimensions"
                )
        ...
        assert (
            c is not None
        ), "as_homogeneous_tensor() returned invalid 'type' enumeration value"
...
def homogeneous_matrix(
    tensor: paddle.Tensor,
    offset: Optional[paddle.paddle.Tensor] = None,
    dtype: Optional[paddle.dtype] = None,
    device: Optional[Device] = None,
) -> paddle.Tensor:
    """Convert square matrix or vector to homogeneous coordinate transformation matrix.
...
def vectordot(
    ...
    """Inner product of vectors over specified input tensor dimension."""
    c = a.mul(b)
    if w is not None:
        c.mul(w)
...
def vector_rotation(a: paddle.Tensor, b: paddle.Tensor) -> paddle.Tensor:
    """Calculate rotation matrix which aligns two 3D vectors."""
    ...
    norm: paddle.Tensor = axis.norm(p=2, axis=-1, keepdim=True)
