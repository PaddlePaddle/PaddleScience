...
def apply_transform(
    ...
    """Alias for :func:`.homogeneous_transform`."""
...
def affine_rotation_matrix(matrix: paddle.Tensor) -> paddle.Tensor:
    """Get orthonormal rotation matrix from (homogeneous) affine transformation.
    ...
    if (
        matrix.ndim < 2
        or tuple(matrix.shape)[-2] != 3
        or tuple(matrix.shape)[-1] not in (3, 4)
    ):
        raise ValueError(
            "affine_rotation_matrix() 'matrix' must have shape (..., 3, 3|4)"
        )
    matrix = matrix[(...), :3].clone()
    sx: paddle.Tensor = paddle.linalg.norm(x=matrix[..., 0], p=2, axis=-1)
    ...
    sy: paddle.Tensor = paddle.linalg.norm(x=matrix[..., 1], p=2, axis=-1)
    ...
    sz: paddle.Tensor = paddle.linalg.norm(x=matrix[..., 2], p=2, axis=-1)
...
def identity_transform(
    ...
    *args,
    ...
    """Create homogeneous coordinate transformation matrix of identity mapping.
...
def rotation_matrix(*args, **kwargs) -> paddle.Tensor:
    """Alias for :func:`.euler_rotation_matrix`."""
...
def euler_rotation_matrix(
    ...
    Args:
        ...
        dtype: Data type of rotation matrix. If ``None``, use ``angles.dtype`` if it is
            a floating point type, and ``paddle.float`` otherwise.
        device: Device on which to create rotation matrix. If ``None``, use ``angles.device``.

    Returns:
        paddle.Tensor of square rotation matrices of shape ``(..., D, D)`` if ``homogeneous=False``, or tensor
    ...
    matrix = paddle.empty(
        shape=tuple(angles_.shape)[:-1] + (D, D + 1 if homogeneous else D),
        dtype=angles_.dtype,
    )
    if homogeneous:
        matrix[..., D] = 0
...
def euler_rotation_angles(
    matrix: paddle.Tensor, order: Optional[str] = None
) -> paddle.Tensor:
    ...
    if matrix.ndim < 2:
        raise ValueError(
            "euler_rotation_angles() 'matrix' must be at least 2-dimensional"
        )
...
def euler_rotation_order(arg: Optional[str] = None, ndim: int = 3) -> str:
    """Standardize rotation order argument."""
...
def scaling_transform(
    ...
    Args:
        scales: paddle.Tensor of anisotropic scaling factors of shape ``(..., D)``.
        homogeneous: Whether to return homogeneous transformation matrices.
    ...
    Returns:
        paddle.Tensor of square scaling matrices of shape ``(..., D, D)`` if ``homogeneous=False``, or tensor of
    ...
    J = tuple(range(D))
    matrix = paddle.zeros(
        shape=tuple(scales_.shape)[:-1] + (D, D + 1 if homogeneous else D),
        dtype=scales_.dtype,
    )
    # matrix[..., J, J] = scales_
    for j in J:
...
def shear_matrix(
    ...
    Args:
        angles: paddle.Tensor of anisotropic shear angles in radians of shape ``(..., N)``,
...
def translation(
    ...
    if homogeneous:
        ...
        J = tuple(range(D))
        matrix = paddle.zeros(
            shape=tuple(offset_.shape)[:-1] + (D, D + 1), dtype=offset_.dtype
        )
        matrix[..., J, J] = 1
        matrix[..., D] = offset_
...
def transform_points(transforms: paddle.Tensor, points: paddle.Tensor) -> paddle.Tensor:
    """Transform points by given homogeneous transformation."""
    return apply_transform(transforms, points, vectors=False)


def transform_vectors(
    transforms: paddle.Tensor, vectors: paddle.Tensor
) -> paddle.Tensor:
    """Transform vectors by given homogeneous transformation."""
