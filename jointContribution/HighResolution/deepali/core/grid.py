...
class Axes(Enum):
    ...
    CUBE = "cube"
    """Oriented along grid axes with units corresponding to unit cube :math:`[-1, 1]^D`
    ...
    def from_grid(cls, grid: Grid) -> Axes:
        """Create enumeration value from sampling grid object."""
...
class Grid(object):
    ...
    __slots__ = "_size", "_center", "_spacing", "_direction", "_align_corners"
    ...
    def __init__(
        ...
        """Initialize sampling grid attributes.
        ...
        if origin is None:
            ...
            with paddle.no_grad():
                origin = cat_scalars(
                    origin, num=self.ndim, dtype=self.dtype, device=self.device
                )
    ...
    def numpy(self) -> np.ndarray:
        """Get grid attributes as 1-dimensional NumPy array."""
    ...
    def from_numpy(
        ...
        """Create Grid from 1-dimensional NumPy array."""
    ...
    def from_seq(
        ...
        """Create Grid from sequence of attribute values.
        ...
        Args:
            attrs: Array of length (D + 3) * D, where ``D=2`` or ``D=3`` is the number
                ...
                list or tuple, NumPy array, or tensor.
    ...
    def from_batch(cls, tensor: paddle.Tensor) -> Grid:
        """Create default grid from (image) batch tensor.
    ...
    def from_file(cls, path: PathStr, align_corners: bool = ALIGN_CORNERS) -> Grid:
        """Create sampling grid from image file header information."""
        if _sitk is None:
    ...
    @classmethod
    def from_reader(
        cls, reader: "_sitk.ImageFileReader", align_corners: bool = ALIGN_CORNERS
    ) -> Grid:
        """Create sampling grid from image file reader attributes."""
        return cls(
            size=reader.GetSize(),
    ...
    @classmethod
    def from_sitk(
        cls, image: "_sitk.Image", align_corners: bool = ALIGN_CORNERS
    ) -> Grid:
        """Create sampling grid from ``SimpleITK.Image`` attributes."""
        return cls(
            size=image.GetSize(),
    ...
    def cube(self) -> "Cube":
        """Get oriented cube defining the space of normalized coordinates."""
        from .cube import Cube
    ...
    def domain(self) -> "Cube":
        """Get oriented bounding box defining the sampling grid domain in world space."""
        return self.cube()
    ...
    def dtype(self) -> paddle.dtype:
        """Get data type of grid attribute tensors."""
        return self._size.dtype

    @property
    def device(self) -> str:
        """Get device on which grid attribute tensors are stored."""
        return self._size.place

    def clone(self) -> Grid:
        """Make deep copy of this ``Grid`` instance."""
    ...
    def __deepcopy__(self, memo) -> Grid:
        """Support copy.deepcopy to clone this grid."""
        if id(self) in memo:
            return memo[id(self)]
        copy = self.clone()
        memo[id(self)] = copy
    ...
    def _round_size(size: paddle.Tensor) -> paddle.Tensor:
        """Round sampling grid size attribute."""
    ...
    def size_tensor(self) -> paddle.Tensor:
        """Sampling grid size as floating point tensor."""
        return self._round_size(self._size)

    @overload
    def size(self, i: int) -> int:
        """Sampleing grid size along the specified spatial dimension."""
        ...

    @overload
    def size(self) -> list:
        """Sampling grid size for dimensions ordered as ``(X, ...)``."""
        ...
    ...
    def extent(self, i: Optional[int] = None) -> paddle.Tensor:
        """Extent of sampling grid in physical world units."""
        if i is None:
    ...
    def cube_extent(self, i: Optional[int] = None) -> paddle.Tensor:
        """Extent of sampling grid cube in physical world units."""
        if i is None:
            n = self.size_tensor()
    ...
    def center(self) -> paddle.Tensor:
        """Get grid center point in world space."""
        ...

    @overload
    def center(self, arg: Union[float, Array], *args: float) -> Grid:
        """Get new grid with specified center point in world space."""
        ...

    def center(
        self, arg: Union[float, Array, None] = None, *args: float
    ) -> Union[paddle.Tensor, Grid]:
        """Get center point in world space or new grid with specified center point, respectively."""
        if arg is None:
            if args:
                raise ValueError(
    ...
    def center_(self, arg: Union[float, Array], *args: float) -> Grid:
        """Set grid center point in world space."""
    ...
    def origin(self) -> paddle.Tensor:
        """Get world coordinates of grid point with index zero."""
        ...

    @overload
    def origin(self, arg: Union[float, Array], *args: float) -> Grid:
        """Get new grid with specified world coordinates of grid point at index zero."""
        ...

    def origin(
        self, arg: Union[float, Array, None] = None, *args: float
    ) -> Union[paddle.Tensor, Grid]:
        """Get grid origin in world space or new grid with specified origin, respectively."""
        if arg is None:
            if args:
                raise ValueError(
            ...
            size = self.size_tensor()
            offset = paddle.where(
                condition=size.greater_than(y=paddle.to_tensor(0.0)),
                x=size.sub(1.0),
                y=size,
            ).div(2.0)
    ...
    def origin_(self, arg: Union[float, Array], *args: float) -> Grid:
        """Set world coordinates of grid point with zero index."""
        origin = cat_scalars(
            arg, *args, num=self.ndim, dtype=self.dtype, device=self.device
        )
        size = self.size_tensor()
    ...
    def spacing(self) -> paddle.Tensor:
        """Get spacing between grid points in world units."""
        ...

    @overload
    def spacing(self, arg: Union[float, Array], *args: float) -> Grid:
        """Get new grid with specified spacing between grid points in world units."""
        ...

    def spacing(
        self, arg: Union[float, Array, None] = None, *args: float
    ) -> Union[paddle.Tensor, Grid]:
        """Get spacing between grid points in world units or new grid with specified spacing, respectively."""
        if arg is None:
            if args:
                raise ValueError(
    ...
    def spacing_(self, arg: Union[float, Array], *args: float) -> Grid:
        """Set spacing between grid points in physical world units."""
        spacing = cat_scalars(
            arg, *args, num=self.ndim, dtype=self.dtype, device=self.device
        )
        if spacing.less_equal(y=paddle.to_tensor(0.0)).astype("bool").any():
    ...
    def direction(self) -> paddle.Tensor:
        """Get grid axes direction cosines matrix."""
        ...

    @overload
    def direction(self, arg: Union[float, Array], *args: float) -> Grid:
        """Get new grid with specified axes direction cosines."""
        ...

    def direction(
        self, arg: Union[float, Array, None] = None, *args: float
    ) -> Union[paddle.Tensor, Grid]:
        """Get grid axes direction cosines matrix or new grid with specified direction, respectively."""
        if arg is None:
            if args:
                raise ValueError(
    ...
    def direction_(self, arg: Union[float, Array], *args: float) -> Grid:
        """Set grid axes direction cosines matrix of this grid."""
        D = self.ndim
        direction = paddle.to_tensor(data=(arg,) + args) if args else as_tensor(arg)
        direction = direction.astype(self.dtype)
        if direction.ndim == 1:
        ...
        with paddle.no_grad():
            if abs(paddle.linalg.det(direction).abs().item() - 1) > 0.0001:
                raise ValueError(
                    "Grid direction cosines matrix must be valid rotation matrix"
                )
        self._direction = direction
        return self

    def affine(self) -> paddle.Tensor:
        """Affine transformation from ``Axes.GRID`` to ``Axes.WORLD``, excluding translation of origin."""
        return paddle.mm(input=self.direction(), mat2=paddle.diag(x=self.spacing()))

    def inverse_affine(self) -> paddle.Tensor:
        """Affine transformation from ``Axes.WORLD`` to ``Axes.GRID``, excluding translation of origin."""
    ...
    def transform(
        ...
        """Transformation from one grid domain to another.
        ...
        if to_grid is None or to_grid == self:
            if axes is to_axes:
                ...
                if to_axes is Axes.CUBE:
                    ...
                    if not vectors:
                        one = paddle.to_tensor(
                            data=1, dtype=size.dtype, place=size.place
                        )
                ...
                if to_axes is Axes.CUBE_CORNERS:
                    ...
                    grid_to_world = self.transform(
                        Axes.GRID, Axes.WORLD, vectors=vectors
                    )
                    if vectors:
            ...
            raise ValueError(
                f"Grid.transform() 'to_grid' must have {self.ndim} spatial dimensions"
            )
        else:
        ...
        if matrix is None:
            raise NotImplementedError(
                f"Grid.transform() for axes={axes} and to_axes={to_axes}"
            )
        return matrix

    def inverse_transform(self, vectors: bool = False) -> paddle.Tensor:
        """Transform which maps from world to grid cube space."""
        cube_axes = Axes.CUBE_CORNERS if self._align_corners else Axes.CUBE
    ...
    def apply_transform(
        ...
        """Map point coordinates or displacement vectors from one grid to another.
    ...
    def transform_points(
        ...
        """Map point coordinates from one grid domain to another.
        ...
        return self.apply_transform(
            points, axes, to_axes, to_grid=to_grid, decimals=decimals
        )
            ...
            if axes is not to_axes:
                ...
                if to_axes is Axes.WORLD:
                    ...
                    if scales is None:
                        ...
                        affine = paddle.mm(
                            input=grid_to_world, mat2=paddle.diag(x=scales)
                        )
                    ...
                    if affine is None:
                        ...
                        affine = paddle.mm(
                            input=paddle.diag(x=grid_to_cube), mat2=affine
                        )
                ...
                if affine is None:
                    ...
                    vectors = paddle.nn.functional.linear(
                        weight=affine.T, x=tensor
                    ).reshape(tuple(vectors.shape))
        else:
    ...
    def index_to_cube(
        ...
        """Map points from grid indices to grid-aligned cube with side length 2.
        ...
        return self.transform_points(
            indices, axes=Axes.GRID, to_axes=to_axes, decimals=decimals
        )

    def cube_to_index(
        self, coords: Array, decimals: int = -1, align_corners: Optional[bool] = None
    ) -> paddle.Tensor:
        """Map points from grid-aligned cube to grid point indices.
        ...
        return self.transform_points(
            coords, axes=axes, to_axes=Axes.GRID, decimals=decimals
        )

    def index_to_world(self, indices: Array, decimals: int = -1) -> paddle.Tensor:
        """Map points from grid indices to world coordinates.
        ...
        """
        return self.transform_points(
            indices, axes=Axes.GRID, to_axes=Axes.WORLD, decimals=decimals
        )

    def world_to_index(self, points: Array, decimals: int = -1) -> paddle.Tensor:
        """Map points from world coordinates to grid point indices.
        ...
        """
        return self.transform_points(
            points, axes=Axes.WORLD, to_axes=Axes.GRID, decimals=decimals
        )

    def cube_to_world(
        self, coords: Array, decimals: int = -1, align_corners: Optional[bool] = None
    ) -> paddle.Tensor:
        """Map point coordinates from grid-aligned cube with side length 2 to world space.
        ...
        axes = Axes.from_align_corners(align_corners)
        return self.transform_points(
            coords, axes=axes, to_axes=Axes.WORLD, decimals=decimals
        )

    def world_to_cube(
        self, points: Array, decimals: int = -1, align_corners: Optional[bool] = None
    ) -> paddle.Tensor:
        """Map point coordinates from world space to grid-aligned cube with side length 2.
        ...
        to_axes = Axes.from_align_corners(align_corners)
        return self.transform_points(
            points, axes=Axes.WORLD, to_axes=to_axes, decimals=decimals
        )
    ...
    def coords(
        ...
        """Get tensor of grid point coordinates.
        ...
        Args:
            ...
            device: Device on which to create tensor. If ``None``, use ``self.device``.
    ...
    def points(
        ...
        """paddle.Tensor of grid point coordinates with respect to specified coordinate axes."""
        axes = Axes(axes)
        coords = self.coords(
    ...
    def _resize(
        self, size: paddle.Tensor, align_corners: Optional[bool] = None
    ) -> Grid:
        """Get new grid of specified size.
        ...
        if align_corners:
            ...
            grid._spacing = paddle.where(
                condition=self._size.greater_than(y=paddle.to_tensor(0.0)),
                x=spacing,
                y=self._spacing,
            )
    ...
    def resize(
        ...
        *args: int,
        ...
        """Create new grid of same extent with specified size.

        Specify new grid size for grid dimensions in the order (X...). Note that this is in reverse order
        of ``paddle.Tensor.size()``! To set the grid size given a data tensor shape, use ``Grid.reshape()``.
        ...
        if not is_int_dtype(size.dtype):
            raise TypeError(
                f"Grid.resize() 'size' must be integer values, got dtype={size.dtype}"
            )
    ...
    def reshape(
        ...
        *args: int,
        ...
        """Create new grid of same extent with specified data tensor shape.
        ...
        convenience function to change the grid size given the ``paddle.Tensor.shape`` of a data tensor.
    ...
    def resample(
        self, spacing: Union[float, Array, str], *args: float, min_size: int = 1
    ) -> Grid:
        """Create new grid with specified spacing.
        ...
        spacing = cat_scalars(
            spacing, *args, num=self.ndim, dtype=self.dtype, device=self.device
        )
    ...
    def pool(
        ...
        """Output grid after applying pooling operation.
        ...
        ks = cat_scalars(
            kernel_size, num=self.ndim, dtype=self.dtype, device=self.device
        )
    ...
    def avg_pool(
        ...
        """Output grid after applying average pooling."""
        return self.pool(
    ...
    def downsample(
        ...
        """Create new grid with size halved the specified number of times.
        ...
        for dim in dims:
            size[dim] /= scale
        size = paddle.where(
            condition=size.greater_equal(
                y=paddle.to_tensor(min_size, dtype=size.dtype)
            ),
            x=size,
            y=self._size,
        )
    ...
    def upsample(
        ...
        """Create new grid of same extent with size doubled the specified number of times.
    ...
    def pyramid(
        ...
        """Compute image size at each image resolution pyramid level.
    ...
    def crop(
        ...
        *args: int,
        ...
        """Create new grid with a margin along each axis removed.
        ...
        """
        if (
            sum([1 if args else 0, 0 if num is None else 1, 0 if margin is None else 1])
            != 1
        ):
            raise AssertionError(
                "Grid.pad() 'args', 'margin', and 'num' are mutually exclusive"
            )
    ...
    def pad(
        ...
        *args: int,
        ...
        """Create new grid with an additional margin along each axis.
    ...
    def center_crop(self, size: Union[int, Array], *args: int) -> Grid:
        """Crop grid to specified maximum size."""
        size = cat_scalars(size, *args, num=self.ndim, device=self.device)
        if not is_int_dtype(size.dtype):
            raise TypeError(
    ...
    def center_pad(self, size: Union[int, Array], *args: int) -> Grid:
        """Pad grid to specified minimum size."""
        size = cat_scalars(size, *args, num=self.ndim, device=self.device)
        if not is_int_dtype(size.dtype):
            raise TypeError(
                f"Grid.center_crop() expected scalar or array of integer values, got dtype={size.dtype}"
    ...
    def narrow(self, dim: int, start: int, length: int) -> Grid:
        """Narrow grid along specified dimension."""
    ...
    def region_of_interest(
        self, start: Union[int, Array], size: Union[int, Array]
    ) -> Grid:
        """Get region of interest grid."""
        ...
        grid_size = self.size()
        num = [
            [start[i], grid_size[i] - (start[i] + size[i])] for i in range(self.ndim)
        ]
...
def grid_points_transform(
    grid: Grid, axes: Axes, to_grid: Grid, to_axes: Optional[Axes] = None
):
    """Get linear transformation of points from one grid domain to another.
...
def grid_vectors_transform(
    ...
    """Get affine transformation which maps vectors with respect to one grid domain to another.
