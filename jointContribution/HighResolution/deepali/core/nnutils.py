...
def as_immutable_container(
    ...
    """Convert mutable container such as dict or list to an immutable container type.
...
def conv_output_size(
    ...
    """Calculate spatial size of output tensor after convolution."""
    device = str("cpu").replace("cuda", "gpu")
    m: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=in_size, dtype="int32", place=device)
    )
    k: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=kernel_size, dtype="int32", place=device)
    )
    s: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=stride, dtype="int32", place=device)
    )
    d: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=dilation, dtype="int32", place=device)
    )
    if m.ndim != 1:
    ...
    if padding == "valid":
        ...
        raise ValueError(
            "conv_output_size() 'padding' string must be 'valid' or 'same'"
        )
    p: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=padding, dtype="int32", place=device)
    )
...
def conv_transposed_output_size(
    ...
    """Calculate spatial size of output tensor after transposed convolution."""
    device = str("cpu").replace("cuda", "gpu")
    m: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=in_size, dtype="int32", place=device)
    )
    ...
    o: paddle.Tensor = paddle.atleast_1d(
    ...
    if m.ndim != 1:
        raise ValueError(
            "conv_transposed_output_size() 'in_size' must be scalar or sequence"
        )
    ndim = tuple(m.shape)[0]
    if ndim == 1 and tuple(k.shape)[0] > 1:
        ndim = tuple(k.shape)[0]
...
def pad_output_size(
    ...
    """Calculate spatial size of output tensor after padding."""
    device = str("cpu").replace("cuda", "gpu")
    m: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=in_size, dtype="int32", place=device)
    )
...
def pool_output_size(
    ...
    """Calculate spatial size of output tensor after pooling."""
    device = str("cpu").replace("cuda", "gpu")
    m: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=in_size, dtype="int32", place=device)
    )
    ...
    if ndim == 1 and tuple(k.shape)[0] > 1:
        ndim = tuple(k.shape)[0]
    for arg, name in zip(
        [k, s, p, d], ["kernel_size", "stride", "padding", "dilation"]
    ):
        if arg.ndim != 1 or arg.shape[0] not in (1, ndim):
            raise ValueError(
...
def unpool_output_size(
    ...
    """Calculate spatial size of output tensor after unpooling."""
    device = str("cpu").replace("cuda", "gpu")
    m: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=in_size, dtype="int32", place=device)
    )
...
def same_padding(
    ...
    """Padding value needed to ensure convolution preserves input tensor shape.
    ...
    device = str("cpu").replace("cuda", "gpu")
    k: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=kernel_size, dtype="int64", place=device)
    )
    d: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=dilation, dtype="int64", place=device)
    )
    if k.ndim != 1:
    ...
    for arg, name in zip([k, d], ["kernel_size", "dilation"]):
        if arg.ndim != 1 or arg.shape[0] not in (1, ndim):
            raise ValueError(
                f"same_padding() {name!r} must be scalar or sequence of length {ndim}"
            )
    ...
    p = k.sub(1).div(2).mul(d).astype("int32")
...
def stride_minus_kernel_padding(
    kernel_size: Sequence[int], stride: int
) -> Tuple[int, ...]:
    ...
...
@overload
def stride_minus_kernel_padding(
    kernel_size: int, stride: Sequence[int]
) -> Tuple[int, ...]:
    ...
...
def stride_minus_kernel_padding(
    ...
    if k.ndim != 1:
        raise ValueError(
            "stride_minus_kernel_padding() 'kernel_size' must be scalar or sequence"
        )
    ndim = tuple(k.shape)[0]
    ...
    assert (
        s.ndim == 1
    ), "stride_minus_kernel_padding() 'stride' must be scalar or sequence"
...
def upsample_padding(
    ...
    """Padding on both sides for transposed convolution."""
    device = str("cpu").replace("cuda", "gpu")
    k: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=kernel_size, dtype="int32", place=device)
    )
...
def upsample_output_padding(
    ...
    """Output padding on one side for transposed convolution."""
    device = str("cpu").replace("cuda", "gpu")
    k: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=kernel_size, dtype="int32", place=device)
    )
    ...
    assert (
        k.ndim == 1
    ), "upsample_output_padding() 'kernel_size' must be scalar or sequence"
    assert (
        s.ndim == 1
    ), "upsample_output_padding() 'scale_factor' must be scalar or sequence"
...
def upsample_output_size(
    ...
    if size is not None and scale_factor is not None:
        raise ValueError(
            "upsample_output_size() 'size' and 'scale_factor' are mutually exclusive"
        )
    device = str("cpu").replace("cuda", "gpu")
    m: paddle.Tensor = paddle.atleast_1d(
        paddle.to_tensor(data=in_size, dtype="int32", place=device)
