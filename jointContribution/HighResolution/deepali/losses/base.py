...
class RegistrationLoss(paddle.nn.Layer, metaclass=ABCMeta):
    """Base class of registration loss functions.
    ...
    the main loop which performs individual gradient steps using an instance of ``paddle.optim.Optimizer``.
    ...
    @staticmethod
    def as_module_dict(
        arg: Optional[RegistrationLosses], start: int = 0
    ) -> paddle.nn.LayerDict:
        """Convert argument to ``ModuleDict``."""
        if arg is None:
    ...
    def forward(self) -> paddle.Tensor:
        ...
        if "loss" not in result:
            raise ValueError(
                f"{type(self).__name__}.eval() result must contain key 'loss'"
            )
        loss = result["loss"]
        if not isinstance(loss, paddle.Tensor):
            raise TypeError(
                f"{type(self).__name__}.eval() result 'loss' must be tensor"
            )
        return loss


class PairwiseImageLoss(paddle.nn.Layer, metaclass=ABCMeta):
    """Base class of pairwise image dissimilarity criteria."""
...
class NormalizedPairwiseImageLoss(PairwiseImageLoss):
    """Base class of pairwise image dissimilarity criteria with implicit input normalization."""
    ...
    def __init__(
        ...
        if norm is None:
            ...
            norm = None
        assert norm is None or isinstance(norm, (float, int, paddle.Tensor))
        self.norm = norm
...
class DisplacementLoss(paddle.nn.Layer, metaclass=ABCMeta):
    """Base class of regularization terms based on dense displacements."""
...
class BSplineLoss(paddle.nn.Layer, metaclass=ABCMeta):
    """Base class of loss terms based on cubic B-spline deformation coefficients."""
    ...
    @abstractmethod
    def forward(
        self, params: paddle.Tensor, stride: ScalarOrTuple[int] = 1
    ) -> paddle.Tensor:
...
class PointSetDistance(paddle.nn.Layer, metaclass=ABCMeta):
    """Base class of point set distance terms."""

    @abstractmethod
    def forward(self, x: paddle.Tensor, *ys: paddle.Tensor) -> paddle.Tensor:
        """Evaluate point set distance.
        ...
        Args:
            x: paddle.Tensor of shape ``(M, X, D)`` with points of (transformed) target point set.
...
class ParamsLoss(paddle.nn.Layer, metaclass=ABCMeta):
    """Regularization loss based on model parameters."""

    @abstractmethod
    def forward(self, params: paddle.Tensor) -> paddle.Tensor:
        """Evaluate loss term for given model parameters."""
        raise NotImplementedError(f"{type(self).__name__}.forward()")
