...
from ..core.image import avg_pool
from ..core.image import dot_channels
from ..core.image import rand_sample
from ..core.image import spatial_derivatives
...
class ElementwiseLoss(Protocol):
    """Type annotation of a eleemntwise loss function."""
...
def label_smoothing(
    ...
    if labels.ndim < 4:
        raise ValueError(
            "label_smoothing() 'labels' must be tensor of shape (N, C, ..., X)"
        )
    if tuple(labels.shape)[1] == 1:
        target = as_one_hot_tensor(
            labels, num_classes, ignore_index=ignore_index, dtype="float32"
        )
    else:
...
def balanced_binary_cross_entropy_with_logits(
    ...
    neg_weight = (
        target.flatten(start_axis=1)
        .mean(axis=-1)
        .reshape((-1,) + (1,) * (target.ndim - 1))
    )
    pos_weight = 1 - neg_weight
    log_y_pred: paddle.Tensor = paddle.nn.functional.log_sigmoid(x=logits)
    ...
    loss = masked_loss(
        loss, weight, "balanced_binary_cross_entropy_with_logits", inplace=True
    )
...
def dice_score(
    ...
    """Soft Dice similarity of multi-channel classification result.
    ...
    denominator = dot_channels(y_pred, y_pred, weight=weight) + dot_channels(
        y, y, weight=weight
    )
    loss = (
...
def dice_loss(
    ...
    """One minus soft Dice similarity of multi-channel classification result.
...
def tversky_index(
    ...
    """Tversky index as described in https://arxiv.org/abs/1706.05721.
    ...
    if target.ndim == input.ndim:
        ...
        if tuple(target.shape)[1] == 1:
            ...
            if tuple(y_pred.shape)[1] == 2:
                start_20 = y_pred.shape[1] + 1 if 1 < 0 else 1
                y_pred = paddle.slice(y_pred, [1], [start_20], [start_20 + 1])
        else:
            ...
            if tuple(y_pred.shape)[1] == 1:
                start_21 = y.shape[1] + 1 if 1 < 0 else 1
                y = paddle.slice(y, [1], [start_21], [start_21 + 1])
        if binarize:
            y = y.round()
    elif target.ndim + 1 == y_pred.ndim:
        if num_classes == 2 and tuple(y_pred.shape)[1] == 1:
            y = (
                target.unsqueeze(axis=1)
                .greater_equal(y=paddle.to_tensor(0.5))
                .astype(y_pred.dtype)
            )
            if binarize:
                y = y.round()
    ...
    if tuple(y.shape) != tuple(y_pred.shape):
        raise ValueError(
            "tversky_index() 'input' and 'target' shapes must be compatible"
        )
    if weight is not None:
        ...
        if weight.ndim != y.ndim:
            raise ValueError(
                "tversky_index() 'weight' shape must be (N, ..., X) or (N, C, ..., X)"
            )
...
def tversky_loss(
    ...
    """Tversky loss as described in https://arxiv.org/abs/1706.05721.
    ...
    if gamma:
        if gamma > 1:
            ...
            raise ValueError(
                "tversky_loss() 'gamma' must be greater than or equal to 1"
            )
    loss = reduce_loss(loss, reduction)
    return loss
...
def focal_loss_with_logits(
    ...
    """
    bce = binary_cross_entropy_with_logits(logit=input, label=target, reduction="none")
...
def kld_loss(
    mean: paddle.Tensor, logvar: paddle.Tensor, reduction: str = "mean"
) -> paddle.Tensor:
    """Kullback-Leibler divergence in case of zero-mean and isotropic unit variance normal prior distribution.
...
def lcc_loss(
    ...
    def local_sum(data: paddle.Tensor) -> paddle.Tensor:
        return avg_pool(
            data, kernel_size=kernel_size, stride=1, padding=None, divisor_override=1
        )
    ...
    loss = (
        ...
        .divide_(
            y=paddle.to_tensor(
                b.multiply_(y=paddle.to_tensor(c)).add_(y=paddle.to_tensor(epsilon))
            )
        )
        .neg_()
...
def wlcc_loss(
    ...
    def local_mean(
        data: paddle.Tensor, weight: Optional[paddle.Tensor] = None
    ) -> paddle.Tensor:
        if weight is None:
            return avg_pool(
                data,
...
def smooth_l1_loss(
    ...
    """Normalized masked smooth L1 loss.
...
def mi_loss(
    ...
    if mask is not None:
        if (
            mask.ndim < 3
            or tuple(mask.shape)[2:] != shape[2:]
            or tuple(mask.shape)[1] != 1
        ):
            raise ValueError(
    ...
    if sample_ratio is not None:
        if num_samples is not None:
            raise ValueError(
                "mi_loss() 'num_samples' and 'sample_ratio' are mutually exclusive"
            )
        if sample_ratio <= 0 or sample_ratio > 1:
            raise ValueError(
                "mi_loss() 'sample_ratio' must be in open-closed interval (0, 1]"
            )
        num_samples = max(1, int(sample_ratio * tuple(target.shape)[2:].size))
    if num_samples is not None:
        input, target = rand_sample(
            [input, target], num_samples, mask=mask, replacement=True
        )
    elif mask is not None:
        input = input.mul(mask)
        target = target.mul(mask)
    bin_width = (vmax - vmin) / num_bins
    out_3 = paddle.linspace(start=vmin, stop=vmax, num=num_bins)
    ...
    perm_11[1] = 2
    perm_11[2] = 1
    hist_joint = pw_input.bmm(y=x.transpose(perm=perm_11))
    hist_norm = hist_joint.flatten(start_axis=1, stop_axis=-1).sum(axis=1) + 1e-05
    p_joint = hist_joint / hist_norm.view(-1, 1, 1)
    p_input = paddle.sum(x=p_joint, axis=2)
    p_target = paddle.sum(x=p_joint, axis=1)
...
def grad_loss(
    ...
    """Loss term based on p-norm of spatial gradient of vector fields.
    ...
    if u.ndim - 2 != D:
        raise ValueError(
            f"grad_loss() 'u' must be tensor of shape (N, {u.ndim - 2}, ..., X)"
        )
    if q is None:
        q = 1.0 / p
    derivs = spatial_derivatives(
        u, mode=mode, which=which, order=1, sigma=sigma, spacing=spacing
    )
    loss = paddle.concat(
        x=[deriv.unsqueeze(axis=-1) for deriv in derivs.values()], axis=-1
    )
    if p == 1:
        loss = loss.abs()
    elif p != 0:
...
def bending_loss(
    ...
    if u.ndim - 2 != D:
        raise ValueError(
            f"bending_energy() 'u' must be tensor of shape (N, {u.ndim - 2}, ..., X)"
        )
    which = SpatialDerivativeKeys.unique(SpatialDerivativeKeys.all(ndim=D, order=2))
    derivs = spatial_derivatives(
        u, mode=mode, which=which, sigma=sigma, spacing=spacing
    )
    derivs = paddle.concat(
        x=[deriv.unsqueeze(axis=-1) for deriv in derivs.values()], axis=-1
    )
    derivs *= paddle.to_tensor(
        data=[(2 if SpatialDerivativeKeys.is_mixed(key) else 1) for key in which],
        place=u.place,
    )
    loss = derivs.pow(y=2).sum(axis=-1)
    loss = reduce_loss(loss, reduction)
    return loss
...
def bspline_bending_loss(
    ...
    """Evaluate bending energy of cubic B-spline function, e.g., spatial free-form deformation.
    ...
    if C != D:
        raise ValueError(
            f"bspline_bending_loss() 'data' number of channels ({C}) does not match number of spatial dimensions ({D})"
        )
    ...
    for deriv in derivs:
        ...
        values = evaluate_cubic_bspline(
            data, stride=stride, derivative=derivative
        ).square()
        if reduction != "none":
            npoints = len(tuple(values.shape)[2:])
            values = values.sum()
        if not SpatialDerivativeKeys.is_mixed(deriv):
            values = values.multiply_(y=paddle.to_tensor(2))
        energy = values if energy is None else energy.add_(y=paddle.to_tensor(values))
    assert energy is not None
    assert npoints > 0
    if reduction == "mean" and npoints > 1:
        energy = energy.divide_(y=paddle.to_tensor(npoints))
    return energy
...
def curvature_loss(
    ...
    """Loss term based on unmixed 2nd order spatial derivatives of vector fields.
    ...
    if u.ndim - 2 != D:
        raise ValueError(
            f"curvature_loss() 'u' must be tensor of shape (N, {u.ndim - 2}, ..., X)"
        )
    ...
    loss = 0.5 * derivs.sum(axis=-1).pow(y=2)
    loss = reduce_loss(loss, reduction)
    return loss
...
def diffusion_loss(
    u: paddle.Tensor,
    spacing: Optional[paddle.Tensor] = None,
    sigma: Optional[float] = None,
    mode: str = "central",
    reduction: str = "mean",
) -> paddle.Tensor:
    """Diffusion regularization loss."""
    loss = grad_loss(
        u, p=2, q=1, spacing=spacing, sigma=sigma, mode=mode, reduction=reduction
    )
...
def divergence_loss(
    ...
    """Loss term encouraging divergence-free vector fields."""
    if u.ndim < 4:
        if reduction == "none":
            raise NotImplementedError(
    ...
    if u.ndim - 2 != D:
        raise ValueError(
            f"divergence_loss() 'u' must be tensor of shape (N, {u.ndim - 2}, ..., X)"
        )
    derivs = spatial_derivatives(u, mode=mode, order=1, sigma=sigma, spacing=spacing)
    derivs = paddle.concat(
        x=[deriv.unsqueeze(axis=-1) for deriv in derivs.values()], axis=-1
    )
    loss = derivs.sum(axis=-1)
    loss = loss.abs_() if q < 2 else loss.pow_(y=q)
    loss = reduce_loss(loss, reduction)
    return loss
...
def lame_parameters(
    ...
    """Get Lame parameters of linear elasticity given different quantities.
    ...
    if material_name:
        ...
        if material_name == "rubber":
            ...
            raise ValueError(
                f"lame_parameters() unknown 'material_name': {material_name}"
            )
    elif len(kwargs) != 2:
        raise ValueError(
    ...
    if first_parameter is None:
        if shear_modulus is None:
            ...
            if poissons_ratio is None:
                poissons_ratio = RUBBER_POISSONS_RATIO
            first_parameter = (
                2 * shear_modulus * poissons_ratio / (1 - 2 * poissons_ratio)
            )
        else:
            first_parameter = (
                shear_modulus
        ...
        if youngs_modulus is None:
            if poissons_ratio is None:
                poissons_ratio = RUBBER_POISSONS_RATIO
            second_parameter = (
                first_parameter * (1 - 2 * poissons_ratio) / (2 * poissons_ratio)
            )
        else:
            r = math.sqrt(
                youngs_modulus**2
...
def elasticity_loss(
    ...
    """Loss term based on Navier-Cauchy PDE of linear elasticity.
    ...
    if u.ndim - 2 != D:
        raise ValueError(
            f"elasticity_loss() 'u' must be tensor of shape (N, {u.ndim - 2}, ..., X)"
        )
    derivs = spatial_derivatives(u, mode=mode, order=1, sigma=sigma, spacing=spacing)
    derivs = [derivs[str(SpatialDim(i))] for i in range(D)]
    start_22 = derivs[0].shape[1] + 0 if 0 < 0 else 0
    loss = paddle.slice(derivs[0], [1], [start_22], [start_22 + 1]).clone()
    for i in range(1, D):
        start_23 = derivs[i].shape[1] + i if i < 0 else i
        loss = loss.add_(
            y=paddle.to_tensor(paddle.slice(derivs[i], [1], [start_23], [start_23 + 1]))
        )
    loss = paddle.square_(loss).multiply_(y=paddle.to_tensor(lambd / 2))
    for j, k in itertools.product(range(D), repeat=2):
        start_24 = derivs[j].shape[1] + k if k < 0 else k
        start_25 = derivs[k].shape[1] + j if j < 0 else j
        temp = paddle.slice(derivs[j], [1], [start_24], [start_24 + 1]).add(
            paddle.slice(derivs[k], [1], [start_25], [start_25 + 1])
        )
        loss = loss.add_(
            y=paddle.to_tensor(
                paddle.square_(temp).multiply_(y=paddle.to_tensor(mu / 4))
            )
        )
    loss = reduce_loss(loss, reduction)
    return loss
...
def total_variation_loss(
    ...
    return grad_loss(
        u, p=1, q=1, spacing=spacing, sigma=sigma, mode=mode, reduction=reduction
    )
...
def inverse_consistency_loss(
    ...
    Args:
        forward: paddle.Tensor representation of spatial transformation.
        inverse: paddle.Tensor representation of inverse transformation.
    ...
    if mask is not None:
        ...
        if tuple(mask.shape)[1] != 1:
            raise ValueError(
                "inverse_consistency_loss() 'mask' must have shape (N, 1, ..., X)"
            )
    ...
    if margin > 0:
        ...
        subgrid = tuple(
            reversed([slice(i, n - i) for i, n in zip(m, tuple(grid.shape))])
        )
        error = error[
            (slice(0, tuple(error.shape)[0]),) + subgrid + (slice(0, grid.ndim),)
        ]
    if units in ("voxel", "world"):
        error = denormalize_flow(error, size=tuple(grid.shape), channels_last=True)
        if units == "world":
            error *= grid.spacing().to(error)
    error: paddle.Tensor = error.norm(p=2, axis=-1)
    if reduction != "none":
        count = error.size
        error = error.sum()
...
def elementwise_loss(
    ...
    input: paddle.paddle.Tensor,
    target: paddle.Tensor,
    mask: Optional[paddle.Tensor] = None,
    norm: Optional[Union[float, paddle.Tensor]] = None,
    reduction: str = "mean",
) -> paddle.Tensor:
    """Evaluate, aggregate, and normalize elementwise loss, optionally within masked region.
...
def masked_loss(
    ...
    """Multiply loss with an optionally specified spatial mask."""
    if mask is None:
        return loss
    if not name:
        name = "masked_loss"
    ...
    if tuple(mask.shape)[0] != 1 and tuple(mask.shape)[0] != tuple(loss.shape)[0]:
        raise ValueError(
            f"{name}() 'mask' must have same batch size as 'target' or batch size 1"
        )
    if tuple(mask.shape)[1] != 1 and tuple(mask.shape)[1] != tuple(loss.shape)[1]:
        raise ValueError(
            f"{name}() 'mask' must have same number of channels as 'target' or only 1"
        )
    ...
    if inplace:
        ...
        loss = loss.mul(mask.astype(loss.dtype))
    return loss
...
def reduce_loss(
    ...
    """Reduce loss computed at each grid point."""
    ...
    if reduction == "mean":
        ...
        value = value.divide_(y=paddle.to_tensor(numel, dtype=value.dtype))
    return value
