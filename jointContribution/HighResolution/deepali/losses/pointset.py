...
class ClosestPointDistance(PointSetDistance):
    ...
    def __init__(self, scale: float = 10, split_size: int = 100000.0):
        """Initialize closest point distance loss.
    ...
    def forward(self, x: paddle.Tensor, *ys: paddle.Tensor) -> paddle.Tensor:
        ...
        if not ys:
            raise ValueError(
                f"{type(self).__name__}.forward() requires at least two point sets"
            )
        ...
        for y in ys:
            ...
            dists: paddle.Tensor = paddle.linalg.norm(x=x - y, p=2, axis=2)
            loss += dists.mean()
...
class LandmarkPointDistance(PointSetDistance):
    """Average distance between corresponding landmarks."""

    def __init__(self, scale: float = 10):
        """Initialize point distance loss.
