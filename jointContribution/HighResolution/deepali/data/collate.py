...
def collate_samples(batch: Sequence[Sample]) -> Batch:
    ...
    for name in names:
        ...
        if is_none:
            ...
            data = paddle.concat(
                x=[flow_field.tensor() for flow_field in flow_fields], axis=0
            )
            grid = tuple(
                grid for flow_field in flow_fields for grid in flow_field.grids()
            )
            values.append(FlowFieldsType(data, grid, elem0.axes()))
            ...
            isinstance(elem0, abc.Mapping)
            or is_dataclass(elem0)
            or is_namedtuple(elem0)
        ):
            values.append(collate_samples(samples))
        else:
            values.append(
                paddle.io.dataloader.collate.default_collate_fn(batch=samples)
            )
