...
from ..core.grid import grid_transform_vectors
...
class FlowFields(ImageBatch):
    ...
    def _make_subitem(self, data: paddle.Tensor, grid: Grid) -> Union[FlowField, Image]:
        """Create FlowField in __getitem__. Can be overridden by subclasses to return a subtype."""
    ...
    def _paddle_function_axes(args) -> Optional[Axes]:
        """Get flow field Axes from args passed to __paddle_function__."""
        if not args:
            return None
        ...
        axes: Sequence[Axes]
        axes = [
            ax for ax in (getattr(arg, "_axes", None) for arg in args) if ax is not None
        ]
        if not axes:
            return None
    ...
    def __paddle_function__(cls, func, types, args=(), kwargs=None):
        ...
        if func in (
            ...
            return tuple(
                cls._paddle_function_result(func, res, grid, axes) for res in data
            )
    ...
    @overload
    def __getitem__(
        self: TFlowFields, index: Union[list, slice, paddle.Tensor]
    ) -> TFlowFields:
        ...
    ...
    def __getitem__(
        ...
        """Get flow field at specified batch index, get a sub-batch, or extract region of interest tensor."""
        return super().__getitem__(index)

    @overload
    def axes(self: TFlowFields) -> Axes:
        """Get axes with respect to which flow vectors are defined."""
        ...

    @overload
    def axes(self: TFlowFields, axes: Axes) -> TFlowFields:
        """Get new batch of flow fields with flow vectors defined with respect to specified axes."""
        ...

    def axes(
        self: TFlowFields, axes: Optional[Axes] = None
    ) -> Union[Axes, TFlowFields]:
        """Rescale and reorient vectors."""
        if axes is None:
            return self._axes
        data = self.tensor()
    ...
    def curl(self: TFlowFields, mode: str = "central") -> ImageBatch:
        if self.ndim not in (2, 3):
            raise RuntimeError(
                "Cannot compute curl of {self.ndim}-dimensional flow field"
            )
    ...
    def sample(
        ...
        """Sample flow fields at optionally deformed unit grid points.
        ...
        Returns:
            ...
            Otherwise, a ``paddle.Tensor`` of shape (N, C, ...) of sampled flow values is returned.
            ...
            flow vectors with respect to the new sampling grids. If this is not desired, use a ``paddle.Tensor``
        ...
        if isinstance(flow, FlowFields):
            ...
            if axes != Axes.WORLD:
                ...
                data = paddle.concat(
                    x=[
                        grid_transform_vectors(v, grid, axes, to_grid, axes).unsqueeze_(
                            0
                        )
    ...
    def warp_image(
        ...
        """Deform given image (batch) using this batch of vector fields.
        ...
        grid = (
            g.coords(align_corners=align_corners, device=self.device)
            for g in self._grid
        )
...
class FlowField(Image):
    ...
    @classmethod
    def _paddle_function_result(
        cls, func, data, grid: Optional[Grid], axes: Optional[Axes]
    ) -> Any:
        if not isinstance(data, paddle.Tensor):
            return data
        if (
    ...
    @classmethod
    def from_image(
        cls: Type[TFlowField], image: Image, axes: Optional[Axes] = None
    ) -> TFlowField:
        """Create flow field from image instance."""
        return cls(image, image._grid, axes)

    def batch(self: TFlowField) -> FlowFields:
        """Batch of flow fields containing only this flow field."""
    ...
    def axes(self: TFlowField, axes: Axes) -> TFlowField:
        """Get new flow field with flow vectors defined with respect to specified axes."""
        ...

    def axes(self: TFlowField, axes: Optional[Axes] = None) -> Union[Axes, TFlowField]:
        """Rescale and reorient vectors with respect to specified axes."""
        if axes is None:
            return self._axes
        batch = self.batch()
    ...
    def from_sitk(
        ...
        """Create vector field from ``SimpleITK.Image``."""
    ...
    def sitk(self: TFlowField, axes: Optional[Axes] = None) -> "sitk.Image":
        """Create ``SimpleITK.Image`` from this vector field."""
    ...
    @classmethod
    def read(
        cls: Type[TFlowField], path: PathStr, axes: Optional[Axes] = None
    ) -> TFlowField:
    ...
    def exp(
        ...
        """Group exponential map of vector field computed using scaling and squaring."""
        batch = self.batch()
    ...
    def warp_image(
        ...
        """Deform given image using this displacement field."""
        ...
    ...
    def warp_image(
        ...
        """Deform images in batch using this displacement field."""
        ...
    ...
    def warp_image(
        ...
        """Deform given image (batch) using this displacement field.
