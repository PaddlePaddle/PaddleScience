"""Functions for dealing with dataset sample collections."""
...
def sample_field_names(sample: Sample) -> Tuple[str]:
    """Get names of fields in data sample."""
...
def sample_field_value(sample: Sample, name: str) -> Any:
    """Get sample value of named data field."""
...
def replace_all_sample_field_values(sample: Sample, values: Sequence[Any]) -> Sample:
    """Replace all sample field values."""
    names = sample_field_names(sample)
    if len(names) != len(values):
        raise ValueError(
            "replace_all_values() 'values' must contain an entry for every field"
        )
    if is_dataclass(sample):
