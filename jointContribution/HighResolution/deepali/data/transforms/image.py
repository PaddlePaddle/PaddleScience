...
class CastImage(ItemwiseTransform, paddle.nn.Layer):
    ...
    def __init__(self, dtype: Union[paddle.dtype, str]) -> None:
        super().__init__()
        if isinstance(dtype, str):
            ...
            raise TypeError(
                f"{type(self).__name__}() 'dtype' must by dtype name or paddle.dtype"
            )
        self.dtype = dtype
...
class ClampImage(ItemwiseTransform, paddle.nn.Layer):
    ...
    def __repr__(self) -> str:
        return (
            type(self).__name__
            + f"(min={self.min!r}, max={self.max!r}, inplace={self.inplace!r})"
        )
...
class NarrowImage(ItemwiseTransform, paddle.nn.Layer):
    ...
    def forward(self, image: Image) -> paddle.Tensor:
        if not isinstance(image, Image):
            raise TypeError(f"{type(self).__name__}.forward() argument must be Image")
        start_26 = image.shape[self.dim] + self.start if self.start < 0 else self.start
        image = paddle.slice(image, [self.dim], [start_26], [start_26 + self.length])
        return image

    def __repr__(self) -> str:
        return (
            type(self).__name__
            + f"(dim={self.dim}, start={self.start}, length={self.length})"
        )
...
class NormalizeImage(ItemwiseTransform, paddle.nn.Layer):
    ...
    def __init__(
        ...
        if mode not in ("center", "unit", "zscore", "z-score"):
            raise ValueError(
                "NormalizeImage() 'mode' must be 'center', 'unit', or 'zscore'"
            )
        self.min = min
        self.max = max
        self.mode = mode
...
class ReadImage(ItemwiseTransform, paddle.nn.Layer):
    ...
    def __init__(
        self,
        dtype: Optional[Union[paddle.dtype, str]] = None,
...
class ResampleImage(ItemwiseTransform, paddle.nn.Layer):
    ...
    def __repr__(self) -> str:
        return (
            type(self).__name__
            + f"(spacing={self.spacing!r}, mode={self.mode.value!r})"
        )
...
def config_has_read_image_transform(config: ImageTransformConfig) -> bool:
    """Whether image data transformation configuration contains a "read" image transform."""
    ...
    if isinstance(config, Sequence):
        ...
        return False
    raise TypeError(
        "config_has_read_image_transform() 'config' must be str, Mapping, or Sequence"
    )
...
def prepend_read_image_transform(
    ...
    """Insert a "read" image transform before any other image data transform."""
    ...
    raise TypeError(
        "prepend_read_image_transform() 'config' must be str, Mapping, or Sequence"
    )
...
def image_transform(
    ...
    """Create image data transform given its name."""
