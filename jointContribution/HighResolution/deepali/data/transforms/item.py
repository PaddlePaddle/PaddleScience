...
from ...core.types import RE_OUTPUT_KEY_INDEX
...
class ItemTransform(paddle.nn.Layer):
    ...
    def forward(self, data: Any) -> Any:
        ...
        if isinstance(self.key, set):
            keys = self.key
        elif not isinstance(self.key, str) and isinstance(
            self.key, (KeysView, Iterable)
        ):
            keys = set(self.key)
        else:
            keys = [self.key]
        for key in keys:
            if isinstance(key, int):
                ...
                raise TypeError(
                    f"{type(self).__name__}() 'key' must be None, int, or str"
                )
        return data

    def _apply_all(self, data: Any) -> Any:
        """Transform all leaf items."""
        if is_dataclass(data):
            ...
            if self.ignore_missing:
                ...
                raise ValueError(
                    f"{type(self).__name__}() value is None (ignore_missing=False)"
                )
        else:
    ...
    def _apply_index(self, data: Any, index: int) -> Any:
        """Transform item at specified index."""
        if not isinstance(data, (list, tuple)):
            raise TypeError(
                f"{type(self).__name__}() 'data' must be list or tuple when key is int"
            )
        try:
            item = data[index]
        except IndexError:
    ...
    def _apply_key(self, data: Any, key: str, prefix: str = "") -> Any:
        """Transform specified item in data map."""
        parts = key.split(".", 1)
        if len(parts) == 1:
            parts = [parts[0], ""]
        key, subkey = parts
        ...
        if is_dataclass(data) or is_namedtuple(data):
            ...
            try:
                ...
                if prefix:
                    ...
                    msg = (
                        f"{type(self).__name__}() 'data' index {index} is out of bounds"
                    )
            ...
            try:
                ...
                try:
                    ...
                    if prefix:
                        msg = f"{type(self).__name__}() 'data' dict {prefix!r} must have key {key!r}"
                    else:
                        msg = (
                            f"{type(self).__name__}() 'data' dict must have key {key!r}"
                        )
                    raise KeyError(msg)
        else:
            if prefix:
            ...
            raise TypeError(msg)
        if subkey:
            item = self._apply_key(
                item, subkey, prefix=prefix + "." + key if prefix else key
            )
        else:
        ...
        if is_dataclass(data):
            ...
            data = tuple(
                item if i == index else self._maybe_copy(v) for i, v in enumerate(data)
            )
        elif isinstance(data, (list, tuple)):
            assert index is not None
            data = list(
            ...
            data = {
                k: (item if k == key else self._maybe_copy(v)) for k, v in data.items()
            }
        return data
    ...
    def __repr__(self) -> str:
        return (
            type(self).__name__
            + f"({self.transform!r}, key={self.key!r}, copy={self.copy!r})"
        )
