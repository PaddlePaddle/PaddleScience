...
import paddle
from paddle.io import _T
from paddle.io import Sampler


class DistributedWeightedRandomSampler(Sampler[_T]):
    """A version of WeightedRandomSampler that can be used with DistributedDataParallel training."""
    ...
    def __init__(
        ...
        """Initialize map-style dataset index sampler.
    ...
    def set_epoch(self, epoch: int) -> None:
        """Set training epoch used to adjust seed of number generator when shuffling is enabled."""
        if epoch < 0:
            raise ValueError(
                f"{type(self).__name__}.set_epoch() 'epoch' must be non-negative"
            )
    ...
    def __iter__(self) -> Iterator[paddle.utils.data.sampler.T_co]:
