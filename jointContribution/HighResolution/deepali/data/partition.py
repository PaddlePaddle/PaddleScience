...
class Partition(Enum):
    """Enumeration of dataset partitions / splits."""
...
def dataset_split_lengths(
    ...
    """Split dataset in training, validation, and test subset.

    The output ``lengths`` of this function can be passed to ``paddle.utils.data.random_split`` to obtain
    the ``paddle.utils.data.dataset.Subset`` for each split.
    ...
    if ratios[0] <= 0 or ratios[0] > 1:
        raise ValueError(
            "dataset_split_lengths() training split ratio must be in (0, 1]"
        )
...
def random_split_indices(
    lengths: Sequence[int], generator: Optional[paddle.Generator] = None
) -> List[List[int]]:
    """Randomly split dataset indices into non-overlapping sets of given lengths.
