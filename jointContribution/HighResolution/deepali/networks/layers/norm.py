...
NormFunc = Callable[[paddle.Tensor], paddle.Tensor]
    ...
    if isinstance(arg, str):
        ...
        if not isinstance(norm_args, dict):
            if norm_name == "group" and isinstance(norm_args, Integral):
                ...
                raise TypeError(
                    "normalization() second 'arg' sequence argument must be dict"
                )
        norm_args = norm_args.copy()
    elif arg is not None:
        raise TypeError(
            "normalization() 'arg' must be str, mapping, 2-tuple, callable, or None"
        )
    ...
    if norm_name in ("none", "identity"):
        ...
        if num_features is None:
            if "num_channels" not in norm_args:
                raise ValueError(
                    "normalization() 'num_features' required for 'group' norm"
                )
        ...
        if spatial_dims is None:
            raise ValueError(
                "normalization() 'spatial_dims' required for 'instance' norm"
            )
        if spatial_dims < 0 or spatial_dims > 3:
            raise ValueError("normalization() 'spatial_dims' must be 1, 2, or 3")
        if num_features is None:
            raise ValueError(
                "normalization() 'num_features' required for 'instance' norm"
            )
        norm_type = (
...
def norm_layer(*args, **kwargs) -> paddle.nn.Layer:
    """Create normalization layer, see ``normalization``."""
    return normalization(*args, **kwargs)


def is_norm_layer(arg: Any) -> bool:
    """Whether given module is a normalization layer."""
...
def is_batch_norm(arg: Any) -> bool:
    """Whether given module is a batch normalization layer."""
    return isinstance(
        arg, (paddle.nn.BatchNorm1D, paddle.nn.BatchNorm2D, paddle.nn.BatchNorm3D)
    )


def is_group_norm(arg: Any) -> bool:
    """Whether given module is a group normalization layer."""
    return isinstance(arg, paddle.nn.GroupNorm)


def is_instance_norm(arg: Any) -> bool:
    """Whether given module is an instance normalization layer."""
    return isinstance(
        arg,
