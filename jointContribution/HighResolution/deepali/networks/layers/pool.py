...
PoolFunc = Callable[[paddle.Tensor], paddle.Tensor]
...
def pooling(
    ...
    if isinstance(arg, str):
        ...
        raise TypeError(
            "pooling() 'arg' must be str, mapping, 2-tuple, callable, or None"
        )
    pool_type: Union[
        Type[paddle.nn.Layer], Sequence[Type[paddle.nn.Layer]]
    ] = POOLING_TYPES.get(pool_name.lower())
    if pool_type is None:
        raise ValueError(f"pooling() unknown pooling layer {pool_name!r}")
    if isinstance(pool_type, Sequence):
        if spatial_dims is None:
            raise ValueError(
                f"pooling() 'spatial_dims' required for pooling layer {pool_name!r}"
            )
        try:
        ...
        if pool_type is None:
            raise ValueError(
                f"pooling({pool_name!r}) does not support spatial_dims={spatial_dims}"
            )
