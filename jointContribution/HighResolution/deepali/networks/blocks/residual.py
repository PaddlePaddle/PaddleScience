...
class ResidualUnit(SkipConnection):
    ...
    def __init__(
        ...
        skip: Optional[
            Union[SkipFunc, str, Mapping[str, Any]]
        ] = "identity | conv1 | conv",
        ...
        for i in range(
            1 if is_bottleneck else 0, num_layers - 1 if is_bottleneck else num_layers
        ):
            name = f"layer_{len(residual)}"
            is_first_layer = i == 0
        ...
        if is_bottleneck:
            ...
            residual.add_sublayer(name=name, sublayer=conv)
        has_strided_conv = (
            not paddle.to_tensor(data=stride, dtype="int32", place="cpu")
            .prod()
            .equal(y=1)
        )
        if skip in (None, ""):
        ...
        if isinstance(skip, str):
            ...
            if has_strided_conv or in_channels != out_channels:
                if "conv" in skip_names and (
                    has_strided_conv or "conv1" not in skip_names
                ):
                    skip = {}
        ...
        if isinstance(skip, dict):
            if "padding" in skip and "kernel_size" not in skip:
                raise ValueError(
                    "ResidualUnit() 'skip' specifies 'padding' but not 'kernel_size'"
                )
            conv_args = dict(
            ...
            if "padding" not in conv_args:
                conv_args["padding"] = same_padding(
                    conv_args["kernel_size"], conv_args["dilation"]
                )
            skip = convolution(spatial_dims, in_channels, out_channels, **conv_args)
        elif not callable(skip):
            raise TypeError(
                "ResidualUnit() 'skip' must be str, dict, callable, or None"
            )
        add = JoinLayer("add")
    ...
    def is_bottleneck(self) -> bool:
        """Whether this residual unit is a "bottleneck" type ResNet block."""
        return self.out_channels != self.num_channels

    @property
    def last_conv_layer(self) -> ConvLayer:
        """Get last residual convolutional layer."""
        return self.func[-1]

    def zero_init_residual(self) -> None:
        """Zero-initialize normalization layer in residual branch.
