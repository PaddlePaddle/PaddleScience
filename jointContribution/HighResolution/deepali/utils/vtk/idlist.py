"""Common auxiliary functions for working with VTK data structures."""
...
def iter_id_list(
    ids: vtkIdList, start: int = 0, stop: int = None
) -> Generator[int, None, None]:
    """Iterate over IDs in vtkIdList."""
    if stop is None:
...
def iter_cell_point_ids(
    ...
    """Iterate over IDs of points that a given vtkPolyData cell is made up of."""
    point_ids = vtkIdList()
...
def iter_point_cell_ids(
    ...
    """Iterate over IDs of vtkPolyData cells that contain a specified point."""
    cell_ids = vtkIdList()
