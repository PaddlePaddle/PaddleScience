...
def apply_warp_field_to_points(
    ...
    """Transform vtkPoints by linearly interpolated dense vector field.
...
def apply_warp_field_to_pointset(
    ...
    """Transform vtkPoints of vtkPointSet by linearly interpolated dense displacement field.
...
def image_data_grid(data: vtkImageData) -> Grid:
    """Create image grid from vtkImageData object."""
...
def vtk_image_from_sitk_image(
    ...
    """Create vtkImageData from SimpleITK image.
...
def sitk_image_from_vtk_image(
    image: vtkImageData, grid: Optional[Grid] = None
) -> sitk.Image:
    """Create SimpleITK image from vtkImageData."""
    if image.GetNumberOfScalarComponents() != 1:
        raise NotImplementedError(
            "sitk_image_from_vtk_image() only supports scalar 'image'"
        )
