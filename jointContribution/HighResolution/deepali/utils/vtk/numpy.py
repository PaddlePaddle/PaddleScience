...
def numpy_to_vtk_cell_array(arr: np.ndarray) -> vtkCellArray:
    ...
    if arr.dtype not in (np.dtype("int32"), np.dtype("int64")):
        raise TypeError(
            "numpy_to_vtk_cell_array() 'arr' must have dtype int32 or int64"
        )
    cells = vtkCellArray()
    use_set_data = False
    if use_set_data:
        dtype = arr.dtype
        offsets = np.repeat(
            np.array(tuple(arr.shape)[1], dtype=dtype), tuple(arr.shape)[0]
        )
        ...
        cells.AllocateExact(tuple(arr.shape)[0], np.prod(tuple(arr.shape)))
        for cell in arr:
...
def vtk_to_numpy_array(data: vtkDataArray) -> np.ndarray:
    """Convert vtkDataArray to NumPy array."""
