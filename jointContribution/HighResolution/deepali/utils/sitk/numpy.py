...
def image_dtype(image: sitk.Image) -> np.dtype:
    """Get NumPy data type of SimpleITK image."""
...
def image_from_array(
    ...
    """Create image from sampling grid and image data array."""
    components = 1
    ndim = 0
    if origin:
        ndim = len(origin)
    ...
    if ndim:
        if values.ndim < ndim:
            values = values.reshape((1,) * (ndim - values.ndim) + tuple(values.shape))
        elif values.ndim > ndim:
            values = values.reshape(tuple(values.shape)[:ndim] + (-1,))
            components = tuple(values.shape)[-1]
...
def array_from_image(image: sitk.Image, view: bool = False) -> np.ndarray:
    """Get NumPy array with copy of image values."""
