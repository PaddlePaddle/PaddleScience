"""Representation of objects stored in AWS Simple Storage Service (S3)."""
...
from ..resource import PathStr
from ..resource import Resource
from .client import S3Client


class S3Object(Resource):
    """Object stored in AWS Simple Storage Service (S3)."""
    ...
    def normkey(key: str) -> str:
        """Normalize S3 object key string."""
    ...
    def default_path(bucket: str, key: str) -> Path:
        """Get default local path of S3 object copy.
    ...
    def reset_path(self) -> S3Object:
        """Reset ``path`` to ``default_path`` for given ``bucket`` and ``key``.
    ...
    @property
    def uri(self) -> str:
        """URI of storage object."""
        return "s3://" + self.bucket + "/" + self.key

    @property
    def bucket(self) -> str:
        """Name of S3 bucket containing storage object."""
        return self._bucket

    @property
    def key(self) -> str:
        """Name of S3 key corresponding to storage object referenced by this instance."""
        return self._key
    ...
    def with_bucket(self, bucket: str) -> S3Object:
        """Create copy of storage object reference with modified ``bucket``.
    ...
    def with_key(self, key: str) -> S3Object:
        """Create copy of storage object reference with modified ``key``.
    ...
    def pull(self, force: bool = False) -> S3Object:
        """Download content from AWS S3 to local path.
    ...
    def push(self, force: bool = False) -> S3Object:
        """Upload content of local path to AWS S3.
    ...
    def read_bytes(self) -> bytes:
        """Read file content from local path if it exists, or corresponding S3 object otherwise.
    ...
    def write_bytes(self, data: bytes) -> S3Object:
        """Write bytes directly to storage object.
    ...
    def read_text(self, encoding: Optional[str] = None) -> str:
        """Read text file content from local path if it exists, or corresponding S3 object otherwise.
    ...
    def rmdir(self) -> S3Object:
        """Remove S3 objects both locally and from cloud storage."""
    ...
    def unlink(self) -> S3Object:
        """Remove S3 object both locally and from cloud storage."""
        assert not self.key.endswith(
            "/"
        ), "S3 object key must not end with '/' character"
        super().unlink()
