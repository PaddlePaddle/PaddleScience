"""Client for AWS Simple Storage Service (S3)."""
...
def match_all(key: str) -> bool:
    """Default 'match' function used by 'S3Client.download_files()'."""
    return True
...
class S3Client(object):
    ...
    class Operation(Enum):
        """Enumeration of permissible S3 operations.
    ...
    def init_default(cls, *args, **kwargs) -> Generator[S3Client, None, None]:
        """Set new default client to use in current context.
    ...
    def from_arg(cls, arg: Union[S3Config, S3Client, Dict[str, Any], None]) -> S3Client:
        """Get client instance given function argument.
    ...
    @property
    def exceptions(self):
        """Get boto3 exceptions for connected client."""
    ...
    def ops(self) -> Set[S3Client.Operation]:
        """Get list of permissible client operations."""
    ...
    def close(self) -> None:
        """Close connection with AWS S3."""
        self._client = None

    def is_closed(self) -> bool:
        """Check if S3 connection is closed."""
    ...
    def __enter__(self) -> S3Client:
        """Ensure client connection is open when entering context.
    ...
    def exists(self, bucket: str, key: str) -> bool:
        """Check if specified S3 object exists.
    ...
    def keys(
        ...
        """List S3 objects with specified key prefix.
    ...
    def iterdir(
        ...
        """List S3 objects with specified key prefix, excluding subfolder contents.
    ...
    def listdir(
        ...
        """List names of S3 objects whose keys match a given prefix, excluding subfolder contents.
    ...
    def write_text(
        self, bucket: str, key: str, text: str, encoding: Optional[str] = None
    ) -> None:
    ...
    def download_file(
        self, bucket: str, key: str, path: PathStr, overwrite: bool = True
    ) -> None:
        ...
        if not overwrite and path.is_file():
            raise FileExistsError(
                "Use overwrite=True to force overwriting existing local file '{}'".format(
                    path
                )
            )
        data = self.read_bytes(bucket, key)
    ...
    def upload_file(
        self, path: PathStr, bucket: str, key: str, overwrite: bool = True
    ) -> None:
    ...
    def upload_files(
        ...
        for child in path.iterdir():
            ...
            if child.is_dir():
                ...
                try:
                    self.upload_file(
                        path=child, bucket=bucket, key=key, overwrite=overwrite
                    )
                    count += 1
                except FileExistsError:
                    pass
        ...
        while True:
            ...
            count += len(
                self._client.delete_objects(
                    Bucket=bucket, Delete=dict(Objects=resp["Contents"])
                )["Deleted"]
            )
            try:
