...
from ...core import TensorCollection
...
def get_output_transform(key: str) -> Callable[[TensorCollection], paddle.Tensor]:
    """Get tensor at specified nested engine output map key entry."""
...
def cm_binary_output_transform(
    ...
    """Convert engine output to a vector-valued one-hot prediction.
    ...
    Returns:
        y_pred: paddle.Tensor of one-hot encoded predictions with shape ``(N, 2, ..., X)``.
        y: paddle.Tensor of target labels with shape ``(N, ..., X)``.
    ...
    def output_transform(
        ...
        if tuple(y_pred_tensor.shape)[2:] != grid_shape and reshape == "y_pred":
            y_pred_tensor = grid_reshape(
                y_pred_tensor, grid_shape, align_corners=align_corners
            )
...
def cm_multilabel_output_transform(
    ...
    Returns:
        y_pred: paddle.Tensor of multi-class probabilites with shape ``(N, C, ..., X)``.
        y: paddle.Tensor of target labels with shape ``(N, ..., X)``.
...
def cm_output_transform(
    ...
    """Get engine output transformation for binary or multi-class segmentation output, respectively."""
    if multilabel:
...
def y_pred_y_output_transform(
    ...
    """Get engine output transformation which returns (y_pred, y) tuples.
