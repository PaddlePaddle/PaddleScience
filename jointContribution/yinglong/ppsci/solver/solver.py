...
class Solver:
    ...
    def eval(self, epoch_id: int = 0) -> float:
        ...
        for key in self.eval_output_info:
            logger.info(f"[Avg] {self.eval_output_info[key].avg_info}")
            # warning: the folowing code is only applicable to cases based on fourcastnet
            if "output_" in key:
                key_split = key.replace("(Sup_Validator)", "").split(".")
                if len(key_split) == 3:
                    metric_name, output_step, var_name = key_split
                    step = int(output_step.replace("output_", ""))
                    logger.scaler(
                        f"{metric_name}.{var_name}",
                        self.eval_output_info[key].avg,
                        step,
                        self.vdl_writer,
                    )
                else:
                    metric_name, output_step = key_split
                    step = int(output_step.replace("output_", ""))
                    logger.scaler(
                        f"{metric_name}",
                        self.eval_output_info[key].avg,
