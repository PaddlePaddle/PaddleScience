...
class BondEncoder(paddle.nn.Layer):
    ...
    def __init__(self, *, atom_graph_cutoff: float=5, bond_graph_cutoff:
        float=3, num_radial: int=9, cutoff_coeff: int=5, learnable: bool=False
        ) ->None:
        ...
        """
        super().__init__()
        self.rbf_expansion_ag = RadialBessel(num_radial=num_radial, cutoff=
            atom_graph_cutoff, smooth_cutoff=cutoff_coeff, learnable=learnable)
        self.rbf_expansion_bg = RadialBessel(num_radial=num_radial, cutoff=
            bond_graph_cutoff, smooth_cutoff=cutoff_coeff, learnable=learnable)

    def forward(self, center: paddle.Tensor, neighbor: paddle.Tensor,
        undirected2directed: paddle.Tensor, image: paddle.Tensor, lattice:
        paddle.Tensor) ->tuple[paddle.Tensor, paddle.Tensor, paddle.Tensor]:
        ...
        undirected_bond_lengths = paddle.index_select(x=bond_lengths, axis=
            0, index=undirected2directed)
...
class AngleEncoder(paddle.nn.Layer):
    ...
    def __init__(self, *, num_angular: int=9, learnable: bool=True) ->None:
        ...
        if num_angular % 2 != 1:
            raise ValueError(
                f'num_angular={num_angular!r} must be an odd integer')
        circular_harmonics_order = (num_angular - 1) // 2
        self.fourier_expansion = Fourier(order=circular_harmonics_order,
            learnable=learnable)

    def forward(self, bond_i: paddle.Tensor, bond_j: paddle.Tensor
        ) ->paddle.Tensor:
