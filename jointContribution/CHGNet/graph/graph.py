...
class Edge(ABC):
    ...
    def __init__(self, nodes: list, index: (int | None)=None, info: (dict |
        None)=None) ->None:
        """Initialize an Edge."""
        self.nodes = nodes
        self.index = index
    ...
    def __repr__(self) ->str:
        ...
        return (
            f'{type(self).__name__}(nodes={nodes!r}, index={index!r}, info={info!r})'
            )
...
class DirectedEdge(Edge):
    ...
    def make_undirected(self, index: int, info: (dict | None)=None
        ) ->UndirectedEdge:
    ...
    def __eq__(self, other: object) ->bool:
        ...
        if self.nodes == other.nodes and (none_img or all(self_img ==
            other_img)):
            print(
                '!!!!!! the two directed edges are equal but this operation is not supposed to happen'
                , file=sys.stderr)
            return True
        return self.nodes == other.nodes[::-1] and (none_img or all(
            self_img == -1 * other_img))
...
class Graph:
    ...
    def add_edge(self, center_index, neighbor_index, image, distance,
        ...
        this_directed_edge = DirectedEdge([center_index, neighbor_index],
            index=directed_edge_index, info={'image': image, 'distance':
            distance})
        tmp = frozenset([center_index, neighbor_index])
        if tmp not in self.undirected_edges:
            this_directed_edge.info['undirected_edge_index'] = len(self.
                undirected_edges_list)
            this_undirected_edge = this_directed_edge.make_undirected(index
                =len(self.undirected_edges_list), info={
            ...
            for undirected_edge in self.undirected_edges[tmp]:
                if abs(undirected_edge.info['distance'] - distance
                    ) < dist_tol and len(undirected_edge.info[
                    'directed_edge_index']) == 1:
                    added_dir_edge = self.directed_edges_list[undirected_edge
                        .info['directed_edge_index'][0]]
    ...
    def adjacency_list(self) ->tuple[list[list[int]], list[int]]:
        ...
        directed2undirected = [edge.info['undirected_edge_index'] for edge in
            self.directed_edges_list]
        return graph, directed2undirected

    def line_graph_adjacency_list(self, cutoff) ->tuple[list[list[int]],
        list[int]]:
        ...
        for u_edge in self.undirected_edges_list:
            ...
            for center, dir_edge in zip(u_edge.nodes, u_edge.info[
                'directed_edge_index'], strict=True):
                for directed_edges in self.nodes[center].neighbors.values():
                    for directed_edge in directed_edges:
                        ...
                        if directed_edge.info['distance'] < cutoff:
                            line_graph.append([center, u_edge.index,
                                dir_edge, directed_edge.info[
                                'undirected_edge_index'], directed_edge.index])
    ...
    def undirected2directed(self) ->list[int]:
        ...
        index.
        """
        return [undirected_edge.info['directed_edge_index'][0] for
            undirected_edge in self.undirected_edges_list]

    def as_dict(self) ->dict:
        """Return dictionary serialization of a Graph."""
        return {'nodes': self.nodes, 'directed_edges': self.directed_edges,
    ...
    def __repr__(self) ->str:
        ...
        return (
            f'Graph(num_nodes={num_nodes!r}, num_directed_edges={num_directed_edges!r}, num_undirected_edges={num_undirected_edges!r})'
            )
