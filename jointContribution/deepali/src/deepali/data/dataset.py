...
from deepali.core.typing import is_path_str
...
class Dataset(paddle.io.Dataset, metaclass=ABCMeta):
    ...
    def __init__(self, transforms: Optional[Union[Transform, Sequence[Transform]]] = None):
        ...
        if transforms is None:
            transform = paddle.nn.Sequential()
        elif isinstance(transforms, paddle.nn.Sequential):
            transform = transforms
        else:
            if not isinstance(transforms, (list, tuple)):
                transforms = [transforms]
            transform = paddle.nn.Sequential(*transforms)
    ...
    @overload
    def transform(self) -> paddle.nn.Sequential:
        ...
    ...
    ) -> Union[paddle.nn.Sequential, TDataset]:
    ...
    def transform_(
        ...
        *args: Union[Transform, Sequence[Transform], None],
        ...
        elif len(transforms) == 1 and isinstance(transforms[0], paddle.nn.Sequential):
            self._transform = transforms[0]
        else:
            self._transform = paddle.nn.Sequential(*transforms)
        return self
...
class GroupDataset(paddle.io.Dataset):
    ...
    def __getitem__(self, index: int) -> Subset[Dict[str, Any]]:
        ...
        return paddle.io.Subset(dataset=self.dataset, indices=indices)
