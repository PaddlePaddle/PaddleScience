...
SkipFunc = Callable[[Tensor], paddle.Tensor]
...
class DenseBlock(ReprWithCrossReferences, paddle.nn.Layer):
    ...
    @overload
    def __init__(self, *args: paddle.nn.Layer, join: Union[str, JoinFunc], dim: int) -> None:
        ...

    @overload
    def __init__(
        self, arg: Mapping[str, paddle.nn.Layer], join: Union[str, JoinFunc], dim: int
    ) -> None:
        ...
...
class SkipConnection(ReprWithCrossReferences, paddle.nn.Layer):
    ...
    def __init__(
        self,
        arg: Mapping[str, paddle.nn.Layer],
        name: str = "func",
        skip: Union[str, SkipFunc] = "identity",
        join: Union[str, JoinFunc] = "cat",
        dim: int = 1,
    ...
    def __init__(
        ...
        *args: Any,
        ...
        self.skip = skip
        self.join = join_func(join, dim=dim)
        self._sub_layers[self.name] = func

    @property
    def func(self) -> paddle.nn.Layer:
        return self._sub_layers[self.name]
