...
from deepali.utils.tensorboard import add_summary_images
from deepali.utils.tensorboard import escape_channel_index_format_string
...
def reset_optimizer_grads(engine: Engine, optimizer) -> None:
...
def write_summary_images(
    ...
    names: Optional[Union[Mapping[str, str], Sequence[str], Set[str]]] = None,
    prefix: Optional[str] = None,
    image_transform: Optional[Callable[[str, Tensor], paddle.Tensor]] = True,
    ...
    if isinstance(names, dict):
        names = {re_output_key_index.sub(".\\1", name): tag for name, tag in names.items()}
