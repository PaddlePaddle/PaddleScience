...
def bspline1d(stride: int, order: int = 4) -> paddle.Tensor:
    ...
    for _ in range(1, order + 1):
        kernel = (
            paddle.nn.functional.conv1d(x=kernel, weight=kernel_ones, padding=stride - 1) / stride
        )
...
def cubic_bspline(
    ...
    *args: int,
    ...
    """
    stride_ = cat_scalars(
        stride, *args, derivative=derivative, dtype="int32", device=paddle.CPUPlace()
    ).tolist()
    D = len(stride_)
    if D == 1:
...
def cubic_bspline2d(
    ...
    *args: int,
    ...
    if dtype is None:
        dtype = "float32"
    stride_ = cat_scalars(stride, *args, num=2, dtype="int32", device=paddle.CPUPlace())
...
def cubic_bspline3d(
    ...
    *args: int,
    ...
    if dtype is None:
        dtype = "float32"
    stride_ = cat_scalars(stride, *args, num=3, dtype="int32", device=paddle.CPUPlace())
...
def gaussian_kernel_radius(sigma: Union[Scalar, Array], factor: Scalar = 3) -> paddle.Tensor:
    ...
    if sigma.less_than(y=paddle.to_tensor(0)).astype("bool").any():
...
def gaussian(
    ...
    *args: Scalar,
    ...
    sigma_ = cat_scalars(sigma, *args, dtype=dtype, device=paddle.CPUPlace())
...
def gaussian1d(
    ...
    if sigma.less_than(y=paddle.to_tensor(0)):
        raise ValueError("Gaussian standard deviation must be non-negative")
    if dtype is not None and not paddle_aux.is_floating_point(dtype):
        raise TypeError("Gaussian kernel dtype must be floating point type")
    if radius is None:
...
def gaussian2d(sigma: Union[Scalar, Array], *args: Scalar, **kwargs) -> paddle.Tensor:
    ...
    sigma = cat_scalars(sigma, *args, num=2, device=paddle.CPUPlace())
...
def gaussian3d(sigma: Union[Scalar, Array], *args: Scalar, **kwargs) -> paddle.Tensor:
    ...
    sigma = cat_scalars(sigma, *args, num=3, device=paddle.CPUPlace())
    return gaussian(sigma, **kwargs)
