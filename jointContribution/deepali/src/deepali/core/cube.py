...
class Cube(object):
    ...
    def numpy(self) -> np.ndarray:
        ...
        return np.concatenate(
            [self._extent.numpy(), self._center.numpy(), self._direction.flatten().numpy()], axis=0
        )
    ...
    def direction_(self, arg: Union[Array, float], *args: float) -> Cube:
        ...
        if direction.ndim == 1:
            ...
            raise ValueError(f"Cube direction must be array or square matrix with numel={D * D}")
        with paddle.no_grad():
...
def cube_transform_points(
    points: paddle.Tensor, cube: Cube, axes: Axes, to_cube: Cube, to_axes: Optional[Axes] = None
):
    return cube.transform_points(points, axes=axes, to_axes=to_axes, to_cube=to_cube)


def cube_transform_vectors(
    vectors: paddle.Tensor, cube: Cube, axes: Axes, to_cube: Cube, to_axes: Optional[Axes] = None
):
