...
def distance_matrix(x: paddle.Tensor, y: paddle.Tensor) -> paddle.Tensor:
    ...
    out_dtype = x.dtype
    if not paddle_aux.is_floating_point(out_dtype):
        out_dtype = "float32"
    x = x.astype("float64")
    y = y.astype("float64")
    ...
    dist = (
        ...
        + y_norm
        - 2.0
        * paddle.bmm(x=x, y=paddle.transpose(x=y, perm=paddle_aux.transpose_aux_func(y.ndim, 1, 2)))
    )
...
def closest_point_distances(
    ...
    for i, points in enumerate(x.split(split_size, dim=1)):
        ...
        min_dists[:, j] = (paddle.min(x=dists, axis=2), paddle.argmin(x=dists, axis=2)).values
    return min_dists
...
def closest_point_indices(
    ...
    for i, points in enumerate(x.split(split_size, dim=1)):
        dists = distance_matrix(points, y)
        j = slice(i * split_size, i * split_size + tuple(points.shape)[1])
        indices[:, j] = (paddle.min(x=dists, axis=2), paddle.argmin(x=dists, axis=2)).indices
    return indices
...
def polyline_directions(
    ...
    start_38 = points.shape[dim] + 1 if 1 < 0 else 1
    start_39 = points.shape[dim] + 0 if 0 < 0 else 0
    d = paddle.slice(points, [dim], [start_38], [start_38 + n - 1]).sub(
        paddle.slice(points, [dim], [start_39], [start_39 + (n - 1)])
    )
    if normalize:
        d = paddle.nn.functional.normalize(x=d, p=2, axis=dim)
    if repeat_last:
        start_40 = d.shape[dim] + (n - 2) if n - 2 < 0 else n - 2
        d = paddle.concat(x=[d, paddle.slice(d, [dim], [start_40], [start_40 + 1])], axis=dim)
    return d
...
def polyline_tangents(
    ...
    dim = points.ndim - 2
    n = tuple(points.shape)[dim]
    start_41 = points.shape[dim] + 0 if 0 < 0 else 0
    start_42 = points.shape[dim] + 1 if 1 < 0 else 1
    d = paddle.slice(points, [dim], [start_41], [start_41 + n - 1]).sub(
        paddle.slice(points, [dim], [start_42], [start_42 + (n - 1)])
    )
    if normalize:
        d = paddle.nn.functional.normalize(x=d, p=2, axis=dim)
    if repeat_first:
        start_43 = d.shape[dim] + 0 if 0 < 0 else 0
        d = paddle.concat(x=[paddle.slice(d, [dim], [start_43], [start_43 + 1]), d], axis=dim)
    return d
