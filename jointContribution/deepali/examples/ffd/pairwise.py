...
def read_images(
    sample: Union[PathStr, Dict[str, PathStr]],
    names: Set[str],
...
def load_transform(path: PathStr, grid: Grid) -> SpatialTransform:
    ...
    def convert_matrix(matrix: paddle.Tensor, grid: Optional[Grid] = None) -> paddle.Tensor:
        if grid is None:
    ...
    path = Path(path)
    if path.suffix == ".pt":
        value = paddle.load(path=str(path))
    ...
    return DisplacementFieldTransform(target_grid, params=flow.tensor().unsqueeze(axis=0))
...
def get_post_transform(
    ...
    center_offset = (
        target_grid.world_to_cube(source_grid.center()).unsqueeze(axis=0) if align_centers else None
    )
    rotation_matrix = (
...
def get_normalize_config(
    ...
    for channel, (start, stop) in channels.items():
        start_62 = image.tensor().shape[0] + start if start < 0 else start
        data = paddle.slice(image.tensor(), [0], [start_62], [start_62 + (stop - start)])
...
def normalize_data_(
    ...
    if scale is None:
        scale = {}
    for channel, (start, stop) in channels.items():
        start_63 = image.tensor().shape[0] + start if start < 0 else start
        data = paddle.slice(image.tensor(), [0], [start_63], [start_63 + (stop - start)])
