...
TModule = TypeVar("TModule", bound=Layer)
...
class PairwiseImageRegistrationLoss(RegistrationLoss):
    ...
    def device(self) -> (paddle.CPUPlace, paddle.CUDAPlace, str):
        ...
        assert isinstance(device, (paddle.CPUPlace, paddle.CUDAPlace, str))
        return device
    ...
    def data_mask(
        ...
        start_64 = data.shape[1] + start if start < 0 else start
        mask = paddle.slice(data, [1], [start_64], [start_64 + (stop - start)])
        return mask > 0.9

    def overlap_mask(self, source: paddle.Tensor, target: paddle.Tensor) -> Optional[paddle.Tensor]:
    ...
    def loss_input(
        self, name: str, data: paddle.Tensor, channels: Dict[str, Union[int, Tuple[int, int]]]
    ) -> paddle.Tensor:
