...
double  f[M1][N1][Q], g[M1][N1][Q], u[M1][N1], v[M1][N1], p[M1][N1], A[M1][N1], tau[M1][N1], Sh[M1][N1][3], uans[M1];
...
void main()
{
  ...
  diag[0]=1.0/9; diag[1]=diag[2]=1.0/36; diag[3]=diag[5]=1.0/6; diag[4]=diag[6]=1.0/12; diag[7]=diag[8]=1.0/4;

  printf("mu_ori=%e, A0=%e, pindex=%f, dx=%e, dt=%e\n", mu_PL, A0, n_PL, dx, dt);
...
void lbini()
{
  ...
  for(j=0;j<M1;j++) for(i=0;i<N1;i++)
    {
	  u[j][i]=0.0; v[j][i]=0.0;  p[j][i]=rho0;
	  A[j][i]=A0; tau[j][i] = tau0;
...
double feq(int k, int y, int x)
{
  ...
  PHO=p[y][x]; U=u[y][x]; V=v[y][x]; At=A[y][x];
  eu=(e[k][0]*U+e[k][1]*V)*rc;
  uv=(U*U+V*V)*rcc;
...
double force(int k, int y, int x)
{
   ...
   U=u[y][x]; V=v[y][x]; rtau = 1.0/tau[y][x];
...
void Evol()
{
 ...
 for(j=1;j<M;j++) for(i=1;i<N;i++)
   {
       p[j][i]=f[j][i][0]+f[j][i][1]+f[j][i][2]+f[j][i][3]+f[j][i][4]+f[j][i][5]+f[j][i][6]+f[j][i][7]+f[j][i][8];
       ...
       v[j][i]=v[j][i]*c/rho0;
            ...
            Eps = Eps + tau*dt*F0 - rho0*tp[0]/cs2*0.5*(u[j][i]*u[j][i]+v[j][i]*v[j][i]);
...
//---left and right
 for(j=0; j<M1; j++)
   {
       u[j][0]=0.0;   v[j][0]=0.0; p[j][0] = p[j][1];
       A[j][0]=A[j][1];
	   Sh[j][0][0]=Sh[j][1][0];
	   Sh[j][0][1]=Sh[j][1][1];
	   Sh[j][0][2]=Sh[j][1][2];

	   u[j][N]=0.0; v[j][N]=0.0; p[j][N] = p[j][N-1];
	   A[j][N]=A[j][N-1];
	   Sh[j][N][0]=Sh[j][N-1][0];
	   Sh[j][N][1]=Sh[j][N-1][1];
	   Sh[j][N][2]=Sh[j][N-1][2];
...
//---upper and bottom
 for(i=0; i<N1; i++)
   {
       j = 0;
       u[j][i]=0.0;   v[j][i]=0.0; p[j][i] = p[j+1][i];
       A[j][i]=A[j+1][i];
	   Sh[j][i][0]=Sh[j+1][i][0];
	   Sh[j][i][1]=Sh[j+1][i][1];
	   Sh[j][i][2]=Sh[j+1][i][2];
       ...
       j = M;
       if(m <= 1000) u[j][i] = U0;
       else u[j][i]=U0*(1. - cosh(r*(i*dx - 0.5))/cosh(0.5*r));
       v[j][i]=0.0;
       p[j][i] = p[j-1][i];
	   A[j][i]=A[j-1][i];
	   Sh[j][i][0]=Sh[j-1][i][0];
		 ...
		 for(k=0;k<Q;k++)
		  {
             ...
             at00/=cst;   at01/=cst;  at10=at01;  at11/=cst;

			 Sh[j][i][0]=at00;  Sh[j][i][1]=at01; Sh[j][i][2]=at11;
