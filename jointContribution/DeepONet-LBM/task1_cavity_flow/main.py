...
def main():
    ...
    while n <= nmax:
        ...
        f_train, u_train, v_train = pd.to_tensor(f_train), pd.to_tensor(u_train), pd.to_tensor(v_train)
        out_B = model(f_train)
        out_B_u, out_B_v = out_B[:, :modes], out_B[:, modes:]
        u_pred = pd.einsum('bi,ni->bn', out_B_u, u_basis)
        v_pred = pd.einsum('bi,ni->bn', out_B_v, v_basis)
        loss = F.mse_loss(u_pred, u_train[:, :, 0]) + F.mse_loss(v_pred, v_train[:, :, 0])
        loss.backward()
        opt.step()
        opt.clear_grad()
        ...
        if n%100 == 0:
            ...
            print('Step: %d, Loss: %.3e, err_u: %.3e, err_v: %.3e, Time (secs): %.3f'%(n, float(loss), err_u, err_v, T))
