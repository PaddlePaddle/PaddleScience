...
double f[Q][NY][NX]={0.0}, g[Q][NY][NX]={0.0};//分配CPU分布函数 f(液) g(气)的空间
double phi[NY][NX]={0.0}, rho[NY][NX]={0.0}, p[NY][NX]={0.0}, mu[NY][NX]={0.0}; 
...
//////////////////////////////////////////////////////////
#define FEQ_0(phi,vx,vy,mu,A)     ( phi-5.0/9.0*A*mu )               //速度vx,vy都没有除以c, vv=1.5*(vx*vx+vy*vy)
...
#define FEQ_5(phi,vx,vy,mu,A)     ( 1.0/36.0*(A*mu+3.0*phi*( vx+vy)) )
#define FEQ_6(phi,vx,vy,mu,A)     ( 1.0/36.0*(A*mu+3.0*phi*(-vx+vy)) )
#define FEQ_7(phi,vx,vy,mu,A)     ( 1.0/36.0*(A*mu+3.0*phi*(-vx-vy)) )
#define FEQ_8(phi,vx,vy,mu,A)     ( 1.0/36.0*(A*mu+3.0*phi*( vx-vy)) )
//////////////////////////////////////////////////////////
#define GEQ_0(rho,p,vx,vy,vv)     ( 4.0/9.0*(3.0*p-rho*vv)-3*p )               //速度vx,vy都没有除以c, vv=1.5*(vx*vx+vy*vy)
#define GEQ_1(rho,p,vx,vy,vv)     ( 1.0/9.0*(3.0*p+rho*( 3.0*vx+4.5*vx*vx-vv)) )
#define GEQ_2(rho,p,vx,vy,vv)     ( 1.0/9.0*(3.0*p+rho*( 3.0*vy+4.5*vy*vy-vv)) )
#define GEQ_3(rho,p,vx,vy,vv)     ( 1.0/9.0*(3.0*p+rho*(-3.0*vx+4.5*vx*vx-vv)) )
#define GEQ_4(rho,p,vx,vy,vv)     ( 1.0/9.0*(3.0*p+rho*(-3.0*vy+4.5*vy*vy-vv)) )
#define GEQ_5(rho,p,vx,vy,vv)     ( 1.0/36.0*(3.0*p+rho*(3.0*( vx+vy)+4.5*( vx+vy)*( vx+vy)-vv)) )
#define GEQ_6(rho,p,vx,vy,vv)     ( 1.0/36.0*(3.0*p+rho*(3.0*(-vx+vy)+4.5*(-vx+vy)*(-vx+vy)-vv)) )
#define GEQ_7(rho,p,vx,vy,vv)     ( 1.0/36.0*(3.0*p+rho*(3.0*(-vx-vy)+4.5*(-vx-vy)*(-vx-vy)-vv)) )
#define GEQ_8(rho,p,vx,vy,vv)     ( 1.0/36.0*(3.0*p+rho*(3.0*( vx-vy)+4.5*( vx-vy)*( vx-vy)-vv)) )
//////////////////////////////////////////////////////////
...
#define WEQ_5(vx,vy,vv)     ( 1.0/36.0*(1.0+3.0*( vx+vy)+4.5*( vx+vy)*( vx+vy)-vv) )
#define WEQ_6(vx,vy,vv)     ( 1.0/36.0*(1.0+3.0*(-vx+vy)+4.5*(-vx+vy)*(-vx+vy)-vv) )
#define WEQ_7(vx,vy,vv)     ( 1.0/36.0*(1.0+3.0*(-vx-vy)+4.5*(-vx-vy)*(-vx-vy)-vv) )
#define WEQ_8(vx,vy,vv)     ( 1.0/36.0*(1.0+3.0*( vx-vy)+4.5*( vx-vy)*( vx-vy)-vv) )
//////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////
#define MGEQ_0(rho,p,vx,vy)     ( 0. )               //速度vx,vy都没有除以c, vv=1.5*(vx*vx+vy*vy)
...
//////////////////////////////////////////////////////////
#define MFF_0(phi,vx,vy,phi0,vx0,vy0)    ( 0. )               //速度vx,vy都没有除以c, vv=1.5*(vx*vx+vy*vy)
...
////////////////////////////////////////////////////////////////////////////                                              
#define F_d(k,y,x)    F_d[(k)*NYNX + (y)*NX + (x)] //分布函数F在y行x列k方向的offset 
#define G_d(k,y,x)    G_d[(k)*NYNX + (y)*NX + (x)] //分布函数F在y行x列k方向的offset 
...
///////////////////////////////////////////////////////////////////////////////////
__global__ void collision_propagation(double A, double MM, double w_f, double w_g, double ggy, double *phi_d, double *mu_d, double *rho_d, double *p_d, double *u_d, double *v_d, double *phi0_d, double *u0_d, double *v0_d, double *f_d, double *F_d, double *g_d, double *G_d);
__global__ void Macro_rho(double *phi_d, double *phi0_d, double *rho_d, double *f_d);
__global__ void Macro_mu(double Kappa, double beta, double *phi_d, double *mu_d);
__global__ void Macro_u(double ggy, double MM, double *phi_d, double *rho_d, double *p_d, double *mu_d, double *u_d, double *v_d, double *u0_d, double *v0_d, double *g_d);
...
int main( int argc, char **argv)
{
    ...
    cudaMemcpy( f_dev,    &f[0][0][0], sizeof(double)*Q*NY*NX, cudaMemcpyHostToDevice );
    cudaMemcpy( g_dev,    &g[0][0][0], sizeof(double)*Q*NY*NX, cudaMemcpyHostToDevice );
    cudaMemcpy( phi_dev,  &phi[0][0],  sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
	cudaMemcpy( rho_dev,  &rho[0][0],  sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
	cudaMemcpy( p_dev,    &p[0][0],    sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
	cudaMemcpy( phi0_dev, &phi0[0][0], sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
	cudaMemcpy( mu_dev,   &mu[0][0],   sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
	cudaMemcpy( u_dev,    &u[0][0],    sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
    cudaMemcpy( v_dev,    &v[0][0],    sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
    cudaMemcpy( u0_dev,   &u0[0][0],   sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
    cudaMemcpy( v0_dev,   &v0[0][0],   sizeof(double)*NY*NX,   cudaMemcpyHostToDevice );
 

    //time_begin=clock();
	while(m<Max)
	{
         collision_propagation<<<grid, threads>>>(A, MM, ww_f, ww_g, ggy, phi_dev, mu_dev, rho_dev, p_dev, u_dev, v_dev, phi0_dev, u0_dev, v0_dev, f_dev, F_dev, g_dev, G_dev);
	 	 Macro_rho<<<grid, threads>>>(phi_dev, phi0_dev, rho_dev, F_dev);
         Macro_mu<<<grid, threads>>>(Kappa, beta, phi_dev, mu_dev);
         Macro_u<<<grid, threads>>>(ggy, MM, phi_dev, rho_dev, p_dev, mu_dev, u_dev, v_dev, u0_dev, v0_dev, G_dev);
		...
		if(m%10000==0)
		{
             cudaMemcpy( &f[0][0][0], f_dev,  Q*NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
			 cudaMemcpy( &u[0][0],    u_dev,    NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
			 cudaMemcpy( &v[0][0],    v_dev,    NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
			 cudaMemcpy( &phi[0][0],  phi_dev,  NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
             cudaMemcpy( &phi0[0][0], phi0_dev, NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
			 cudaMemcpy( &rho[0][0],  rho_dev,  NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
			 cudaMemcpy( &p[0][0],    p_dev,    NY*NX*sizeof(double), cudaMemcpyDeviceToHost );
...
void init() //分布函数的初始化和flag的初始化
{
	...
	beta=12.0*sigma/(D*(phil-phig)*(phil-phig)*(phil-phig)*(phil-phig));  
	...
	for(j=1; j<=M+1; j++)
	{
		...
		//	h=865+5.36*cos(2.0*pi*(k-N16)/lamda+3.0);
          ...
          /*  if(j>=h&&j<=NY)
            ...
            mu0=4.*beta*(phi[j][k]-phil)*(phi[j][k]-phig)*(phi[j][k]-phim);
 	...
 	printf("rhotal_1=%lf Re=%lf ggy=%e Pe=%lf tau_f=%lf tau_g=%lf niu=%e MM=%lf A=%lf D=%lf sigma=%lf beta=%lf Kappa=%lf Ban=%lf t_s=%e\n", rhotal_1, Re, ggy, Pe, tau_f, tau_g, niu, MM, A, D, sigma, beta, Kappa, Ban, t_s);
}
...
{	
    ...
    double wx0, wy0, wx1, wy1, wx2, wy2, wx3, wy3, wx4, wy4, wx5, wy5, wx6, wy6, wx7, wy7, wx8, wy8;
	double vv; 
    ...
    //256方向
 	 ...
 	 else
	 {
      ...
      Fx=3.0*mu_d(y,x)*Dphix*rdt + 3.0*u_d(y,x)*(rhol-rhog)*MM*DDmu*rdt*rdt;//Fx=Fsx+Fax
	  Fy=3.0*mu_d(y,x)*Dphiy*rdt + 3.0*v_d(y,x)*(rhol-rhog)*MM*DDmu*rdt*rdt;

  	  vv=1.5*( u_d(y,x)*u_d(y,x)+v_d(y,x)*v_d(y,x) );
	
	  wx0=(WEQ_0(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_0(u_d(y,x),v_d(y,x),vv)-4.0/9.0)*rdt*Drhox);
	  wy0=(WEQ_0(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_0(u_d(y,x),v_d(y,x),vv)-4.0/9.0)*rdt*Drhoy);
	  wx1=(WEQ_1(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_1(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhox);
	  wy1=(WEQ_1(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_1(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhoy);
	  wx2=(WEQ_2(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_2(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhox);
	  wy2=(WEQ_2(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_2(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhoy);
	  wx3=(WEQ_3(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_3(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhox);
	  wy3=(WEQ_3(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_3(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhoy);
	  wx4=(WEQ_4(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_4(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhox);
	  wy4=(WEQ_4(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_4(u_d(y,x),v_d(y,x),vv)-1.0/9.0)*rdt*Drhoy);
	  wx5=(WEQ_5(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_5(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhox);
 	  wy5=(WEQ_5(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_5(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhoy);
	  wx6=(WEQ_6(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_6(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhox);
	  wy6=(WEQ_6(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_6(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhoy);
	  wx7=(WEQ_7(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_7(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhox);
 	  wy7=(WEQ_7(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_7(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhoy);
	  wx8=(WEQ_8(u_d(y,x),v_d(y,x),vv)*Fx                        +(WEQ_8(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhox);
	  wy8=(WEQ_8(u_d(y,x),v_d(y,x),vv)*(Fy+(rho_d(y,x)-rhom)*ggy)+(WEQ_8(u_d(y,x),v_d(y,x),vv)-1.0/36.0)*rdt*Drhoy);
	  ...
	  mf0=diag0*( mf0-s_f0*(mf0-MFEQ_0(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f0)*MFF_0(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf1=diag1*( mf1-s_f1*(mf1-MFEQ_1(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f1)*MFF_1(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf2=diag2*( mf2-s_f2*(mf2-MFEQ_2(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f2)*MFF_2(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf3=diag3*( mf3-s_f3*(mf3-MFEQ_3(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f3)*MFF_3(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf4=diag4*( mf4-s_f4*(mf4-MFEQ_4(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f4)*MFF_4(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf5=diag5*( mf5-s_f5*(mf5-MFEQ_5(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f5)*MFF_5(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
 	  mf6=diag6*( mf6-s_f6*(mf6-MFEQ_6(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f6)*MFF_6(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf7=diag7*( mf7-s_f7*(mf7-MFEQ_7(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f7)*MFF_7(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );
	  mf8=diag8*( mf8-s_f8*(mf8-MFEQ_8(phi_d(y,x),u_d(y,x),v_d(y,x),mu_d(y,x),A)) + (1-0.5*s_f8)*MFF_8(phi_d(y,x),u_d(y,x),v_d(y,x),phi0_d(y,x),u0_d(y,x),v0_d(y,x)) );

	  mg0=diag0*( mg0-s_g0*(mg0-MGEQ_0(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g0)*mGG0 );
	  mg1=diag1*( mg1-s_g1*(mg1-MGEQ_1(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g1)*mGG1 );
	  mg2=diag2*( mg2-s_g2*(mg2-MGEQ_2(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g2)*mGG2 );
	  mg3=diag3*( mg3-s_g3*(mg3-MGEQ_3(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g3)*mGG3 );
	  mg4=diag4*( mg4-s_g4*(mg4-MGEQ_4(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g4)*mGG4 );
	  mg5=diag5*( mg5-s_g5*(mg5-MGEQ_5(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g5)*mGG5 );
	  mg6=diag6*( mg6-s_g6*(mg6-MGEQ_6(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g6)*mGG6 );
	  mg7=diag7*( mg7-s_g7*(mg7-MGEQ_7(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g7)*mGG7 );
	  mg8=diag8*( mg8-s_g8*(mg8-MGEQ_8(rho_d(y,x),p_d(y,x),u_d(y,x),v_d(y,x))) + dt*(1-0.5*s_g8)*mGG8 );
      ...
      //0 1 3方向 
	  F_d[k] = f0;         G_d[k] = g0;
      F_d(1, y, xd ) = f1; G_d(1, y, xd ) = g1;
	  F_d(3, y, xs ) = f3; G_d(3, y, xs ) = g3;
      //2 5 6方向
	  if( (y+1)<=M1 ) 
	  { 
	    F_d(2, y+1, x ) = f2; G_d(2, y+1, x ) = g2;
	    F_d(5, y+1, xd) = f5; G_d(5, y+1, xd) = g5;
        F_d(6, y+1, xs) = f6; G_d(6, y+1, xs) = g6;  
	  }
      else
	  { 
	    F_d(4, y, x) = f2; G_d(4, y, x) = g2;
	    F_d(7, y, x) = f5; G_d(7, y, x) = g5;
        F_d(8, y, x) = f6; G_d(8, y, x) = g6;  
	  }
 	  //4 7 8方向
	  if( (y-1)>=1 )  
	  { 
	    F_d(4, y-1, x ) = f4; G_d(4, y-1, x ) = g4;
	    F_d(7, y-1, xs) = f7; G_d(7, y-1, xs) = g7;
	    F_d(8, y-1, xd) = f8; G_d(8, y-1, xd) = g8;
	  }	
	  else
	  { 
	    F_d(2, y, x) = f4; G_d(2, y, x) = g4; 
        F_d(5, y, x) = f7; G_d(5, y, x) = g7;
	    F_d(6, y, x) = f8; G_d(6, y, x) = g8;
	  }
  }
...
__global__ void Macro_mu(double Kappa, double beta, double *phi_d, double *mu_d)
{
	...
	if(x<=N+N16)
	{
	  ...
	  mu0=4*beta*(phi_d(y,x)-phil)*(phi_d(y,x)-phig)*(phi_d(y,x)-phim);
