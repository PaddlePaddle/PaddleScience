...
class DeepONet(pd.nn.Layer):
    ...
    def forward(self, Bin, Tin):
        ...
        out_B_u, out_B_v, out_B_p = out_B[:, :, :modes], out_B[:, :, modes:2*modes], out_B[:, :, 2*modes:]
        out_T_u, out_T_v, out_T_p = out_T[:, :, :modes], out_T[:, :, modes:2*modes], out_T[:, :, 2*modes:]
...
def main():
    ...
    x_train, x_eq_train, f_train, rRe_train, u_train, v_train = pd.to_tensor(x_train), pd.to_tensor(x_eq_train), pd.to_tensor(f_train), \
                                                                pd.to_tensor(rRe_train), pd.to_tensor(u_train), pd.to_tensor(v_train)
    ...
    while n <= nmax:
        ...
        f_train, rRe_train, u_train, v_train = pd.to_tensor(f_train), pd.to_tensor(rRe_train), pd.to_tensor(u_train), pd.to_tensor(v_train)
        '''
        out_B = fnn_B(f_train)
        out_T = fnn_T(x_train)
        ...
        eq_loss = eq1_loss + 0.1*(F.mse_loss(eq2, pd.zeros(shape=eq2.shape)) + F.mse_loss(eq3, pd.zeros(eq3.shape)))
        ...
        if n%100 == 0:
            ...
            print('Step: %d, Loss: %.3e, err_u: %.3e, err_v: %.3e, Time (secs): %.3f'\
