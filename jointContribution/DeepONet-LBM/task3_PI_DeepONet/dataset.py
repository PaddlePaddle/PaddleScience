...
class DataSet:
    def __init__(self, num, bs, modes):
        ...
        self.bs = bs
        self.modes = modes
        self.F_train, self.U_train, self.V_train, self.rRe, \
        self.F_test, self.U_test, self.V_test, \
        self.x_train, self.x_eq_train, self.x_test, \
    ...
    def PODbasis(self):
        ...
        save_dict = {'u_basis': u_basis_out, 'v_basis': v_basis_out, 'lam_u': self.lam_u, 'lam_v': self.lam_v}
    ...
    def load_data(self):
        ...
        f_train_mean = np.mean(np.reshape(a_train, (-1, s_f)), 0)
        f_train_std = np.std(np.reshape(a_train, (-1, s_f)), 0)
        ...
        return F_train, U_train, V_train, rRe, F_test, U_test, V_test, x_train, x_eq_train, x_test, \
               u_train_mean, u_train_std, v_train_mean, v_train_std, \
               u_basis, v_basis, lam_u, lam_v
    ...
    def minibatch(self):
        ...
        return self.x_train, self.x_eq_train, f_train, rRe_train, u_train, v_train, Xmin, Xmax
