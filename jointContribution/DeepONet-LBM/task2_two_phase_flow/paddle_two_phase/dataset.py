...
class DataSet:
    def __init__(self, num, bs, modes):
        ...
        self.bs = bs
        self.modes = modes
        self.F_train, self.U_train, \
        self.F_test, self.U_test, \
        self.x_train, \
        self.u_basis, \
        self.lam_u = self.load_data()
        '''
    ...
    def load_data(self):
        ...
        loc = np.cos(2*np.pi*x)
        a_train = np.arange(20., 1000., 20., dtype=np.float32).reshape((-1, 1))
        ...
        a_test = np.arange(55., 1000., 100., dtype=np.float32).reshape((-1, 1))
        print(a_test)
        ...
        return F_train, U_train, F_test, U_test, x_train, \
               u_basis, lam_u
