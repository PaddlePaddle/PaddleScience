...
int main( int argc, char **argv )
{
	...
	cudaMemcpy( f_dev,  &f[0][0][0], sizeof(double)*Q*NY2*NX2, cudaMemcpyHostToDevice );
...
loop:
	...
	while(n < nmax)//&&(err > 1.0e-5))
	{
		...
		if( n % T == 0 )
		{
		       cudaMemcpy( &f[0][0][0], f_dev, Q*NY2*NX2*sizeof(double), cudaMemcpyDeviceToHost );	
		       uc = UX(Mc, Nc); vc = VY(Mc, Nc);
   			   err = error();		
	...
	time_end = clock(); 
	printf( "\nThe computing time is: %f seconds ", (double)( time_end - time_begin ) / CLOCKS_PER_SEC );
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
