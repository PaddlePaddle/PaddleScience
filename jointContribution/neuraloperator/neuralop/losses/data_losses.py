...
class IregularLpqLoss(paddle.nn.Layer):
    ...
    def norm(self, x, vol_elm):
        if len(x.shape) > 1:
            s = paddle.sum(paddle.abs(x) ** self.q, axis=1, keepdim=False) ** (
                self.p / self.q
            )
        else:
...
def pressure_drag(
    ...
    direction = paddle.sum(
        inward_surface_normal * flow_direction_normal, axis=1, keepdim=False
    )
...
def friction_drag(
    ...
    const = 2.0 / (mass_density * (flow_speed**2) * reference_area)
    direction = paddle.sum(
        wall_shear_stress * flow_direction_normal, axis=1, keepdim=False
    )
...
class WeightedL2DragLoss(object):
    ...
    def __call__(
        ...
        **kwargs
        ...
        pred_stress_pad[:, 0] = pred_stress.view(
            -1,
        )

        truth_stress_pad = paddle.zeros((truth_stress.shape[0], 3))
        truth_stress_pad[:, 0] = truth_stress.view(
            -1,
        )
