...
def _transpose(tensor, dim0, dim1, group=None, async_op=False):
    ...
    x_send = [y for y in paddle.split(tensor, comm_size, dim=dim0)]
    x_recv = [paddle.empty_like(x_send[0]) for _ in range(comm_size)]

    # global transposition
    req = dist.alltoall(x_send, x_recv, group=group, async_op=not async_op)
...
def _gather(input_, dim_, group=None):
    ...
    output = paddle.concat(tensor_list, axis=dim_)
