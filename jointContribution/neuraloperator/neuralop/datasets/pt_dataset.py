...
def load_pt_traintestsplit(
    ...
    if subsample_fact is not None:
        assert (
            len(subsample_fact) == 2 or len(subsample_fact) == 3
        ), "Only 2D and 3D data supported for subsampling"

    if interp_res is not None:
        assert (
            len(interp_res) == 2 or len(interp_res) == 3
        ), "Only 2D and 3D data supported for interpolation"
        if len(interp_res) == 2:
    ...
    if n_train > 0:
        ...
        for j in range(n_tensors):
            ...
            if subsample_fact is not None:
                if len(subsample_fact) == 2:
                    current_data = current_data[
                        ..., :: subsample_fact[0], :: subsample_fact[1]
                    ]
                else:
                    current_data = current_data[
                        ...,
            ...
            if interp_res is not None:
                ...
                current_data = paddle.nn.functional.interpolate(
    ...
    if n_test > 0:
        ...
        for j in range(n_tensors):
            current_data = (
                data[labels[j]][n_train : (n_train + n_test), ...]
                .type(paddle.float32)
                .clone()
            )
