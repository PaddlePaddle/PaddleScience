...
class SparseStorage(object):
    ...
    @classmethod
    def empty(self):
        row = paddle.to_tensor([], dtype=paddle.int64)
        col = paddle.to_tensor([], dtype=paddle.int64)
    ...
    def row(self):
        ...
        rowptr = self._rowptr
        if rowptr is not None:
            row = paddle.ops.torch_sparse.ptr2ind(rowptr, self._col.numel())
            self._row = row
            return row
    ...
    def rowptr(self) -> paddle.Tensor:
        ...
        row = self._row
        if row is not None:
            rowptr = paddle.ops.torch_sparse.ind2ptr(row, self._sparse_sizes[0])
            self._rowptr = rowptr
            return rowptr
    ...
    def set_value_(self, value: Optional[paddle.Tensor],
    ...
    def set_value(self, value: Optional[paddle.Tensor],
                  layout: Optional[str] = None):
        if value is not None:
            if get_layout(layout) == 'csc':
                value = value[self.csc2csr()]
    ...
    def sparse_resize(self, sparse_sizes: Tuple[int, int]):
        ...
        if diff_0 > 0:
            if rowptr is not None:
                rowptr = paddle.concat([rowptr, rowptr.new_full((diff_0, ), nnz)])
            if rowcount is not None:
                rowcount = paddle.concat([rowcount, rowcount.new_zeros(diff_0)])
        elif diff_0 < 0:
            if rowptr is not None:
        ...
        if diff_1 > 0:
            if colptr is not None:
                colptr = paddle.concat([colptr, colptr.new_full((diff_1, ), nnz)])
            if colcount is not None:
                colcount = paddle.concat([colcount, colcount.new_zeros(diff_1)])
        elif diff_1 < 0:
            if colptr is not None:
    ...
    def colptr(self) -> paddle.Tensor:
        ...
        if csr2csc is not None:
            colptr = paddle.ops.torch_sparse.ind2ptr(self._col[csr2csc],
                                                    self._sparse_sizes[1])
        else:
            colptr = self._col.new_zeros(self._sparse_sizes[1] + 1)
            paddle.cumsum(self.colcount(), dim=0)
        self._colptr = colptr
        return colptr
    ...
    def colcount(self) -> paddle.Tensor:
        ...
        if colptr is not None:
            ...
            colcount = scatter_add(paddle.ones_like(self._col), self._col,
    ...
    def coalesce(self, reduce: str = "add"):
        ...
        if value is not None:
            ...
            ptr = paddle.concat([ptr, ptr.new_full((1, ), value.size(0))])
    ...
    def type_as(self, tensor: paddle.Tensor):
        value = self._value
        if value is not None:
