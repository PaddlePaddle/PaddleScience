...
class SinusoidalPositionalEncoding(nn.Layer):
    def __init__(self, d_model: int, max_len: int = 4096):
        ...
        div_term = np.exp(
            np.arange(0, d_model, 2, dtype="float32") * (-math.log(10000.0) / d_model)
        )
...
class ResNet3D(nn.Layer):
    def __init__(self, block, layers, in_channels=20, base_width=64):
        ...
        self.layer2 = self._make_layer(
            block, base_width * 2, layers[1], stride=(2, 2, 2)
        )
        self.layer3 = self._make_layer(
            block, base_width * 4, layers[2], stride=(2, 2, 2)
        )
        self.layer4 = self._make_layer(
            block, base_width * 8, layers[3], stride=(2, 2, 2)
        )
...
class Volume3DEncoder(nn.Layer):
    def __init__(self, in_channels: int = 20, base: int = 64, dropout: float = 0.0):
        ...
        self.backbone = ResNet3D(
            BasicBlock3D, layers=[2, 2, 2, 2], in_channels=in_channels, base_width=base
        )
...
class MoE(nn.Layer):
    def __init__(self, d_model: int, cfg: MoEConfig):
        ...
        self.experts = nn.LayerList(
            [ExpertFFN(d_model, cfg.d_ff, cfg.dropout) for _ in range(cfg.n_experts)]
        )
    ...
    def forward(self, x, domain_id=None):
        ...
        probs = self._router_probs(logits)
        ...
        if self.cfg.diversity_w > 0 and self.cfg.n_experts > 1:
            chosen = F.one_hot(topk_idx[:, 0], num_classes=self.cfg.n_experts).astype(
                "float32"
            )
        ...
        self.layers = nn.LayerList(
            [
                TransformerEncoderLayerMoE(
                    d_model, nhead, d_ff, dropout, use_moe=use_moe, moe_cfg=moe_cfg
                )
...
class AFNO1DLayer(nn.Layer):
    def __init__(
        ...
        self.w1r = self.create_parameter(
            [num_blocks, self.block, self.block], default_initializer=param()
        )
        self.w1i = self.create_parameter(
            [num_blocks, self.block, self.block], default_initializer=param()
        )
        self.w2r = self.create_parameter(
            [num_blocks, self.block, self.block], default_initializer=param()
        )
        self.w2i = self.create_parameter(
            [num_blocks, self.block, self.block], default_initializer=param()
        )
        self.ln = nn.LayerNorm(d_model)
        self.drop = nn.Dropout(dropout)
...
class AFNOTransformerFlexible(nn.Layer):
    def __init__(
        ...
        self.layers = nn.LayerList(
            [
                AFNO1DLayer(
                    d_model, modes=modes, num_blocks=8, shrink=0.01, dropout=dropout
                )
                for _ in range(num_layers)
            ]
        )
...
class BiModalCrossFusion(nn.Layer):
    def __init__(self, d_model=512, nhead=8, dropout=0.1, fuse_hidden=512):
        ...
        self.fuse = nn.Sequential(
            nn.Linear(2 * d_model, fuse_hidden), nn.ReLU(), nn.Dropout(dropout)
        )
...
class TwoModalMultiLabelModel(nn.Layer):
    def __init__(
        ...
        self.trans_attn = TemporalTransformerFlexible(
            ...
            dropout=dropout,
            max_len=vid_frames,
            use_moe=moe_temporal_attn,
            moe_cfg=moe_cfg_temporal_attn
            or MoEConfig(
                n_experts=8,
                top_k=1,
                d_ff=max(2048, trans_ff),
                router_temp=0.5,
                balance_loss_w=0.005,
                entropy_reg_w=-0.005,
                diversity_w=1e-3,
        ...
        self.trans_afno = AFNOTransformerFlexible(
            ...
            dropout=dropout,
            d_ff=trans_ff,
            use_moe=moe_temporal_afno,
            moe_cfg=moe_cfg_temporal_afno
            or MoEConfig(
                n_experts=8,
                top_k=1,
                d_ff=max(2048, trans_ff),
...
class Retriever:
    ...
    def query_and_fuse(
        ...
        if self.sim_metric == "cos":
            ...
            k2 = paddle.sum(self.keys * self.keys, axis=-1, keepdim=True).transpose(
                [1, 0]
            )
