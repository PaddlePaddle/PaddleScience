...
import sys
sys.path.append('/mnt/2T-disk/vscode/neuraloperator-0.3.0-MyConvert/data/qikai/PaddlePaddle_Competition/neuraloperator-0.3.0-MyConvert') 

sys.path.append('/mnt/2T-disk/vscode/neuraloperator-0.3.0-MyConvert/data/qikai/PaddlePaddle_Competition/neuraloperator-0.3.0-MyConvert/neuralop') 
import numpy as np
import paddle
import matplotlib
...
gridx = paddle.to_tensor(np.linspace(-1, 1, size_x), dtype=paddle.float64)
gridx = paddle.tile(paddle.reshape(gridx, [1, size_x, 1]), [batchsize, 1, size_y])
gridy = paddle.to_tensor(np.linspace(-1, 1, size_y), dtype=paddle.float64)
gridy = paddle.tile(paddle.reshape(gridy, [1, 1, size_y]), [batchsize, size_x, 1])
