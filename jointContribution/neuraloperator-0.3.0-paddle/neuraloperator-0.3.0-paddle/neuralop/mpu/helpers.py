...
#paddle不支持下面contiguous_format,is_contiguous,channels_last
def get_memory_format(tensor):
    if tensor.is_contiguous(memory_format=paddle.channels_last):
...
def split_tensor_along_dim(tensor, dim, num_chunks):
    ...
    tensor_list = paddle.split(tensor, chunk_size, axis=dim)
...
def _transpose(tensor, dim0, dim1, group=None, async_op=False):
    ...
    x_send = [y.contiguous(memory_format=input_format) for y in paddle.split(tensor, split_size, dim=dim0)]
...
def _gather(input_, dim_, group=None):
    ...
    # Note: paddle.cat already creates a contiguous tensor.
    output = paddle.concat(tensor_list, axis=dim_).contiguous(memory_format=input_format)
