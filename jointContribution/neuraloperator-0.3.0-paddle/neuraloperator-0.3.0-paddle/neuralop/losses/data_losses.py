...
class LpLoss(object):
    ...
    def abs(self, x, y, h=None):
        ...
        diff = const*paddle.linalg.norm(paddle.flatten(x, start_axis=-self.d) - paddle.flatten(y, start_axis=-self.d), \
    ...
    def rel(self, x, y):
        ...
        diff = paddle.linalg.norm(paddle.flatten(x, start_axis=-self.d) - paddle.flatten(y, start_axis=-self.d), \
                          p=self.p, axis=-1, keepdim=False)
        ynorm = paddle.linalg.norm(paddle.flatten(y, start_axis=-self.d), p=self.p, axis=-1, keepdim=False)
...
class H1Loss(object):
    ...
    def abs(self, x, y, h=None):
        ...
        const = math.prod(h)
        diff = const*paddle.linalg.norm(dict_x[0] - dict_y[0], p=2, axis=-1, keepdim=False)**2

        for j in range(1, self.d + 1):
            diff += const*paddle.linalg.norm(dict_x[j] - dict_y[j], p=2, axis=-1, keepdim=False)**2
    ...
    def rel(self, x, y, h=None):
        ...
        diff = paddle.linalg.norm(dict_x[0] - dict_y[0], p=2, axis=-1, keepdim=False)**2
        ynorm = paddle.linalg.norm(dict_y[0], p=2, axis=-1, keepdim=False)**2

        for j in range(1, self.d + 1):
            diff += paddle.linalg.norm(dict_x[j] - dict_y[j], p=2, axis=-1, keepdim=False)**2
            ynorm += paddle.linalg.norm(dict_y[j], p=2, axis=-1, keepdim=False)**2
...
class IregularLpqLoss(paddle.nn.Layer):
    ...
    def norm(self, x, vol_elm):
        if len(x.shape) > 1:
            s = paddle.sum(paddle.abs(x)**self.q, dim=1, keepdim=False)**(self.p/self.q)
        else:
...
def pressure_drag(pressure, vol_elm, inward_surface_normal, 
    ...
    direction = paddle.sum(inward_surface_normal*flow_direction_normal, dim=1, keepdim=False)
...
def friction_drag(wall_shear_stress, vol_elm, 
    ...
    const = 2.0/(mass_density*(flow_speed**2)*reference_area)
    direction = paddle.sum(wall_shear_stress*flow_direction_normal, dim=1, keepdim=False)
...
class WeightedL2DragLoss(object):
    ...
    def __init__(self, mappings: dict, device: str = 'gpu'):
        ...
        device : str, optional
            device on which to do tensor calculations, by default 'gpu'
        """
    ...
    def __call__(self, y_pred, y, vol_elm, inward_normals, flow_normals, flow_speed, reference_area, **kwargs):
        ...
        pred_stress_pad = paddle.zeros((pred_stress.shape[0], 3), device=self.device)
        pred_stress_pad[:,0] = pred_stress.view(-1,)

        truth_stress_pad = paddle.zeros((truth_stress.shape[0], 3), device=self.device)
