...
class UnitGaussianNormalizer(Transform):
    ...
    def update_mean_std(self, data_batch):
        ...
        if self.mask is None:
            ...
            self.n_elements = paddle.count_nonzero(self.mask, dim=dim)*batch_size
            self.mean = paddle.zeros(shape)
            self.std = paddle.zeros(shape)
            self.squared_mean = paddle.zeros(shape)
            data_batch[:, self.mask==1] = 0
            self.mean[self.mask == 1] = paddle.sum(data_batch, dim=dim, keepdim=True) / self.n_elements
            self.squared_mean = paddle.sum(data_batch**2, dim=dim, keepdim=True) / self.n_elements
            self.std = paddle.sqrt(self.squared_mean - self.mean**2)
    ...
    def incremental_update_mean_std(self, data_batch):
        if self.mask is None:
            ...
            dim = [i - 1 for i in self.dim if i]
            n_elements = paddle.count_nonzero(self.mask, dim=dim)*data_batch.shape[0]
            data_batch[:, self.mask == 1] = 0

        self.mean = (1.0/(self.n_elements + n_elements))*(
            self.n_elements*self.mean + paddle.sum(data_batch, dim=dim, keepdim=True))
        self.squared_mean = (1.0/(self.n_elements + n_elements - 1))*(
            self.n_elements*self.squared_mean + paddle.sum(data_batch**2, dim=dim, keepdim=True))
