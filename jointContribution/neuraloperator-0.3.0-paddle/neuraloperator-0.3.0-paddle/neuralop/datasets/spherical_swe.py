...
import paddle
# from ppsci.arch.paddle_harmonics import ShallowWaterSolver
...
def load_spherical_swe(n_train, n_tests, batch_size, test_batch_sizes,
    ...
    train_loader = paddle.io.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0, persistent_workers=False)
    ...
    for (res, n_test, test_batch_size) in zip(test_resolutions, n_tests, test_batch_sizes):
        ...
        test_loader = paddle.io.DataLoader(test_dataset, batch_size=test_batch_size, shuffle=True, num_workers=0, persistent_workers=False)
...
class SphericalSWEDataset(paddle.io.Dataset):
    ...
    def __init__(self, dt=3600, dims=(256, 512), initial_condition='random', num_examples=32,
                 device=paddle.CPUPlace(), normalize=True, stream=None):
