...
def load_pt_traintestsplit(data_path, 
    ...
    if n_train > 0:
        ...
        for j in range(n_tensors):
            current_data = data[labels[j]][0:n_train, ...].type(paddle.float64).clone()
    ...
    if n_test > 0:
        ...
        test_transforms = []
        for j in range(n_tensors):
            current_data = data[labels[j]][n_train:(n_train + n_test), ...].type(paddle.float64).clone()
    ...
    if train_data is not None:
        ...
        train_loader = paddle.io.DataLoader(train_db,
