...
def load_burgers_1dtime(
    ...
    grid_x = paddle.to_tensor(np.linspace(domain_starts[0], domain_lengths[0], spatial_length + 1)[:-1],dtype=paddle.float64)
    grid_t = paddle.to_tensor(np.linspace(domain_starts[1], domain_lengths[1], temporal_length),dtype=paddle.float64)

    grid_x = grid_x.reshape([1, 1, spatial_length])
    grid_t = grid_t.reshape([1, temporal_length, 1])

    x_train = x_train.reshape([n_train, 1, spatial_length]).tile([1, temporal_length, 1])
    x_test = x_test.reshape([n_test, 1, spatial_length]).tile([1, temporal_length, 1])

    # TODO: add option to not have positional encoding
    x_train = paddle.stack([x_train,
                            grid_t.tile([n_train, 1, spatial_length]),
                            grid_x.tile([n_train, temporal_length, 1]),
                            ],axis=3)
    x_test = paddle.stack([x_test,
    ...
    test_loader = paddle.io.DataLoader(test_db, batch_size=batch_size_test, shuffle=False)
