...
def load_darcy_pt(
    ...
    data = paddle.load(Path(data_path).joinpath(f"darcy_train_{train_resolution}.pdtensor").as_posix())
    x_train = (
        paddle.to_tensor(data["x"][0:n_train, :, :])
        .astype(paddle.float64) 
    )
    x_train = paddle.unsqueeze(x_train, axis=channel_dim)  
    y_train = (
        paddle.to_tensor(data["y"][0:n_train, :, :])
    )
    y_train = paddle.unsqueeze(y_train, axis=channel_dim) 
    ...
    # data = paddle.load(
    #     Path(data_path).joinpath(f"darcy_test_{train_resolution}.npy").as_posix()
    # )
    # x_test = data["x"][:n_test, :, :].unsqueeze(channel_dim).type(paddle.float32).clone()
    # y_test = data["y"][:n_test, :, :].unsqueeze(channel_dim).clone()
    # del data

    data = paddle.load(Path(data_path).joinpath(f"darcy_test_{train_resolution}.pdtensor").as_posix())
    x_test = (
        paddle.to_tensor(data["x"][0:n_test, :, :])
        .astype(paddle.float64) 
    )
    x_test = paddle.unsqueeze(x_test, axis=channel_dim)  
    y_test = (
        paddle.to_tensor(data["y"][0:n_test, :, :])
    )
    y_test = paddle.unsqueeze(y_test, axis=channel_dim)  
