...
class RandomMGPatch():
    ...
    def __call__(self, data):
        ...
        pos_h = paddle.randint(low=0, high=height, size=(1,))[0]
        pos_w = paddle.randint(low=0, high=width, size=(1,))[0]

        shift_h = center_h - pos_h
        shift_w = center_w - pos_w

        shifted_x = paddle.roll(x, (shift_h, shift_w), dims=(0, 1))
        patches_x = _get_patches(shifted_x, self.step, height, width)
        shifted_y = paddle.roll(y, (shift_h, shift_w), dims=(0, 1))
        patches_y = _get_patches(shifted_y, self.step, height, width)

        return paddle.concat(patches_x, dim=0), patches_y[-1]
