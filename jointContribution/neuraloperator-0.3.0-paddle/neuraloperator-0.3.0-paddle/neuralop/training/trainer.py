    ...
    def train(self, train_loader, test_loaders,
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx, sample in enumerate(train_loader):
                sample = {k: paddle.cast(v, dtype='float64') if isinstance(v, paddle.Tensor) else v for k, v in sample.items()}
                if self.callbacks:
                ...
                if self.data_processor is not None:
                    ...
                    sample = {k:v.to(self.device) for k,v in sample.items() if paddle.is_tensor(v)}

                if self.amp_autocast:
                    with paddle.amp.autocast(enabled=True):
                ...
                if self.overrides_loss:
                    if isinstance(out, paddle.Tensor):
                        loss += self.callbacks.compute_training_loss(out=out.astype('float32'), **sample, amp_autocast=self.amp_autocast)
                    ...
                    if self.amp_autocast:
                        with paddle.amp.autocast(enabled=True):
                            if isinstance(out, paddle.Tensor):
                                loss = training_loss(out.astype('float32'), **sample)
                            elif isinstance(out, dict):
                        ...
                        if isinstance(out, paddle.Tensor):
                            ...
                            loss += training_loss(y_pred=out['y'], y=sample['y'])
                if regularizer:
