...
class CheckpointCallback(Callback):
        ...
        if self.resume_from_dir:
            ...
            if 'best_model_state_dict' in saved_modules:
                if hasattr(self.state_dict['model'], 'load_checkpoint'):
                    ...
                    self.state_dict['model'].load_state_dict(paddle.load(self.resume_from_dir / 'best_model.pt'))
                if verbose:
                ...
                if hasattr(self.state_dict['model'], 'load_checkpoint'):
                    ...
                    self.state_dict['model'].load_state_dict(paddle.load(self.resume_from_dir / 'model.pt'))
                if verbose:
            ...
            for module in ['optimizer', 'scheduler', 'regularizer']:
                if module in saved_modules:
                    self.state_dict[module].load_state_dict(paddle.load(self.resume_from_dir / f"{module}.pt"))
    ...
    def on_epoch_end(self, *args, **kwargs):
        ...
        if self.state_dict['epoch'] % self.save_interval == 0 and metric_cond:
            ...
            save_path = Path(self.save_dir) / f"{model_name}.pt"
            if hasattr(self.state_dict['model'], 'save_checkpoint'):
                self.state_dict['model'].save_checkpoint(self.save_dir, model_name)
            else:
                paddle.save(self.state_dict['model'].state_dict(), str(save_path))

            # save optimizer, scheduler, regularizer according to flags
            if self.save_optimizer:
                save_path = Path(self.save_dir) / "optimizer.pt"
                paddle.save(self.state_dict['optimizer'].state_dict(), str(save_path))
            if self.save_scheduler:
                save_path = Path(self.save_dir) / "scheduler.pt"
                paddle.save(self.state_dict['scheduler'].state_dict(), str(save_path))
            if self.save_regularizer:
                save_path = Path(self.save_dir) / "regularizer.pt"
                paddle.save(self.state_dict['regularizer'].state_dict(), str(save_path))
