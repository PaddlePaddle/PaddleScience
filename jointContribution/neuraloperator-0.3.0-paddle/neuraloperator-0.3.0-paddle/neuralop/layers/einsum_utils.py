...
def einsum_complexhalf_two_input(eq, a, b):
    ...
    res = paddle.stack([tmp[0, 0, ...] - tmp[1, 1, ...], tmp[1, 0, ...] + tmp[0, 1, ...]], dim=-1)
    return paddle.as_complex(res)
...
def einsum_complexhalf(eq, *args):
    ...
    for partial_eq in partial_eqns:
        ...
        result = paddle.stack([tmp[0, 0, ...] - tmp[1, 1, ...], tmp[1, 0, ...] + tmp[0, 1, ...]], dim=-1)
        tensors[out_label] = result

    return paddle.as_complex(tensors[output_label])