...
class PysicsInformedNeuralNetwork:
    ...
    def __init__(self, layers=4, hidden_size=40, num_ins=2, num_outs=3,
                 learning_rate=0.001, opt=None, net_params=None, N_f=None,
                 checkpoint_freq=2000, checkpoint_path='./checkpoint/',
                 t_min=None, t_max=None, distributed_env=False, mode='train'):
        ...
        self.net = self.initialize_NN(num_ins=num_ins, num_outs=num_outs, num_layers=layers, hidden_size=hidden_size)

        if mode == 'train':
            self.k1 = paddle.create_parameter(shape=[1], dtype='float32', 
                                              default_initializer=nn.initializer.Constant(value=-4.0))
            self.k2 = paddle.create_parameter(shape=[1], dtype='float32', 
    ...
    def set_eta_data(self, X=None, continuous_time=True):
        stop_gradient = True
        self.t_eta = paddle.to_tensor(X[0], dtype='float32', stop_gradient=False)
    ...
    def train(self, num_epoch=1, optimizer=None, batchsize=None, scheduler=None):
        self.opt = optimizer
        self.scheduler = scheduler
        if isinstance(self.opt, paddle.optimizer.AdamW) or isinstance(self.opt, paddle.optimizer.Adam):
            return self.solve_Adam(self.fwd_computing_loss_2d, num_epoch, batchsize)
    ...
    def solve_Adam(self, loss_func, num_epoch=1000, batchsize=50000):
        ...
        for epoch_id in range(num_epoch):
            ...
            print("epoch/num_epoch: ", epoch_id + 1, "/", num_epoch, "loss[Adam]: ", loss.numpy()[0], 
                  "k1:", k1__, "k2:", k2__,
                  "eta loss:", losses[0].numpy()[0], "eq_loss:", losses[1].numpy()[0])
            if (epoch_id + 1) % self.checkpoint_freq == 0:
                  paddle.save(self.net.state_dict(), self.checkpoint_path + 'net_params_' + str(epoch_id + 1))
