...
class PysicsInformedNeuralNetwork:
    ...
    def __init__(self, layers=10, learning_rate=0.001, weight_decay=0.9, outlet_weight=1,
        ...
        self.nu = nu
        self.training_domain_t_start = None
        self.training_domain_t_end = None
        self.training_domain_t_interval = None
    ...
    def fwd_computing_loss_2d(self):
        ...
        if self.training_type == 'half-supervised':
            ...
            # self.loss_s = paddle.norm((self.p_s.reshape([-1]) - self.p_pred_s.reshape([-1])), p=2)
            self.loss_s = paddle.norm((self.u_s.reshape([-1]) - self.u_pred_s.reshape([-1])), p=2) + \
                          ...
                          paddle.norm((self.p_s.reshape([-1]) - self.p_pred_s.reshape([-1])), p=2)
            '''
    ...
    def train(self, num_epoch=1, optimizer=None, scheduler=None, batchsize=None, checkpoint_freq=1000, checkpoint_path='./checkpoint/'):
        self.opt = optimizer
        if isinstance(self.opt, paddle.optimizer.AdamW) or isinstance(self.opt, paddle.optimizer.Adam):
            return self.solve_Adam(self.fwd_computing_loss_2d, num_epoch, batchsize, scheduler,
                    checkpoint_freq, checkpoint_path=checkpoint_path)
        elif self.opt is paddle.incubate.optimizer.functional.minimize_bfgs:
            return self.solve_bfgs(self.fwd_computing_loss_2d, num_epoch, batchsize, checkpoint_path=checkpoint_path)

    def solve_Adam(self, loss_func, num_epoch=1000, batchsize=50000, scheduler=None,
                   checkpoint_freq=200, distributed=False, checkpoint_path='./checkpoint/'):
        if distributed:
            ...
            for epoch_id in range(num_epoch):
                ...
                if (epoch_id + 1) % checkpoint_freq == 0:
                      paddle.save(self.net.state_dict(), checkpoint_path + 'net_params_' + str(epoch_id + 1))
    '''
    def train(self, num_epoch=1, optimizer=None, batch_size=None, checkpoint_freq=1000, checkpoint_path='./checkpoint/'):
        self.opt = optimizer
        if isinstance(self.opt, paddle.optimizer.AdamW) or isinstance(self.opt, paddle.optimizer.Adam):
            return self.solve_Adam(self.fwd_computing_loss_2d, num_epoch, batch_size, checkpoint_freq, checkpoint_path=checkpoint_path)
        elif self.opt is paddle.incubate.optimizer.functional.minimize_bfgs:
            return self.solve_bfgs(self.fwd_computing_loss_2d, num_epoch, batch_size, checkpoint_path=checkpoint_path)

    def solve_Adam(self, loss_func, num_epoch=1000, batch_size=None, checkpoint_freq=200, distributed=False, checkpoint_path='./checkpoint_1/'):
        if distributed:
            for epoch_id in range(num_epoch):
                with self.model.no_sync():
                    loss, losses = loss_func()
            ...
            for epoch_id in range(num_epoch):
                ...
                if (epoch_id + 1) % checkpoint_freq == 0:
                    paddle.save(self.net.state_dict(),
                                checkpoint_path + 'net_params_' + str(epoch_id + 1))
                    paddle.save(self.opt.state_dict(),
                                checkpoint_path + 'opt_params_' + str(epoch_id + 1))
    '''

    def solve_bfgs(self, loss_func, num_epoch=1000, batch_size=None, checkpoint_freq=1000, checkpoint_path='./checkpoint/'):
        batch_id = 0
        step = 0
        loss = None
        losses = []
